...
folder_path = 'C:\\work\\JTCMM\\test\\학원\\'+ today.strftime('%Y') + '년\\' + amonth_ago.strftime('%#m월')
...
def make_fail_xlsx(cms_xls):
    ...
    amonth_ago_fail_xls = root_path+ payment_date_ago.strftime('%Y') + '년\\' + payment_date_ago.strftime('%#m') + '월\\' + payment_date_ago.strftime('%m') + '99. ' + payment_date_ago.strftime('%y%m') + '99 학원계산서 (★' + payment_date_ago.strftime('%#m') + '월 서비스이용료)- 출금실패.xls'
    two_month_ago_fail_xls = root_path + payment_date_twoago.strftime('%Y') + '년\\' + payment_date_twoago.strftime('%#m') + '월\\' + payment_date_twoago.strftime('%m') + '99. ' + payment_date_twoago.strftime('%y%m') + '99 학원계산서 (★' + payment_date_twoago.strftime('%#m') + '월 서비스이용료)- 출금실패.xls'
...
def cms_check(fail_xlsx_list, cms_xlsx_path):
    ...
    for i in range(2, ws_cms.i + 1):
        ...
        if s_con !='결제실패' and s_con !='결제완료':
            ...
            jtt_xslx_path = root_path + f'{payment_date.year}년\\{payment_date.month}월\\xlsx\\' + payment_date.strftime('%m') + payment_date.strftime('%d') + '. ' + payment_date_b + ' 학원계산서 ' + '(' + str(payment_date.month) + '월 서비스이용료).xlsx'
            # 일반 xls파일
...
if __name__ == "__main__":
    ...
    print(start_time)
    main(r'C:\Users\vivans\Desktop\stu_test\20230701_20230701_학원.xls')
