...
def xlsx2xls(xlsx_path,payment_date):
    ...
    payment_date_threeago = payment_date - relativedelta(months=3)
    today_month = payment_date.strftime('%m')
    one_month_ago = payment_date_ago.strftime('%m')
    two_month_ago = payment_date_twoago.strftime('%m')
    three_month_ago = payment_date_threeago.strftime('%m')
    for z in xlsx_path:
        if "월분이용료" not in z:
            pass
        else:
            ...
            if y_m == 0:
                ...
                if yy =='22':
                    ...
                    xls2_path = root_path + '20' + yy + '년' + '\\' + '20' + yy + '.' + one_month_ago + '\\' + f'{file_name[:-1]}'
                elif mm == one_month_ago:
                    xls2_path = root_path + '20' + yy + '년' + '\\' + '20' + yy + '.' + two_month_ago + '\\' + f'{file_name[:-1]}'
                elif mm == two_month_ago:
                    xls2_path = root_path + '20' + yy + '년' + '\\' + '20' + yy + '.' + three_month_ago + '\\' + f'{file_name[:-1]}'
                else:
...
def monthly_excel_append(cms_xlsx_path, payment_date):
    ...
    if ago_m == 0 :
        m = 12
        monthly_excel_path = 'C:\\RPA\\★월별출금내역\\' + payment_date_ago.strftime('%Y') + '년\\' + payment_date_ago .strftime('%Y') + '년 ' + str(m) + '월 출금내역.xlsx'
    else:
        monthly_excel_path = 'C:\\RPA\\★월별출금내역\\' + payment_date.strftime('%Y') + '년\\' + payment_date.strftime('%Y') + '년 ' + str(ago_m) + '월 출금내역.xlsx'
...
def over_hundred(fail_xlsx_list, cms_xlsx_path, jtt_xlsx_list, payment_date):
    ...
    if (len(payment_date_list) == 1) and payment_date_list[0] == "결제성공":
        ...
        for i in range(1, ws_cms.i + 1):
            ...
            if status == "결제성공":
                ...
                for fail_xlsx in fail_xlsx_list:
                    if payment_date.strftime('%m') + '99' in fail_xlsx:
                        ...
                        # 중복을 피함
                        if (ws_cms["D" + str(i + 1)].value not in member_name_fail) or (
                                ws_cms["O" + str(i + 1)].value not in supply_value_int):
                            print("실패파일에 내용 추가")
                            row_mx = get_i(ws_fail)
                            cd3 = payment_date.strftime('%Y%m%d')
                            ws_fail["B" + str(row_mx)].value = cd3
            ...
            for data in data_to_append_jtt:
                df = pd.read_excel(jtt_xlsx, header=5)
                create_date_jtt = list(
                    df.dropna(axis=0, subset=['작성일자\n(8자리,\nYYYYMMDD 형식)'])['작성일자\n(8자리,\nYYYYMMDD 형식)'].unique())
                member_name_jtt = list(df.dropna(axis=0, subset=['공급받는자 상호'])["공급받는자 상호"].unique())
                supply_value_jtt = list(df.dropna(axis=0, subset=['공급가액'])["공급가액"].unique())
                supply_int = list(map(int, supply_value_jtt))
...
if __name__ == "__main__":
    ...
    print(start_time)
    main(r'C:\work\JTCMM\test\cms\20230701_20230701_어린이집.xls')
