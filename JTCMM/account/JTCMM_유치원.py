...
folder_path = 'C:\\work\\JTCMM\\test\유치원\\'+ today.strftime('%Y') + '년\\' + amonth_ago.strftime('%#m월')
...
backup_path = 'C:\\work\\JTCMM\\test\\백업'
...
def xlsx2xls(fail_xlsx_list, cms_xlsx_path):
    ...
    for fail_xlsx in fail_xlsx_list:
        if payment_date_agom+'월 서비스이용료' in fail_xlsx:
            wb_xls = openpyxl.load_workbook(fail_xlsx)
            ws_xlsx = wb_xls.active
            xls_path = root_path+today.strftime('%Y')+'년\\'+amonth_ago.strftime('%#m월')
            ...
            wb_xls.close()
            os.remove(fail_xlsx)
        elif payment_date_tagom+'월 서비스이용료' in fail_xlsx:
            wb_xls = openpyxl.load_workbook(fail_xlsx)
            ws_xlsx = wb_xls.active
            xls_path = root_path + amonth_ago.strftime('%Y') + '년\\' + two_month_ago.strftime('%#m월')
            file_name = fail_xlsx[:-1]
            r_filename = os.path.basename(file_name)
            dest = os.path.join(xls_path, r_filename)
            wb_xls.save(dest)
            wb_xls.close()
            os.remove(fail_xlsx)
        elif payment_date_fagom + '월 서비스이용료' in fail_xlsx:
            wb_xls = openpyxl.load_workbook(fail_xlsx)
            ws_xlsx = wb_xls.active
            xls_path = root_path + two_month_ago.strftime('%Y') + '년\\' + three_month_ago.strftime('%#m월')
            file_name = fail_xlsx[:-1]
            r_filename = os.path.basename(file_name)
            dest = os.path.join(xls_path, r_filename)
            wb_xls.save(dest)
    ...
    for i in range(2, ws_cms.i + 1):
        ...
        if s_con !='결제실패' and s_con !='결제완료':
            ...
            if os.path.exists(ex_payment_xls_path_a):
                ...
                if ws_cms.i >= 100:
                    payment_xlsx_path = r'C:\work\JTCMM\test\형식\template 100.xlsx'
                else:
                    payment_xlsx_path = r'C:\work\JTCMM\test\형식\template.xlsx'
            ...
            payment_date_ago = payment_date - relativedelta(months=1)
            payment_month = payment_date_ago.strftime('%m')
            jtt_xlsx_path_a = root_path + f'{payment_date.year}년\\{payment_date.month}월\\xlsx\\' +payment_date.strftime('%m')+payment_date.strftime('%d')+'. '+ payment_date_b+' 유치원계산서 '+'(★'+str(payment_date.month)+'월 서비스이용료).xlsx'
            # 일반 xls파일
...
def cmsfile_move(cms_xls):
    ...
    cmsfile_path ='C:\\Users\\vivans\\Desktop\\stu_test\\'
...
if __name__ == "__main__":
    ...
    print(start_time)
    main(r'C:\Users\vivans\Desktop\stu_test\20230710_20230710_유치원.xls')
