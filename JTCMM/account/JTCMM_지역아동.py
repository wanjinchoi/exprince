...
root_path = 'C:\\RPA\지역아동센터\\'
folder_path = 'C:\\RPA\지역아동센터\\' + today.strftime('%Y') + '년\\' + str(today.month) + '월'
folder_xlsx_path = folder_path + '\\xlsx'
today_xls_path = folder_path + '\\' + today.strftime('%m%d') + today.strftime('. %y%m%d') + ' 지역아동센터계산서 (' + str(today.month) + '월 서비스이용료).xls'
fail_xls = folder_path + '\\' + today.strftime('%m') + '99. ' + today.strftime('%y%m') + '99 지역아동센터계산서 (' + str(today.month) + '월 서비스이용료) - 출금실패.xls'
today_fail_xlsx = folder_path + '\\' + today.strftime('%m') + '99. ' + today.strftime('%y%m') + '99 지역아동센터계산서 (' + str(today.month) + '월 서비스이용료)- 출금실패.xlsx'
...
def init_xlsx_folder(cms_xls):
    ...
    for i in payment_date_list:
        ...
        jtt_xslx_path = root_path+ f'{payment_date.year}년\\{payment_date.month}월\\xlsx\\'+ payment_date.strftime('%m') + payment_date.strftime('%d') + '. ' + payment_date_b + ' 지역아동센터계산서 ' + '(' + str(payment_date.month) + '월 서비스이용료).xlsx'
        # 일반 xls파일
        ...
        # 100건 초과 xls파일
        payment_xls_path_b = root_path + f'{payment_date.year}년\\{payment_date.month}월\\{payment_date_a}. {payment_date_b} 100건이상 지역아동센터계산서 ({payment_date.month}월 서비스이용료).xls'
...
def make_fail_xlsx(cms_xls):
    ...
    amonth_ago_fail_xls = root_path + payment_date_ago.strftime('%Y') + '년\\' + payment_date_ago.strftime('%#m') + '월\\' + payment_date_ago.strftime('%m') + '99. ' + payment_date_ago.strftime('%y%m') + '99 지역아동센터계산서 (' + payment_date_ago.strftime('%m') + '월 서비스이용료) - 출금실패.xls'
    two_month_ago_fail_xls = root_path + payment_date_twoago.strftime('%Y') + '년\\' + payment_date_twoago.strftime('%#m') + '월\\' + payment_date_twoago.strftime('%m') + '99. ' + payment_date_twoago.strftime('%y%m') + '99 지역아동센터계산서 (' + payment_date_twoago.strftime('%m') + '월 서비스이용료) - 출금실패.xls'
            ...
            if number == "합계":
                ...
                for fail_xlsx in fail_xlsx_list:
                    if payment_date.strftime('%m') + '99' in fail_xlsx:
                        ...
                        # 중복을 피함
                        if (ws_cms["D" + str(i + 1)].value not in member_name_fail) and (
                                ws_cms["O" + str(i + 1)].value not in supply_fail_int):
                            print("실패파일에 내용 추가")
                            row_mx = get_i(ws_fail)
                            cd3 = payment_date.strftime('%Y%m%d')
                            ws_fail["B" + str(row_mx)].value = cd3
