...
folder_path = 'C:\\RPA\\어린이집\\' + amonth_ago.strftime('%Y') + '년\\' + amonth_ago.strftime(
    '%Y') + '.' + amonth_ago.strftime('%m')
folder_xlsx_path = folder_path + '\\xlsx'
today_xls_path = folder_path + '\\' + today.strftime('%m%d') + today.strftime(
    '. %y%m%d') + ' 청구지역 CMS전자세금계산서발행★ (' + str(amonth_ago.month) + '월분이용료).xls'
fail_xls = folder_path + '\\' + today.strftime('%m') + '99. ' + today.strftime(
    '%y%m') + '99 100건이상 청구지역 CMS전자세금계산서발행★ (' + str(amonth_ago.month) + '월분이용료) - 출금실패.xls'
today_fail_xlsx = folder_path + '\\' + today.strftime('%m') + '99. ' + today.strftime(
    '%y%m') + '99 100건이상 청구지역 CMS전자세금계산서발행★ (' + str(today.month) + '월분이용료) - 출금실패.xlsx'
...
def init_xlsx_folder(cms_xls):
    ...
    for i in payment_date_list:
        ...
        jtt_xlsx_path_a = folder_xlsx_path + '\\' + f'{payment_date_a}. {payment_date_b} 청구지역 CMS전자세금계산서발행★ ({payment_date_ago.month}월분이용료).xlsx'
        jtt_xlsx_path_b = folder_xlsx_path + '\\' + f'{payment_date_a}. {payment_date_b} 100건이상 청구지역 CMS전자세금계산서발행★ ({payment_date_ago.month}월분이용료).xlsx'
        # 일반 xls파일
        payment_xls_path_a = root_path + f'{amonth_ago.year}년\\{amonth_ago.year}.{payment_month}\\{payment_date_a}. {payment_date_b} 청구지역 CMS전자세금계산서발행★ ({payment_date_ago.month}월분이용료).xls'
        # 100건 초과 xls파일
        payment_xls_path_b = root_path + f'{amonth_ago.year}년\\{amonth_ago.year}.{payment_month}\\{payment_date_a}. {payment_date_b} 100건이상 청구지역 CMS전자세금계산서발행★ ({payment_date_ago.month}월분이용료).xls'
...
def make_fail_xlsx():
    # 당월, 전월, 전전월 실패 파일 찾기
    amonth_ago_fail_xls = root_path + amonth_ago.strftime('%Y') + '년\\' + amonth_ago.strftime(
        '%Y') + '.' + two_month_ago.strftime('%m') + '\\' + amonth_ago.strftime('%m') + '99. ' + amonth_ago.strftime(
        '%y%m') + '99 100건이상 청구지역 CMS전자세금계산서발행★ (' + two_month_ago.strftime('%m') + '월분이용료) - 출금실패.xls'
    two_month_ago_fail_xls = root_path + two_month_ago.strftime('%Y') + '년\\' + two_month_ago.strftime(
        '%Y') + '.' + three_month_ago.strftime('%m') + '\\' + two_month_ago.strftime(
        '%m') + '99. ' + two_month_ago.strftime('%y%m') + '99 100건이상 청구지역 CMS전자세금계산서발행★ (' + three_month_ago.strftime(
        '%#m') + '월분이용료) - 출금실패.xls'
