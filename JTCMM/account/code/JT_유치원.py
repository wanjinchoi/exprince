...
root_path ='C:\\RPA\\유치원\\'
folder_path = 'C:\\RPA\\유치원\\'+ today.strftime('%Y') + '년\\' + str(today.month) + '월'
...
def init_xlsx_folder(cms_xls):
    ...
    for i in payment_date_list:
        ...
        jtt_xslx_path = folder_xlsx_path+'\\'+payment_date.strftime('%m')+payment_date.strftime('%d')+'. '+ payment_date_b+' 유치원계산서 '+'(★'+str(payment_date.month)+'월 서비스이용료).xlsx'
        #일반 xls파일
...
def make_fail_xlsx():
    # 당월, 전월, 전전월 실패 파일 찾기
    amonth_ago_fail_xls = root_path + amonth_ago.strftime('%Y') + '년\\' + amonth_ago.strftime('%m') + '월\\' + amonth_ago.strftime('%m') + '99. ' + amonth_ago.strftime('%y%m') + '99 유치원계산서 (★' + amonth_ago.strftime('%m') + '월 서비스이용료)- 출금실패.xls'
    two_month_ago_fail_xls = root_path + two_month_ago.strftime('%Y') + '년\\' + two_month_ago.strftime('%m') + '월\\' + two_month_ago.strftime('%m') + '99. ' + two_month_ago.strftime('%y%m') + '99 유치원계산서 (★' + two_month_ago.strftime('%m') + '월 서비스이용료)- 출금실패.xls'
...
if __name__ == "__main__":
    ...
    print(start_time)
    main(r'C:\Users\vivans\Desktop\stu_test\2023\20230102_20230102_유치원.xls')
