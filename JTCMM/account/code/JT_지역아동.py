...
today_fail_xlsx = folder_path + '\\' + today.strftime('%m') + '99. ' + today.strftime('%y%m') + '99 지역아동센터계산서 (' + str(
    today.month) + '월 서비스이용료)- 출금실패.xlsx'
...
def init_xlsx_folder(cms_xls):
    ...
    for i in payment_date_list:
        ...
        jtt_xslx_path = folder_xlsx_path + '\\' + payment_date.strftime('%m') + payment_date.strftime('%d') + '. ' + payment_date_b + ' 지역아동센터계산서 ' + '(' + str(payment_date.month) + '월 서비스이용료).xlsx'
        # 일반 xls파일
...
def make_fail_xlsx():
    ...
    amonth_ago_fail_xls = root_path + amonth_ago.strftime('%Y') + '년\\' + amonth_ago.strftime(
        '%m') + '월\\' + amonth_ago.strftime('%m') + '99. ' + amonth_ago.strftime(
    ...
    two_month_ago_fail_xls = root_path + two_month_ago.strftime('%Y') + '년\\' + two_month_ago.strftime(
        '%m') + '월\\' + two_month_ago.strftime('%m') + '99. ' + two_month_ago.strftime(
        '%y%m') + '99 지역아동센터계산서 (' + two_month_ago.strftime('%m') + '월 서비스이용료) - 출금실패.xls'
...
def monthly_excel_append(cms_xlsx_path, payment_date):
    ...
    # success_list = []
    # # 반복하며 데이터 리스트에 넣기
    # for i in range(2, ws_monthly.i + 1):
    #     tmp = []
    #     if ws_monthly['D' + str(i)].value is not None:
    #         for j in range(2, ws_cms.i):
    #             if ws_cms["D" + str(j)].value == ws_monthly['D' + str(i)].value:
    #                 if ws_cms["O" + str(j)].value == ws_monthly['O' + str(i)].value:
    #                     if ws_cms["B" + str(j)].value == ws_monthly["B" + str(i)].value:
    #                         print('중복임')
    #                         break
    #                     for k in range(1, 21):
    #                         tmp.append(ws_cms.cell(j, k).value)
...
if __name__ == "__main__":
    ...
    print(start_time)
    main(r'C:\Users\vivans\Desktop\stu_test\2023\20230106_20230106_지역아동센터.xls')
