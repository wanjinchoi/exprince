...
root_path ='C:\\work\\JTCMM\\test\\지역아동센터\\'
folder_path = 'C:\\work\\JTCMM\\test\\지역아동센터\\' + today.strftime('%Y') + '년\\' + today.strftime('%m') + '월'
...
today_fail_xlsx = folder_path + today.strftime('%m') + '99. ' + today.strftime('%y%m') + '99 지역아동센터계산서 (' + str(today.month) + '월 서비스이용료) - 출금실패.xls'
...
def get_i(ws_fail):
    ...
    for i in range(7, ws_fail.i+1):
        if(ws_fail["M"+str(i+1)].value is None) and (ws_fail["M"+str(i+2)].value is None) and (ws_fail["M"+str(i+3)].value is None):
            max_r = i+1
            break
    return max_r
...
def xlsx2xls(xlsx_path):
    for z in range(len(xlsx_path)):
        ...
        wb = xl.Workbooks.Add(xlsx_path[z])
        if os.path.exists(xls2_path):
            if not os.path.exists(f'./test/백업/{today.month}{today.day}/'):
                os.makedirs(f'./test/백업/{today.month}{today.day}/')
            if os.path.exists(f'./test/백업/{today.month}{today.day}/{file_name[:-1]}'):
                os.remove(f'./test/백업/{today.month}{today.day}/{file_name[:-1]}')
            shutil.move(xls2_path, f'./test/백업/{today.month}{today.day}/')

        wb.SaveAs(xls2_path, FileFormat=56)
        xl.Quit()
...
def make_fail_xlsx():
    # 당월, 전월, 전전월 실패 파일 찾기
    amonth_ago_fail_xls = 'C:\\work\\JTCMM\\test\\지역아동센터\\' + amonth_ago.strftime('%Y') + '년\\' + amonth_ago.strftime('%m') + '월\\' + amonth_ago.strftime('%m') + '99. ' + amonth_ago.strftime('%y%m') + '99 지역아동센터계산서 (' + amonth_ago.strftime('%m') + '월 서비스이용료) - 출금실패.xls'
    two_month_ago_fail_xls = 'C:\\work\\JTCMM\\test\\지역아동센터\\' + two_month_ago.strftime('%Y') + '년\\' + two_month_ago.strftime('%m') + '월\\' + two_month_ago.strftime('%m') + '99. ' + two_month_ago.strftime('%y%m') + '99 지역아동센터계산서 (' + two_month_ago.strftime('%m') + '월 서비스이용료) - 출금실패.xls'
...
def monthly_excel_append(cms_xlsx_path):
    monthly_excel_path = 'C:\\work\\JTCMM\\test\\★월별출금내역\\' + amonth_ago.strftime('%Y') + '년\\' + amonth_ago.strftime('%Y년 ') + str(amonth_ago.month) + '월 출금내역.xlsx'
...
def cms_check(fail_xlsx_list, cms_xlsx_path, jtt_xlsx_list, payment_date):
    ...
    if '100건초과' in jtt_xlsx:
        ...
        for i in range(1, ws_cms.i+1):
            ...
            payment_date = ws_cms['B' + str(i+1)].value
                ...
                for fail_xlsx in fail_xlsx_list:
                    if str_m in fail_xlsx:
                        ...
                        if member_name in store_name_list_fail:
                            ...
                            cd2 = datetime.strptime(payment_date, '%Y/%m/%d')
                            cd3 = datetime.strftime(cd2, '%Y-%m-%d')
        ...
        for i in range(1, ws_cms.i + 1):
            ...
            if payment_date == "합계":
                ...
                for fail_xlsx in fail_xlsx_list:
                    ...
                    for j in range(6, ws_fail.i + 1):
                        ...
                        if (member_name == member_store_fail) and (supply_value == supply_value_fail):
                            ...
                            retry_success_list = [
                                '01', today.strftime('%Y%m%d'), registration_number,
                                '',
                                member_store_fail, member_name_fail, member_address_fail,
                                '', '',
                                member_email_fail, supply_value_fail, tax_fail, remark_fail, today.strftime('%d'),
                                product_fail,
                                '', '', '',
                                supply_value_fail, tax_fail
                            ]
                ...
                if "재결제" in status:
                    ...
                    # 서비스 이용료 엑셀에서 정보 가져오기
                    wb_fail = openpyxl.load_workbook(today_fail_xlsx)
                    ws_fail = wb_fail.active
                    for j in range(6, ws_jtt.i + 1):
                        member_store_jtt = ws_jtt['M' + str(j)].value
                        supply_value_jtt = ws_jtt['T' + str(j)].value
                    ...
                    wb_fail.save(today_fail_xlsx)

        # 결제일 엑셀에서, 결제실패 데이터 지우기
        cnt = 0
        for row in row_to_delete_jtt:
            ws_jtt.delete_rows(row - cnt)
            cnt += 1
...
def main(cms_xls):
    ...
    xlsx2xls(xlsx_to_convert)


if __name__ == "__main__":
    start_time = datetime.now()
    print(start_time)
    main(r'C:\work\JTCMM\test\cms\20221125_20221125_지역아동센터.xls')
