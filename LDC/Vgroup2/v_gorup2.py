...
excel_path='C:\\ArgosRPA\\vgroup\\excel2\\xlsx_file\\'
result_path='C:\\ArgosRPA\\vgroup\\result2\\v_result2.xlsx'
except_path ='C:\\ArgosRPA\\vgroup\\form2\\except.xlsx'
...
def main(subject,vcom,excelpath):
    ...
    if (ws_clib['A1'].value is None)and(ws_drp['A1'].value is None):
        for i in range(1, ws_pa.max_row + 1):
            ...
            # itemno입력
            ws_fm["A" + str(d_max_row + 1)].value = ws_it["A" + str(i)].value
            wb_fm.save(result_path)
            # partno입력
            if 'nothing'in str(ws_pno["A" + str(i)].value):
                ...
                ws_fm["C" + str(d_max_row + 1)].value = ws_pno["A" + str(i)].value
            # 수량입력
            ws_fm["E" + str(d_max_row + 1)].value = ws_qty['A' + str(i)].value
            # 단위입력
            ws_fm['F' + str(d_max_row + 1)].value = ws_unit['A' + str(i)].value
            wb_fm.save(result_path)
            ...
            if b is not None:
                ...
                av = ax2.replace('\r', '')
                axc = av.replace('\t', '')
                ws_fm['D' + str(d_max_row + 1)].value = axc
                wb_fm.save(result_path)
                d_max_row = max((a.row for a in ws_fm['D'] if a.value is not None))
            ...
            if b is not None:
                ...
                if ('nothing'in str(ws_clib["A"+str(i)].value))or(str(ws_clib["A"+str(i)].value) is None):
                    if (ws_drp['A' + str(i)].value != 'nothing') and (ws_drp['A' + str(i)].value is not None) and (ws_drp['A' + str(i)].value != ',nothing'):
                        ...
                        ws_fm["D" + str(d_max_row + 1)].value = ws_drp['A' + str(i)].value
                        wb_fm.save(result_path)
                elif str(ws_clib['A'+str(i)].value):
                        pass
                else:
                    clib = ws_clib["A" + str(i)].value
                    if 'C:\\ARGOSRPA\\VGROUP\\EXCEL\\LDCCLIB.CSV' in clib:
                    ...
                    if len(clib) > 250:
                        ...
                        for rclib in z:
                            d_max_row = max((a.row for a in ws_fm['D'] if a.value is not None))
                            ws_fm['B' + str(d_max_row + 1)].value = '='
                            ws_fm['D' + str(d_max_row + 1)].value = rclib
                            wb_fm.save(result_path)
                    else:
                        ws_fm['B' + str(d_max_row + 1)].value = '='
                        ws_fm['D' + str(d_max_row + 1)].value = ws_clib["A" + str(i)].value
                        wb_fm.save(result_path)
                    if ('nothing' in str(ws_drp['A' + str(i)].value)) or (str(ws_drp['A' + str(i)].value) is None) :
                        pass
                    else:
                        if ws_fm["D" + str(d_max_row+1)].value is None:
                            ws_fm["D" + str(d_max_row + 1)].value = str(ws_drp['A' + str(i)].value)
                            wb_fm.save(result_path)
                        else:
                            ws_fm["D" + str(d_max_row+1)].value =ws_fm["D" + str(d_max_row+1)].value+', '+str(ws_drp['A' + str(i)].value)
                            clib2 = ws_fm["D" + str(d_max_row+1)].value
                            ...
                            if len(clib2) > 250:
                                ...
                                d_max_row = max((a.row for a in ws_fm['D'] if a.value is not None))
                                for rclib2 in z:
                                    ws_fm['D'+str(d_max_row)].value =rclib2
                                    d_max_row +=1
                                    wb_fm.save(result_path)
                            wb_fm.save(result_path)
            ...
            if b is not None:
                ...
                if ('nothing'in str(ws_clib["A"+str(i)].value))or(str(ws_clib["A"+str(i)].value) is None):
                    if (ws_drp['A' + str(i)].value != 'nothing') and (ws_drp['A' + str(i)].value is not None) and (ws_drp['A' + str(i)].value != ',nothing'):
                        ...
                        ws_fm["D" + str(d_max_row + 1)].value = ws_drp['A' + str(i)].value
                        wb_fm.save(result_path)
                elif str(ws_clib['A'+str(i)].value) is None:
                        pass
                else:
                    clib = ws_clib["A" + str(i)].value
                    if clib is None:
                        ...
                        if len(clib) > 250:
                            ...
                            ws_fm['B' + str(d_max_row + 1)].value = '='
                            clib = clib.replace('\\','')
                            ws_fm['D' + str(d_max_row + 1)].value = clib
                            wb_fm.save(result_path)
                        if ('nothing' in str(ws_drp['A' + str(i)].value)) or (str(ws_drp['A' + str(i)].value) is None) :
                            pass
                        else:
...
if __name__ == "__main__":
    main('LIFE BOAT DAVIT HOISTING WIRES','Description: Lifeboat Davit No.1 Maker: Oriental Precision & Engineering Co., Ltd. Serial: 12105S/12106P Component Notes:  LIFE BOAT DAVIT (GRAVITY TYPE) NO.1/2 MAKER ORIENTAL Precision & Engineering TYPE / MODEL HGD-062-26 SERIAL NO 12105S/12106P OTHER DETAILS QTY 2 L/Boat Davit : Hinged Gravity Type    ',r'C:\work\LDC\excel')
