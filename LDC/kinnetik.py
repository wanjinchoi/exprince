...
# except_path='C:\\work\\LDC\\kName.xlsx'


form_path='C:\\ArgosRPA\\form\\form.xlsx'
excel_path='C:\\ArgosRPA\\kinnetik\\excel\\xlsx_file\\'
result_path='C:\\ArgosRPA\\kinnetik\\form\\k_result.xlsx'
        ...
        if xfile == 'kpartname2.xlsx':
            ...
            for i in range(1, ws.i+1):
                if ws['A'+str(i)].value is None:
                    ...
                    if str(ws_fm['D' + str(j)].value)  == 'None':
                        ws_fm["D"+str(j)].value = str(ws['A'+str(i)].value)
                        a = ws_fm["D"+str(j)].value
                        b = a.replace("혻",'')
                        ws_fm["D" + str(j)].value = b
                    else:
                        ws_fm["D"+str(j)].value = str(ws_fm['D'+str(j)].value) +', '+ str(ws['A'+str(i)].value)
                        a = ws_fm["D"+str(j)].value
                        b = a.replace("혻",'')
                        ws_fm["D" + str(j)].value = b
                j+=1
        ...
        try:
                ...
                if file =='kpartname.csv':
                    try:
                        ...
                        df = pd.read_csv(excelpath + '\\' + file, encoding='utf-8', sep='\t')
