...
class Output(PySelenium):
    ...
    def __init__(self, file_path, data_time):
        ...
        # PDF 저장 위치
        self.PDF_path = rf'C:\work\NST\1.Computer\output\kyoung\{self.start_ts}'
        # VBA 모듈 위치
        self.vba_path = r'C:\work\NST\python\print_sunil.bas'
        # 출력 데이터 저장 파일
        self.save_path = r'C:\work\NST\1.Computer\save_data\Sunil_save_data.xlsx'
        #요청사항 폴더 삭제
        self.result_path = r'C:\work\NST\경민'
        self.move_path = r'C:\work\NST\경민\endfile'
        ...
        file_list = sorted(glob.glob('C:\\work\\NST\\경민\\' + '*.xlsx'),key=os.path.getmtime)
        self.data_file = file_list[0]
        log_path = r'C:\work\NST\1.Computer\log\output\kyoung'
    ...
    def csv_read(self):
        ...
        #data_list = pd.read_excel(self.data_file, header=12, engine='xlrd')
        data_list = pd.read_excel(self.data_file, engine='openpyxl', header=12)
        df = pd.DataFrame(data_list)

        # 'Unnamed'로 시작하는 열들을 모두 제거
        df = df.loc[:, ~df.columns.str.contains('^Unnamed')]

        # NO 열이 숫자인 행만 필터링
        df_filtered = df[pd.to_numeric(df['NO.'], errors='coerce').notnull()]

        # 필요한 열만 선택
        selected_columns = df_filtered[['발주일자', '로트번호', '품목코드', '규격', '중량','도금사양']]
...
# ==============================================
def folder_remove():
    path = r"C:\work\NST\1.Computer\output\kyoung"
