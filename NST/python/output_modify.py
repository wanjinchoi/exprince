...
class Output(PySelenium):
    ...
    def append_to_excel(self, filtered_data,data,cutoff_time):
        ...
        for (index1, row1), (index2, row2) in zip(df.iterrows(),df2.iterrows()):
            ...
            if pum_number == previous_pum_number:
                ...
                ws['F' + str(start_row)].value = row1['표면처리사양']
    ...
    def csv_read(self):
        ...
        # 가져올 필드들의 열 이름 리스트
        fields_to_extract = ['선일 출고 일자', '선일 출고 시간', '전 Lot 정보 - 품번','표면처리사양']
        all_data = []
        ...
        if '업체 로트 번호' in df_result.columns:
            df_result['NST LOT No.'] = (df_result.groupby('전 Lot 정보 - 품번')['업체 로트 번호'].transform(lambda x: (x.str.split().str[0] != x.str.split().str[0].shift()).cumsum()))
            grouped_data = df_result[['전 Lot 정보 - 품번', '업체 로트 번호', '선일 출고 시간', 'NST LOT No.']]
        else:
            # '업체 로트 번호'가 없을 경우 처리
            grouped_data = df_result[['전 Lot 정보 - 품번', '선일 출고 시간']]
