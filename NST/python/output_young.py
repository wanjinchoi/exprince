...
class Output(PySelenium):
    ...
    def __init__(self, file_path, data_time):
        ...
        # PDF 저장 위치
        self.PDF_path = rf'C:\work\NST\1.Computer\output\young\{self.start_ts}'
        # VBA 모듈 위치
        self.vba_path = r'C:\work\NST\python\print_sunil.bas'
        # 출력 데이터 저장 파일
        self.save_path = r'C:\work\NST\1.Computer\save_data\Sunil_save_data.xlsx'
        #요청사항 폴더 삭제
        self.result_path = r'C:\work\NST\영신'
        self.move_path = r'C:\work\NST\영신\endfile'
        ...
        file_list = sorted(glob.glob('C:\\work\\NST\\영신\\' + '*.xlsx'),key=os.path.getmtime)
        if len(file_list)==0:
            print('None')
        else:
    ...
    def append_to_excel(self, data):
        ...
        for index, row in df.iterrows():
            ...
            if previous_index is None:
                ...
                ws['B'+str(start_row)].value = '영신'
                ws['C'+str(start_row)].value = row['품명']
                ws['D'+str(start_row)].value = row['품번']
                ws['E'+str(start_row)].value = row['규격']
                ws['F'+str(start_row)].value = row['도금']
                ws['H'+str(start_row)].value = '1'
                ws['I' + str(start_row)].value = row['com_lot']
            else:
                code= row['품번']
                prievious_code = data.at[previous_index, '품번']
    ...
    def csv_read(self):
        ...
        for index, row in new_df.iterrows():
            ...
            if index == 0:
                ...
                #이전품목코드
                previous_product_code =new_df.at[index - 1, '품번']
                now_porduct_code = row['품번']
                #품목코드가 같으면 이전 파일 오픈
