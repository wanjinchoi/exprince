...
def main(index):
    ...
    original_file_path = 'X:\\〔1〕   완 료 신 청 = A\\'
    # 폴더이름 가져오기
    folders_check = [f for f in os.listdir(original_file_path) if
    ...
    # 엑셀 파일이름 가져오기
    flist = sorted(glob.glob(excel_folder_path + '*.xlsx'),
    ...
    # 폴더이름 가져오기
    folders = [f for f in os.listdir(original_file_path2) if
               os.path.isdir(os.path.join(original_file_path2, f))]
    folder_name = folders[0]
    # 폴더경로
    folder_path = original_file_path2 + folder_name + '\\'
    folders2 = [f for f in os.listdir(folder_path) if
                os.path.isdir(os.path.join(folder_path, f))]
    folder2_name = folders2[0]
    folder3_path = folder_path + folder2_name + '\\'
    folder4_list = [f for f in os.listdir(folder3_path) if
                    os.path.isdir(os.path.join(folder3_path, f))]
    ...
    skm_files = [file for file in jpg_files if "SKM" not in file]

    print(len(non_skm_files))
