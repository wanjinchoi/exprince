...
def insert_images(ws, image_paths, column, start_row):
    ...
    for path in image_paths:
        ...
        folder_date = os.path.basename(os.path.dirname(os.path.dirname(path)))
...
def main(yaml_path):
    ...
    if report_sendtime_datetime <= now < five_minutes_after_report_sendtime:
        ...
        bf_image_paths_yesterday = [os.path.join(config['send_folder_path'], yesterday_date_str, bf_image) for bf_image in data['bf_image_names']]
        bf_image_paths_today = [os.path.join(config['send_folder_path'], current_date_str, bf_image) for bf_image in data['bf_image_names']]
        af_image_paths_yesterday = [os.path.join(config['send_folder_path'], yesterday_date_str, af_image) for af_image in data['af_image_names']]
        af_image_paths_today = [os.path.join(config['send_folder_path'], current_date_str, af_image) for af_image in data['af_image_names']]

        bf_image_paths = bf_image_paths_yesterday + bf_image_paths_today
        af_image_paths = af_image_paths_yesterday + af_image_paths_today
