...
def main(report_sendtime):
    ...
    with open('road.yaml','r',encoding='utf-8') as file:
    ...
    SELECT date 
    FROM files 
    WHERE datetime(date) >= '{current_date_str} {report_time_start}' 
    AND datetime(date) <= '{tomorrow_date_str} {report_time_end}';
    """
...
####################################엑셀 편집#############################################
    ...
    report_time_end = datetime.strptime(data['report_time_end'], "%H:%M").time()
