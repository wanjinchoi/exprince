...
def upload_image_to_zalo(image_path, access_token,company_name):
    ...
    if upload_response.status_code == 200:
        ...
        if attachment_id:
            ...
            post_message_fail(company_name, '파일 업로드 API호출',access_token,'8cc798b90ce0e5bebcf1', 'A', image_path,error_message=upload_response.json())
...
def send_message_with_attachment(attachment_id, message_text, access_token,file_path,mqtt_client_handler):
    ...
    if message_response.status_code == 200:
        ...
        if message_response.json().get('error') != 0:
            post_message_fail(company_name, message_text, access_token,'8cc798b90ce0e5bebcf1', 'A', file_path,error_message=message_response.json())
        else:
            post_message_success(company_name, message_text, access_token,'8cc798b90ce0e5bebcf1', 'A', file_path)
