...
def image_move():
    ...
    # 옮긴 파일 내에서 최신파일 가져오기
    flist = sorted(glob.glob(detect_folder + '*.jpg', recursive=True) + glob.glob(detect_folder + '*.JPEG', recursive=True), key=os.path.getmtime)
...
def upload_image_to_zalo(image_path, access_token,company_name):
    ...
    if upload_response.status_code == 200:
        ...
        if attachment_id:
            ...
            post_message_fail(company_name, '파일 업로드 API호출',access_token,'ad87bab930e1d9bf80f0', 'A', image_path,error_message=upload_response.json())
...
def send_message_with_attachment(attachment_id, message_text, access_token,file_path,mqtt_client_handler):
    ...
    data = {
        "recipient": {
            ...
            #True_milk : ad87bab930e1d9bf80f0
            "group_id": "ad87bab930e1d9bf80f0"
        },
        "message": {
            "text": message_text,
            "attachment": {
    ...
    if message_response.status_code == 200:
        ...
        post_message_success(company_name, message_text, access_token,'ad87bab930e1d9bf80f0', 'A',file_path)
        if message_response.json().get('error') != 0:
            post_message_fail(company_name, message_text, access_token,'ad87bab930e1d9bf80f0', 'A', file_path,error_message=message_response.json())
        ...
        mqtt_client_handler.publish_message(
            ...
            role=device_role,
            value=-1,
            extensionValue=f"/send-message: {response_content}"
        )

        post_message_fail(company_name,response_content , access_token,'ad87bab930e1d9bf80f0', 'A',file_path,error_message=message_response.json() )
...
def send_all_images(folder_path,full_screen_shot_path,cursor, access_token,company_name,device_role):
    ...
    if len(os.listdir(folder_path)) == 0 and len(os.listdir(full_screen_shot_path)) == 0:
        return None
    # 해당폴더내에 이미지 폴더가 있는지 없는지 검토
    elif len(os.listdir(folder_path)) != 0:
        # 이미지 파일 가져오기
        for filename in os.listdir(folder_path):
            #원래는 af만 이었는데 가끔 오류로 bf도 와서 추가함
            if filename.endswith(".png"):
                #파일경로와 이름 가져오기
                file_path = os.path.join(folder_path, filename)
                output_image_path = compress_image(file_path, target_size_mb=1)
                # zalo는 파일 전송을 위해서는 api를 호출해서 이미지 업로드 후에 이미지 특정 아이디를 가져와 보내기가 가능합니다. 그래서 아이디를 구하는 api를 호출하는 함수입니다.
                attachment_id = upload_image_to_zalo(output_image_path, access_token,company_name)
                # 업로드한 이미지파일의 아이디가 있으면
                if attachment_id:
                    # 데이터베이스에서 파일 정보 가져오기
                    cam, cam_name, register_ts, detection_type = get_file_info(cursor, filename)
                    if all([cam, cam_name, register_ts, detection_type]):
                        # DB에서 정보를 가져와서 이미지와 함께 텍스트를 같이 보냅니다. 아래는 텍스트 문구
                        ...
                        # 앞선 내용들을 바탕으로한 이미지 전송 함수
                        send_message_with_attachment(attachment_id, message_text, access_token,file_path =output_image_path,mqtt_client_handler=mqtt_client_handler)
    #full_screen폴더에 파일이 있는지 없는지 검토합니다.
    elif len(os.listdir(full_screen_shot_path)) != 0:
        #full_screnn내에 있는 파일들을 하나씩 가져옵니다. 이후 과정은 위와  동일합니다.
