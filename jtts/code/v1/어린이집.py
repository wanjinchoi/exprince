...
# XLSX_DIRECTORY_1 = r"C:\RPA\어린이집" + "\\" + datetime.strftime(NOW_1, "%Y") + "년" + "\\" + datetime.strftime(NOW_1, "%Y.%m") + "\\xlsx"
# XLS_DIRECTORY_1 = r"C:\RPA\어린이집" + "\\" + datetime.strftime(NOW_1, "%Y") + "년" + "\\" + datetime.strftime(NOW_1, "%Y.%m")
# XLSX_DIRECTORY = r"C:\RPA\어린이집" + "\\" + datetime.strftime(NOW, "%Y") + "년" + "\\" + datetime.strftime(NOW, "%Y.%m") + "\\xlsx"
# XLS_DIRECTORY = r"C:\RPA\어린이집" + "\\" + datetime.strftime(NOW, "%Y") + "년" + "\\" + datetime.strftime(NOW, "%Y.%m")
# MONTH_DIRECTORY = r"C:\RPA\★월별출금내역\\" + str(NOW.year) + "년"

last_char_zip = r"C:\RPA\어린이집"
...
def xls2xlsx_path(full_path, runday):
    ...
    NOW = rundate - relativedelta(months=1)
    XLSX_DIRECTORY = last_char_zip + "\\" + datetime.strftime(
        NOW, "%Y") + "년" + "\\" + datetime.strftime(NOW, "%Y.%m") + "\\xlsx"
    try:
...
def move_xls_file(runday):
    ...
    XLS_DIRECTORY = last_char_zip + "\\" + datetime.strftime(
        NOW, "%Y") + "년" + "\\" + datetime.strftime(NOW, "%Y.%m")
    XLSX_DIRECTORY = last_char_zip + "\\" + datetime.strftime(
        NOW, "%Y") + "년" + "\\" + datetime.strftime(NOW, "%Y.%m") + "\\xlsx"
    listdir = os.listdir(XLSX_DIRECTORY)
    for i in listdir:
...
def paid_list(date_paid_list, runday):
    ...
    for i in range(ws.nrows):
        ...
        elif ws.cell_value(i, 9) == '결제실패(월 사용한도액 초과)' or ws.cell_value(i,
                                                                        9) == '결제실패(잔액부족)' or ws.cell_value(
            i, 9) == '결제실패(출금불가계좌)' or ws.cell_value(i, 9) == '결제실패(해지된계좌)':
...
def failure_list_return_list(name, date, price, runday):
    rundate = datetime.strptime(runday, "%Y%m%d")
    NOW = rundate - relativedelta(months=1)
    NOW_1 = rundate - relativedelta(months=2)
    XLS_DIRECTORY_1 = last_char_zip + "\\" + datetime.strftime(
        NOW_1, "%Y") + "년" + "\\" + datetime.strftime(NOW_1, "%Y.%m")
    XLS_DIRECTORY = last_char_zip + "\\" + datetime.strftime(
        NOW, "%Y") + "년" + "\\" + datetime.strftime(NOW, "%Y.%m")
    XLSX_DIRECTORY = last_char_zip + "\\" + datetime.strftime(
        NOW, "%Y") + "년" + "\\" + datetime.strftime(NOW, "%Y.%m") + "\\xlsx"
    ...
    for i in range(last_row):
        if ws['E' + str(i + 1)].value == name and ws[
            'L' + str(i + 1)].value == price:
            isExistrow = True
            ...
            break
    if not isExistrow:
        file_name = find_file_name(XLS_DIRECTORY_1, str((
                                                                rundate - relativedelta(
                                                            months=1)).month) + str(
            99))
        try:
            wb = load_workbook(XLSX_DIRECTORY + "\\" + file_name + "x")
...
def delete_xlsx_folder(runday):
    print('START delete_xlsx_folder')
    try:
        # 시간 -1달
        rundate = datetime.strptime(runday, "%Y%m%d")
        NOW = rundate - relativedelta(months=1)
        # 월의 숫자 설정
        num = []
        num.append(str(rundate - relativedelta(months=1)).split(" ")[0].split("-")[1])  # 08
        num.append(str((rundate - relativedelta(months=1)).month))  # 8
        # 올해
        ...
        for n in num:
            # 연도+월
            ym = str(year) + '.' + str(n)
            # datetime.strftime(NOW, "%Y.%m")
            # 경로
            path = last_char_zip + '\\' + year + "년" + "\\" + ym + "\\xlsx"
            # 폴더 존재 확인 후, 삭제
            if os.path.exists(path):
                shutil.rmtree(path)
                print(f'  delete xlsx folder: {path}')
        print('END delete_xlsx_folder')
    except:
        # 혹시 모를 예외 사항을 위해서
        print('ERROR but Except.. delete_xlsx_folder')
        pass
