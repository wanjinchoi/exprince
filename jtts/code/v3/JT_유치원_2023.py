...
root_path ='C:\\work\\JTCMM\\test\\유치원\\'
folder_path = 'C:\\work\\JTCMM\\test\\유치원\\' + today.strftime('%Y') + '년\\' + str(today.month) + '월'
folder_xlsx_path = folder_path + '\\xlsx'
today_xls_path = folder_path  + '\\' +today.strftime('%m%d') + today.strftime('. %y%m%d') + ' 유치원계산서 (' + str(today.month) + '월 서비스이용료).xls'
fail_xls = folder_path + '\\' + today.strftime('%m') + '99. ' + today.strftime('%y%m') + '99 유치원계산서 (★' + str(today.month) + '월 서비스이용료)- 출금실패.xls'
today_fail_xlsx = folder_path + '\\' + today.strftime('%m') + '99. ' + today.strftime('%y%m') + '99 유치원계산서 (★' + str(today.month) + '월 서비스이용료)- 출금실패.xlsx'
...
def xlsx2xls(xlsx_path,payment_date):
    for z in xlsx_path:
        if "이용료" not in z:
            pass
        else:
            ...
            if y_m == 0:
                ...
                xls2_path = root_path + '20' + yy + '년' + '\\' + str(folder_m)+'월' + '\\' + f'{file_name[:-1]}'
...
def init_xlsx_folder(cms_xls):
    ...
    for i in payment_date_list:
        ...
        jtt_xslx_path = root_path+ f'{payment_date.year}년\\{payment_date.month}월\\xlsx\\' +payment_date.strftime('%m')+payment_date.strftime('%d')+'. '+ payment_date_b+' 유치원계산서 '+'(★'+str(payment_date.month)+'월 서비스이용료).xlsx'
        #일반 xls파일
        payment_xls_path_a = root_path + f'{payment_date.year}년\\{payment_date.month}월\\{payment_date_a}. {payment_date_b} 유치원계산서 (★{payment_date.month}월 서비스이용료).xls'
        #100건 초과 xls파일
        payment_xls_path_b = root_path + f'{payment_date.year}년\\{payment_date.month}월\\{payment_date_a}. {payment_date_b} 100건이상 유치원계산서 (★{payment_date.month}월 서비스이용료).xls'
        ...
        if os.path.exists(payment_xls_path_a):
            ...
            if len(payment_date_list) >= 100 :
                wb_jtt = openpyxl.load_workbook(r'C:\work\JTCMM\test\형식\template100.xlsx')
            else:
                wb_jtt = openpyxl.load_workbook(r'C:\work\JTCMM\test\형식\template.xlsx')
...
def backup(xlsx_path):
    ...
    if not os.path.exists(f'C:/work/JTCMM/test/백업{today.month}{today.day}'):
        os.makedirs(f'C:/work/JTCMM/test/백업{today.month}{today.day}/xlsx')
        copy_tree(folder_xlsx_path, f'C:/work/JTCMM/test/백업/{today.month}{today.day}/xlsx/')
    if os.path.exists(f'C:\\work\\JTCMM\\test\\백업\\{today.month}{today.day}'):
        shutil.rmtree(f'C:\\work\\JTCMM\\test\\백업\\{today.month}{today.day}/xlsx')
        os.makedirs(f'C:\\work\\JTCMM\\test\\백업\\{today.month}{today.day}/xlsx')
        copy_tree(folder_xlsx_path, f'C:\\work\\JTCMM\\test\\백업\\{today.month}{today.day}/xlsx/')
    return xlsx_path
...
def make_fail_xlsx(cms_xls):
    ...
    for i in payment_date_list:
        payment_date = datetime.strptime(i, '%Y/%m/%d')
        payment_date_y = payment_date.strftime('%Y')
        payment_date_m = payment_date.strftime('%m')
        payment_date_d = payment_date.strftime('%d')
        payment_date_ago = payment_date - relativedelta(months=1)
        payment_date_twoago = payment_date - relativedelta(months=2)
        payment_date_three = payment_date - relativedelta(months=3)

    amonth_ago_fail_xls = root_path + payment_date_ago.strftime('%Y') + '년\\' + payment_date_ago.strftime('%#m') + '월\\' + payment_date_ago.strftime('%m') + '99. ' + payment_date_ago.strftime('%y%m') + '99 유치원계산서 (★' + payment_date_ago.strftime('%m') + '월 서비스이용료)- 출금실패.xls'
    two_month_ago_fail_xls = root_path + payment_date_twoago.strftime('%Y') + '년\\' + payment_date_twoago.strftime('%#m') + '월\\' + payment_date_twoago.strftime('%m') + '99. ' + payment_date_twoago.strftime('%y%m') + '99 유치원계산서 (★' + payment_date_twoago.strftime('%m') + '월 서비스이용료)- 출금실패.xls'
...
def monthly_excel_append(cms_xlsx_path, payment_date):
    ...
    if ago_m == 0:
        ...
        monthly_excel_path = 'C:\\work\\JTCMM\\test\\★월별출금내역\\' + today.strftime('%Y') + '년\\' + today.strftime('%Y') + '년 ' + str(ago_m) + '월 출금내역.xlsx'
    ...
    if '100건초과' in jtt_xlsx:
        ...
        for i in range(1, ws_cms.i+1):
            ...
            if number == "합계":
                ...
                print("재결제실패")
                for fail_xlsx in fail_xlsx_list:
                    if payment_date.strftime('%m')+'99' in fail_xlsx:
                ...
                for fail_xlsx in fail_xlsx_list:
                    if payment_date.strftime('%m')+'99' in fail_xlsx:
                        ...
                        create_date_fail = list(df.dropna(axis=0, subset=['작성일자'])["작성일자"].unique())
                        ...
                        supply_fail_int = list(map(int, man_fail))
                        for j in range(7, ws_jtt.i+1):
                            ...
                            if member_store_jtt == member_name and supply_value == supply_value_jtt:
                                ...
                                for data in data_to_append_fail:
                                    if (data[12] not in member_name_fail) or (data[19] not in supply_fail_int):
                                        if (data[2] not in create_date_fail):
                                            ws_fail.append(data)
                                wb_fail.save(fail_xlsx)
            ...
            for data in data_to_append_jtt:
                ...
                if (data[12] not in member_name_jtt) or (data[19] not in supply_int):
                    if data[2] not in create_date_jtt:
                        ws_jtt.append(data)
            wb_jtt.save(jtt_xlsx)
        print("cms 파일 비교 완료")
        xlsx2xls(os.listdir(folder_xlsx_path),payment_date)
        return xlsx_to_convert
...
def cmsfile_move(cms_xls, payment_date):
    payment_date_m = payment_date.month
    cms_name = os.path.basename(cms_xls)
    cmsfile_path = 'C:\\Users\\vivans\\Desktop\\stu_test\\2023\\'
    cmsfile_move_path = cmsfile_path+str(payment_date_m)+'월'
    if not os.path.exists(cmsfile_move_path):
        os.makedirs(cmsfile_move_path)

    move_cms = shutil.move(cms_xls,cmsfile_move_path+'\\'+cms_name)
    print("cms파일 이동완료")
...
def main(cms_xls):
    if '유치원' in cms_xls:
        ...
        if ('2022' in cms_xls) or ('2023' in cms_xls):
            print("해당연도있음")
            if (str(today.year) + today.strftime('%m')) in cms_xls or (str(today.year) + amonth_ago.strftime('%m') in cms_xls):
                print("1월용")
                ...
                cmsfile_move(cms_xls,payment_date)
            else:
                print("3검증에서 탈락")
                pass
...
if __name__ == "__main__":
    ...
    print(start_time)
    main(r'C:\Users\vivans\Desktop\stu_test\2023\20230201_20230201_유치원.xls')
