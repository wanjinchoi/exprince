...
root_path ='C:\\RPA\\유치원\\'
folder_path = 'C:\\RPA\\유치원\\'+ today.strftime('%Y') + '년\\' + amonth_ago.strftime('%#m')+'월'
...
def xlsx2xls(fail_xlsx_list, cms_xlsx_path):
    ...
    for fail_xlsx in fail_xlsx_list:
        if payment_date_agom+'월 서비스이용료' in fail_xlsx:
            ...
            xls_path = root_path+today.strftime('%Y')+'년\\'+amonth_ago.strftime('%#m')+'월'
            ...
            wb_xls = openpyxl.load_workbook(fail_xlsx)
            ws_xlsx = wb_xls.active
            xls_path = root_path + amonth_ago.strftime('%Y') + '년\\' + two_month_ago.strftime('%#m')+'월'
            file_name = fail_xlsx[:-1]
            r_filename = os.path.basename(file_name)
            dest = os.path.join(xls_path, r_filename)
            wb_xls.save(dest)
            ...
            wb_xls = openpyxl.load_workbook(fail_xlsx)
            ws_xlsx = wb_xls.active
            xls_path = root_path + two_month_ago.strftime('%Y') + '년\\' + three_month_ago.strftime('%#m')+'월'
            file_name = fail_xlsx[:-1]
            r_filename = os.path.basename(file_name)
            dest = os.path.join(xls_path, r_filename)
            wb_xls.save(dest)
...
if __name__ == "__main__":
    ...
    print(start_time)
    main(r'C:\Users\vivans\Desktop\stu_test\20230701_20230701_유치원.xls')
