...
class Receiving(PySelenium):
    ...
    def get_page(self):
        try:
            ...
            try:
                ...
                pass
            tap_check = self.get_by_xpath('//*[@id="state_60_1"]')
            ...
            for i in range(len(bil)):
                ...
                try:
                    ...
                    if 'X' in findx:
                        self.get_by_xpath(
                            '/html/body/div[4]/div[5]/div[1]/div[4]/div[2]/div[3]/table/tbody/tr[' + f"{i + 1}" + ']/td[1]/div/input').click()
                        continue
                    if 'x' in findx:
                        self.get_by_xpath(
                            '/html/body/div[4]/div[5]/div[1]/div[4]/div[2]/div[3]/table/tbody/tr[' + f"{i + 1}" + ']/td[1]/div/input').click()
                    ...
                    self.logger.error('Receiving2' + err)
                    print(err)
                    raise
    ...
    def set_WMS(self):
        ...
        # 테스트 폴더 네임
        # self.folder_name = rf'C:\work\newone\1.RegisterOrder\data\Receiving\20241029-183720'
        ...
        for filename in os.listdir(self.folder_name):
            ...
            if filename.endswith('.json'):
                with open(filepath, 'r', encoding='utf-8') as file:
                    ...
                    self.logger.info('fine2')

    # ==============================================
    def next_page(self):
        try:
            # screenshotfolder = self.folder_name.split('\\')[-1]
            # self.full_screenshot('C:/work/newone/1.RegisterOrder/data/screenshot/receiving/' + screenshotfolder + '.jpg')
            time.sleep(2)
            # self.switch_to_window(0)
            # 페이지 목록
            self.get_by_xpath('//button[contains(text(), "진행상태변경")]').click()
            time.sleep(1)
            element = self.get_by_xpath('/html/body/div[6]/table/tbody/tr[2]/td[2]/div/div[1]/div/div[3]/a')
            element.click()
            time.sleep(3)
            # screenshotfolder = self.folder_name.split('\\')[-1]
            # self.full_screenshot('C:/work/newone/1.RegisterOrder/data/screenshot/receiving/' + screenshotfolder + '.jpg')
