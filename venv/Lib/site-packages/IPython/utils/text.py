...
def _col_chunks(l, is, row_first=False):
    """Yield successive is-sized column chunks from l."""
    if row_first:
        ncols = (len(l) // is) + (len(l) % is > 0)
        for i in range(ncols):
            yield [l[j] for j in range(i, len(l), ncols)]
    else:
        for i in range(0, len(l), is):
            yield l[i:(i + is)]


def _find_optimal(rlist, row_first=False, separator_size=2, displaywidth=80):
    """Calculate optimal info to columnize a list of string"""
    for is in range(1, len(rlist) + 1):
        col_widths = list(map(max, _col_chunks(rlist, is, row_first)))
...
def compute_item_matrix(items, row_first=False, empty=None, *args, **kwargs) :
    ...
    ::
        ...
        In [4]: ideal = {'num_columns': 3, 'column_widths': [5, 1, 1], 'optimal_separator_width': 2, 'is': 5}
        In [5]: all((info[k] == ideal[k] for k in ideal.keys()))
        Out[5]: True
    """
    info = _find_optimal(list(map(len, items)), row_first, *args, **kwargs)
    nrow, ncol = info['is'], info['num_columns']
    if row_first:
