...
#
#  * [2023/03/21]
#   - C:\Users\toor\.argos-rpa.venv\Python39 Python 3.9로 업그레이드
#  * [2023/01/11]
#   - Ampliforce 용 White-Label 작업
#     ~/.argos-rpa-config.yaml => ~/.alfred-config.yaml
#     ~/.argos-rpa.log* => ~/.alfred.log*
#     ~/.argos-rpa.venv => ~/.alfred.venv
#     ~/.argos-rpa.cache => ~/.alfred.cache
#  * [2022/12/20]
#   - 화면 표시에서 RPA* 지움
#  * [2022/10/02]
#   - Ampliforce OEM 버전용 출력에서 ARGOS => "" 치환
#  * [2022/09/07]
#   - Konika Minolta 의 경우는 특정 계정은 private repo 로 pypi-private.argos-labs.com 을 저장소로 가지고 있었으나
#     PAM only 계정과 같은 하위 계정은 상위 계정의 private plugin 정보를 가져와서 사용하지 못하는 API의 문제
#  * [2022/08/09]
#   - 네트워크가 안되는 On-Prem 같은 환경에서는 pypi.org나 pypi-official.argos-labs.com 을 pip 에 참조 한하도록 수정
#   - If not connected to internet: donot upgrade pip
#     And add --proxy option
#  * [2022/07/18]
#   - _get_indices() 에서 무조건 ~/pypi 를 ~/simple 로 변경한 부분을 다시 뺌
#     Nozomi On-Prem 처리하면서 문제 발생되었음
#  * [2022/06/28]
#   - Free/Citizen 계정에서 upload 안되는 문제를 따라갔더니
#     https://api-rpa.argos-labs.com/oauth2 => https://api2-rpa.argos-labs.com/oauth2
#   - .argos-rpa-config.yaml 을 읽는 경우 처리
#  * [2022/04/27]
...
################################################################################
ALFRED_YAML_NAME = '.alfred-config.yaml'
ALFRED_YAML_PATH = os.path.join(str(Path.home()), ALFRED_YAML_NAME)


################################################################################
def get_argos_rpa():
    if os.path.exists(ALFRED_YAML_PATH):
        return 'alfred'
    return 'argos-rpa'


################################################################################
YAML_NAME = f'.{get_argos_rpa()}-config.yaml'
...
class StatLogger(object):
    ...
    def __init__(self, is_clear=False):
        ...
        if get_argos_rpa() == 'alfred':
            self.filter_msg = StatLogger._filter_msg
        if is_clear:
            self.clear()
    ...
    @staticmethod
    def _filter_msg(msg):
        # for Ampliforce white lable
        msg = msg.replace("RPA", "")
        msg = msg.replace("ARGOS", "")
        msg = msg.replace("Argos", "")
        msg = msg.replace("argos", "-")
        msg = msg.replace("STU", "EDITOR")
        msg = msg.replace("PAM", "RUNNER")
        return msg
...
class VEnv(object):
    ...
    def _can_upgrade_pip(self):
        # If not connected to internet: donot upgrade pip
        if not is_svc_opeded('pypi.org', 443):
            return False
...
class PPM(object):
    ...
    URL_OAUTH = [
        'https://api2-rpa.argos-labs.com/oauth2',
        # 'https://api-rpa.argos-labs.com/oauth2', # OLD
    ...
    # ==========================================================================
    SELFTEST_ROOT = os.path.join(str(Path.home()), f'.{get_argos_rpa()}.test')
    ...
    def __init__(self, _venv, args, logger=None, sta=None):
        self.venv = _venv
        self.args = args
        self.py_cache = os.path.join(str(Path.home()), f'.{get_argos_rpa()}.cache')
    ...
    def _get_indices(self):
        ...
        self.logger.debug(f'')
        # 2022.7.18 아래 내용을 Nozomi On-Prem 처리하면서 다시 뺌: alabsPpmHost 에 지정한 대로
        # # 만약 url 이 ~/pypi 로 끝나면 ~/simple 로 변경
        # if url.endswith('/pypi'):
        #     url = url[:-4] + 'simple'
        self._append_indices(url)
        # 3.9 로 작업하면서 모두 --extra-index-url 로 변경해 주어야 잘 실행됨
        # 제일 첫번째를 --index 로 나머지는 --extra-index-url로
        # if not self.ndx_param:
        ...
        if not is_svc_opeded('pypi-official.argos-labs.com', 443):
            self.ndx_param.append('--proxy')
            o = urlparse(url)
            proxy_url = '{scheme}://{netloc}'.format(scheme=o.scheme, netloc=o.netloc)
            self.ndx_param.append(proxy_url)
        ...
        if not is_pr:
            ...
            return self.indices
        # if cannot connect pypi-official.argos-labs.com
    ...
    def _dumpspec_json(self, modname, version=None):
        ...
        for f in glob.glob(glob_filter):
            ...
            break
        if not mfilename:
            err_msg = 'PPM._dumpspec_json: Cannot get plugin wheel file for "%s"' % modname
            self.sta.error(err_msg)
    ...
    def _set_home_pyvenv_cfg(self, new_d):
        ...
        with open(pyvenv_cfg_f, 'w', encoding='utf-8') as ofp:
            ...
            if os.path.exists(f'{venv_root}{os.path.sep}Python39'):
                ofp.write(f'''home = {venv_root}{os.path.sep}Python39
...
six [('==', '1.10.0')]
    ...
    def _clear_log(self):
        ...
        for handler in handlers:
            handler.close()
        for f in glob.glob(os.path.join(str(Path.home()), f'.{get_argos_rpa()}.log*')):
            os.remove(f)
    ...
    def _report_selftest(self, st_r):
        ...
        dnames = []
        attachments = []
        ceyaml = os.path.join(str(Path.home()), f'.{get_argos_rpa()}.test', '.check_env.yaml')
    ...
    def _venv_clean(self):
        py_root = os.path.join(str(Path.home()), f'.{get_argos_rpa()}.venv')
    ...
    def do_plugin(self):
        ...
        try:
            ...
            if self.args.plugin_cmd == 'venv':
                ...
                venv_d = os.path.join(str(Path.home()), f'.{get_argos_rpa()}.venv')
                ...
                if not match_list:
                    self.sta.log(StatLogger.LT_1, 'Preparing environment. This process may take a few minutes.')
                    now = datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
                    get_venv = os.path.join(venv_d, now)
                    self._get_venv(get_venv)
                else:  # 아니면 가장 많이 매칭된 것을 찾아 이용
                    self.sta.log(StatLogger.LT_1, 'An previous environment was found.')
    ...
    def _get____tk____(self, u___i, p___w):
        try:
            ...
            if root_url == 'https://api-rpa.argos-labs.com/oauth2':
                root_url = 'https://api2-rpa.argos-labs.com/oauth2'
            cookies = {
            ...
            post_url = '%s/oauth/token' % root_url
            ...
            if not self.args.official_only:
                ...
                if self.args.command in ('upload',):
                    ...
                    if not self.args.user:
                        user = input('%s command need user id for Supervisor, User ID? '
                                        % self.args.command)
                    ...
                    if not self.args.user_auth:
                        ...
                        if not u___p:
                            u___p = getpass('%s command need user password for Supervisor, User Password? '
                                            % self.args.command)
...
def _main(argv=None):
    ...
    try:
        ...
        parser = ArgumentParser(
            description='''Plugin Package Manager''',
