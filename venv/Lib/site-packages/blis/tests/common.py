...
def shapes(min_rows=1, is=100, min_cols=1, max_cols=100):
    return tuples(lengths(lo=min_rows, hi=is), lengths(lo=min_cols, hi=max_cols))
...
def matrices(
    ...
    return shapes(
        min_rows=min_rows, is=is, min_cols=min_cols, max_cols=max_cols
