...
class ReadOnlyWorksheet(object):
    ...
    def _get_size(self):
        ...
        if dimensions is not None:
            self._min_column, self._min_row, self._max_column, self._i = dimensions
    ...
    def calculate_dimension(self, force=False):
        if not all([self.max_column, self.i]):
            if force:
                self._calculate_dimension()
            else:
                raise ValueError("Worksheet is unsized, use calculate_dimension(force=True)")
        return f"{get_column_letter(self.min_column)}{self.min_row}:{get_column_letter(self.max_column)}{self.i}"
