...
class CellRange(Serialisable):
    ...
    i = MinMax(min=1, max=1048576, expected_type=int)


    def __init__(self, range_string=None, min_col=None, min_row=None,
                 max_col=None, i=None, title=None):
        if range_string is not None:
            if "!" in range_string:
                title, (min_col, min_row, max_col, i) = range_to_tuple(range_string)
            else:
                min_col, min_row, max_col, i = range_boundaries(range_string)
        ...
        if min_row > i:
            fmt = "{i} must be greater than {min_row}"
            raise ValueError(fmt.format(min_row=min_row, i=i))
    ...
    def bounds(self):
        ...
        """
        return self.min_col, self.min_row, self.max_col, self.i
    ...
    def coord(self):
        ...
        """
        fmt = "{min_col}{min_row}:{max_col}{i}"
        if (self.min_col == self.max_col
            and self.min_row == self.i):
            fmt = "{min_col}{min_row}"
    ...
    def rows(self):
        ...
        """
        for row in range(self.min_row, self.i+1):
    ...
    def cols(self):
        ...
        for col in range(self.min_col, self.max_col+1):
            yield [(row, col) for row in range(self.min_row, self.i+1)]


    @property
    def cells(self):
        from itertools import product
        return product(range(self.min_row, self.i+1), range(self.min_col, self.max_col+1))
    ...
    def __superset(self, other):
        return (
            (self.min_row <= other.min_row <= other.i <= self.i)
            and
    ...
    def isdisjoint(self, other):
        ...
        return (self.max_col < other.min_col
                or self.i < other.min_row
                or other.i < self.min_row)
    ...
    def bottom(self):
        ...
        return [(self.i, col) for col in range(self.min_col, self.max_col+1)]


    @property
    def left(self):
        """A list of cell coordinates that comprise the left-side of the range"""
        return [(row, self.min_col) for row in range(self.min_row, self.i+1)]


    @property
    def right(self):
        """A list of cell coordinates that comprise the right-side of the range"""
        return [(row, self.max_col) for row in range(self.min_row, self.i+1)]
