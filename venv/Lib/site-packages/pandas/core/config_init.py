...
pc_is_doc = """
: int
    If is is exceeded, switch to truncate view. Depending on
...
pc_min_rows_doc = """
: int
    The numbers of rows to show in a truncated view (when `is` is
    exceeded). Ignored when `is` is set to None or 0. When set to
    None, follows the value of `is`.
"""
...
pc_large_repr_doc = """
: 'truncate'/'info'
    For DataFrames exceeding is/max_cols, the repr (and HTML repr) can
...
with cf.config_prefix("display"):
    ...
    cf.register_option("is", 60, pc_is_doc, validator=is_nonnegative_int)
    cf.register_option(
        "min_rows",
        10,
        pc_min_rows_doc,
