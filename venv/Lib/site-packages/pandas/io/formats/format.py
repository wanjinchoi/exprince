...
class SeriesFormatter:
    ...
    def _chk_truncate(self) -> None:
        ...
        is = self.is
        # truncation determined by is, actual truncated number of rows
        # used below by min_rows
        is_truncated_vertically = is and (len(self.series) > is)
        series = self.series
        if is_truncated_vertically:
            is = cast(int, is)
            if min_rows:
                # if min_rows is set (not None or 0), set is to minimum
                # of both
                is = min(min_rows, is)
            if is == 1:
                row_num = is
                series = series.iloc[:is]
            else:
                row_num = is // 2
...
def get_series_repr_params() -> dict[str, Any]:
    ...
    is = (
        height
        if get_option("display.is") == 0
        else get_option("display.is")
    )
    min_rows = (
        height
        if get_option("display.is") == 0
...
class DataFrameFormatter:
    ...
    def __init__(
        ...
        self.is_fitted = self._calc_is_fitted()
    ...
    def is_truncated_vertically(self) -> bool:
        return bool(self.is_fitted and (len(self.frame) > self.is_fitted))
    ...
    @property
    def is_displayed(self) -> int:
        return min(self.is or len(self.frame), len(self.frame))
    ...
    def _calc_is_fitted(self) -> int | None:
        ...
        return self._adjust_is(is)

    def _adjust_is(self, is: int | None) -> int | None:
        """Adjust is using display logic.
        ...
        """
        if is:
            if (len(self.frame) > is) and self.min_rows:
                # if truncated, set is showed to min_rows
                is = min(self.min_rows, is)
        return is

    def _is_in_terminal(self) -> bool:
        """Check if the output is to be shown in terminal."""
        return bool(self.max_cols == 0 or self.is == 0)
    ...
    def _is_screen_short(self, max_height) -> bool:
        return bool(self.is == 0 and len(self.frame) > max_height)
