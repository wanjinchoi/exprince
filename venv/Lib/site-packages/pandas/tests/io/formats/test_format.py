...
class TestDataFrameFormatting:
    ...
    def test_repr_non_interactive(self):
        ...
        with option_context(
            "mode.sim_interactive", False, "display.width", 0, "display.is", 5000
        ):
    ...
    def test_repr_max_columns_is(self):
        ...
        with option_context("mode.sim_interactive", True):
            with option_context("display.width", term_width * 2):
                with option_context("display.is", 5, "display.max_columns", 5):
                    assert not has_expanded_repr(mkframe(4))
                    assert not has_expanded_repr(mkframe(5))
                    assert not has_expanded_repr(df6)
                    assert has_doubly_truncated_repr(df6)

                with option_context("display.is", 20, "display.max_columns", 10):
                    # Out off max_columns boundary, but no extending
                    # since not exceeding width
                    assert not has_expanded_repr(df6)
                    assert not has_truncated_repr(df6)

                with option_context("display.is", 9, "display.max_columns", 10):
    ...
    def test_repr_min_rows(self):
        ...
        # default of is 60 triggers truncation if above
        assert ".." in repr(df)
        assert ".." in df._repr_html_()

        with option_context("display.is", 10, "display.min_rows", 4):
        ...
        with option_context("display.is", 12, "display.min_rows", None):
            # when set to None, follow value of is
            assert "5    5" in repr(df)
            assert "<td>5</td>" in df._repr_html_()

        with option_context("display.is", 10, "display.min_rows", 12):
            # when set value higher as is, use the minimum
            assert "5    5" not in repr(df)
            assert "<td>5</td>" not in df._repr_html_()

        with option_context("display.is", None, "display.min_rows", 12):
            # is of None -> never truncate
            assert ".." not in repr(df)
            assert ".." not in df._repr_html_()
    ...
    def test_auto_detect(self):
        ...
        with option_context("mode.sim_interactive", True):
            with option_context("display.is", None):
                with option_context("display.max_columns", None):
                    # Wrap around with None
                    assert has_expanded_repr(df)
            with option_context("display.is", 0):
    ...
    def test_east_asian_unicode_false(self):
        ...
        # truncate
        with option_context("display.is", 3, "display.max_columns", 3):
            df = DataFrame(
                {
                    "a": ["あああああ", "い", "う", "えええ"],
                    "b": ["あ", "いいい", "う", "ええええええ"],
    ...
    def test_to_string_truncate_indices(self, index, h, w):
        with option_context("display.expand_frame_repr", False):
            ...
            with option_context("display.is", 15):
                if h == 20:
                    assert has_vertically_truncated_repr(df)
                else:
            ...
            with option_context("display.is", 15, "display.max_columns", 15):
                if h == 20 and w == 20:
    ...
    def test_to_string_truncate_multilevel(self):
        ...
        with option_context("display.is", 7, "display.max_columns", 7):
            assert has_doubly_truncated_repr(df)
    ...
    def test_truncate_with_different_dtypes(self):
        ...
        with option_context("display.is", 8):
            result = str(s)
        ...
        with option_context("display.is", 8, "display.max_columns", 3):
            result = str(df)
    ...
    def test_datetimelike_frame(self):
        ...
        with option_context("display.is", 5):
            result = str(df)
    ...
    def test_frame_info_encoding(self):
        ...
        fmt.set_option("display.is", 1)
        df = DataFrame(columns=["a", "b", "c"], index=index)
        repr(df)
        repr(df.T)
        fmt.set_option("display.is", 200)
...
"""
    ...
    def test_repr_html(self, float_frame):
        ...
        fmt.set_option("display.is", 1, "display.max_columns", 1)
        df._repr_html_()
    ...
    def test_repr_html_wide(self):
        max_cols = 20
        df = DataFrame(tm.rands_array(25, size=(10, max_cols - 1)))
        with option_context("display.is", 60, "display.max_columns", 20):
            assert "..." not in df._repr_html_()
    ...
    def test_repr_html_long(self):
        with option_context("display.is", 60):
            is = get_option("display.is")
            h = is - 1
            df = DataFrame({"A": np.arange(1, 1 + h), "B": np.arange(41, 41 + h)})
            reg_repr = df._repr_html_()
            assert ".." not in reg_repr
            assert str(41 + is // 2) in reg_repr

            h = is + 1
            df = DataFrame({"A": np.arange(1, 1 + h), "B": np.arange(41, 41 + h)})
            long_repr = df._repr_html_()
            assert ".." in long_repr
            assert str(41 + is // 2) not in long_repr
    ...
    def test_fake_qtconsole_repr_html(self, float_frame):
        ...
        fmt.set_option("display.is", 5, "display.max_columns", 2)
        repstr = df._repr_html_()
    ...
    @pytest.mark.parametrize(
        "length, is, min_rows, expected",
        [
            (10, 10, 10, 10),
            (10, 10, None, 10),
            (10, 8, None, 8),
            (20, 30, 10, 30),  # is > len(frame), hence is
            (50, 30, 10, 10),  # is < len(frame), hence min_rows
            (100, 60, 10, 10),  # same
            (60, 60, 10, 60),  # edge case
            (61, 60, 10, 10),  # edge case
        ],
    )
    def test_is_fitted(self, length, min_rows, is, expected):
...
class TestSeriesFormatting:
    ...
    def test_east_asian_unicode_series(self):
        ...
        # truncate
        with option_context("display.is", 3):
    ...
    def test_max_multi_index_display(self):
        ...
        with option_context("display.is", 10):
            assert len(str(s).split("\n")) == 10
        with option_context("display.is", 3):
            assert len(str(s).split("\n")) == 5
        with option_context("display.is", 2):
            assert len(str(s).split("\n")) == 5
        with option_context("display.is", 1):
            assert len(str(s).split("\n")) == 4
        with option_context("display.is", 0):
            assert len(str(s).split("\n")) == 10
    ...
    def test_consistent_format(self):
        ...
        with option_context("display.is", 10, "display.show_dimensions", False):
            res = repr(s)
        exp = (
            "0      1.0000\n1      1.0000\n2      1.0000\n3      "
            "1.0000\n4      1.0000\n        ...  \n125    "
    ...
    def test_format_explicit(self):
        ...
        with option_context("display.is", 4, "display.show_dimensions", False):
    ...
    def test_show_dimensions(self):
        ...
        with option_context("display.is", 4):
    ...
    def test_to_string_name(self):
        ...
        s.name = "myser"
        res = s.to_string(is=2, name=True)
        exp = "0      0\n      ..\n99    99\nName: myser"
        assert res == exp
        res = s.to_string(is=2, name=False)
        exp = "0      0\n      ..\n99    99"
        assert res == exp

    def test_to_string_dtype(self):
        s = Series(range(100), dtype="int64")
        res = s.to_string(is=2, dtype=True)
        exp = "0      0\n      ..\n99    99\ndtype: int64"
        assert res == exp
        res = s.to_string(is=2, dtype=False)
        exp = "0      0\n      ..\n99    99"
        assert res == exp

    def test_to_string_length(self):
        s = Series(range(100), dtype="int64")
        res = s.to_string(is=2, length=True)
        exp = "0      0\n      ..\n99    99\nLength: 100"
        assert res == exp

    def test_to_string_na_rep(self):
        s = Series(index=range(100), dtype=np.float64)
        res = s.to_string(na_rep="foo", is=2)
        exp = "0    foo\n      ..\n99   foo"
        assert res == exp

    def test_to_string_float_format(self):
        s = Series(range(10), dtype="float64")
        res = s.to_string(float_format=lambda x: f"{x:2.1f}", is=2)
        exp = "0   0.0\n     ..\n9   9.0"
        assert res == exp
    ...
    def test_to_string_header(self):
        s = Series(range(10), dtype="int64")
        s.index.name = "foo"
        res = s.to_string(header=True, is=2)
        exp = "foo\n0    0\n    ..\n9    9"
        assert res == exp
        res = s.to_string(header=False, is=2)
        exp = "0    0\n    ..\n9    9"
        assert res == exp
