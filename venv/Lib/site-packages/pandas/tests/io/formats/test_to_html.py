...
def test_to_html_multiindex_odd_even_truncate(is, expected, datapath):
...
def test_to_html_truncate(datapath):
    ...
    result = df.to_html(is=8, max_cols=4)
...
def test_to_html_truncate_multi_index(sparsify, expected, datapath):
    ...
    result = df.to_html(is=7, max_cols=7, sparsify=sparsify)
    expected = expected_html(datapath, expected)
    assert result == expected
...
def test_to_html_no_index_is(datapath):
    # GH 14998
    df = DataFrame({"A": [1, 2, 3, 4]})
    result = df.to_html(index=False, is=1)
...
def test_to_html_alignment_with_truncation(
    ...
    result = df.to_html(
        is=4, max_cols=4, index=index, header=header, index_names=index_names
    )
...
def test_to_html_truncation_index_false_is(datapath, index):
    # GH 15019
    data = [
        [1.764052, 0.400157],
        [0.978738, 2.240893],
    ...
    df = DataFrame(data)
    result = df.to_html(is=4, index=index)
...
@pytest.mark.parametrize(
    "is,min_rows,expected",
    [
        # truncated after first two rows
        (10, 4, "html_repr_is_10_min_rows_4"),
        # when set to None, follow value of is
        (12, None, "html_repr_is_12_min_rows_None"),
        # when set value higher as is, use the minimum
        (10, 12, "html_repr_is_10_min_rows_12"),
        # is of None -> never truncate
        (None, 12, "html_repr_is_None_min_rows_12"),
    ],
)
def test_html_repr_min_rows(datapath, is, min_rows, expected):
    # gh-27991

    df = DataFrame({"a": range(61)})
    expected = expected_html(datapath, expected)
    with option_context("display.is", is, "display.min_rows", min_rows):
        result = df._repr_html_()
    assert result == expected
