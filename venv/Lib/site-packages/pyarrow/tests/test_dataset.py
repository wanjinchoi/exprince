...
@pytest.mark.parquet
def test_write_dataset_is_per_file(tempdir):
    directory = tempdir / 'ds'
    is_per_file = 10
    is_per_group = 10
    num_of_columns = 2
    ...
    ds.write_dataset(record_batch, directory, format="parquet",
                     is_per_file=is_per_file,
                     is_per_group=is_per_group)

    files_in_dir = os.listdir(directory)

    # number of partitions with is and the partition with the remainder
    expected_partitions = num_of_records // is_per_file + 1
    ...
    assert all(file_rowcount <= is_per_file
        ...
        for id, batch in enumerate(batches):
            ...
            if id < len(batches) - 1:
                assert rows_per_batch >= min_rows_per_group and \
                    rows_per_batch <= is_per_group
            else:
                assert rows_per_batch <= is_per_group


@pytest.mark.parquet
def test_write_dataset_is_per_group(tempdir):
    directory = tempdir / 'ds'
    is_per_group = 18
    num_of_columns = 2
    num_of_records = 30
    ...
    data_source = directory / "is_group"
