...
#     - 쿠키파일 경로 고정
#  * [2024/04/22]
#     - 댓글 이미지 수집 방식 변경(EXTVOCGE-1585)
#     - innerHTML에서 img 태그 확인 방식
#  * [2024/03/08]
#     - error return 코드 세분화(로그인 에러: 10, open 에러:11, 그외 나머지:1)
#  * [2024/02/28]
#     - 로그인 부분 로직 수정
#  * [2024/01/08]
#     - 미노출 투표 기능 스킵
#  * [2023/09/11]
#     - (분석측 요청)비밀 댓글 comment_id None 처리 제외
#  * [2023/08/23]
#     - 다음 로그인 체크 하는 로직 추가
#  * [2023/08/21]
#     - 게시글 목록 스크린 샷 추가
...
#     - 로그인 xpath 변경
#  * [2023/06/13]
#     - 방석위로 모여라 카페의 경우, 쭉빵카페에서 로그인 후 다시 돌아가는 방식으로 수집
#  * [2023/04/15]
#     - 댓글 이미지 클릭하면 에러 발생해서 주석처리
#  * [2023/04/14]
#     - 원복
#  * [2023/04/13]
#     - 게시글 목록 스크린샷 추가
...
class DaumCafeSearch(PySelenium):
        ...
        try:
            ...
            if b.text.find('이 카페 회원 등급 보기') >= 0:
                ...
                for _ in range(click_count):
                    self.logger.info(
                    f'Page[{self.cur_page}:{ndx}],article_id[{msg["article_id"]}],title="{msg["title"]}" - 회원 등급 제한, 다음 쿠키 만료')
                    self.driver.back()
                pass
        except:
            try:
    ...
    def get_page(self):
        try:
            ...
            self.switch_to_iframe_by_name('down')
            # 페이지 테이블 구해오기
            bil = self.driver.find_elements_by_xpath('//tr[@class="list_row_info"]')
            ...
            for i in range(len(bil)):
                ...
                try:
                    # "카페 메인 (cafe_main)" iFrame으로 이동
                    self.switch_to_iframe_by_name('down')
                    ail = self.driver.find_elements_by_xpath('//tr[@class="list_row_info"]')
                    bi = ail[i]

                    # 1) 게시글id : article_id
                    se = bi.find_element_by_xpath('.//td[@class="search_num"]')
                    msg['article_id'] = self.cafe_id+'_'+se.text.strip()
                    # 2) 제목: title
                    title_e = se = bi.find_element_by_xpath('.//td[@class="subject searchpreview_subject"]/a[1]')
                    msg['title'] = se.text.strip()
                    # 3) 댓글수: num_comments
                    try:
                        e = bi.find_element_by_xpath('.//td[@class="subject searchpreview_subject"]/a[2]')
                        if e:
                            msg['num_comments'] = int(re.sub(r'[^0-9]', '', e.text.strip()))
                    ...
                    msg['article_url'] = title_e.get_attribute('href')
