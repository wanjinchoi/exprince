...
class NaverCafeLogin(PySelenium):
    # ==========================================================================
    def __init__(self, id, pw, user_data_path):
        self.id = id
        self.pw = pw
        self.config = {
        ...
        self.log_d = 'C:/work/voc_data/logs/cafe/naver/login_check'
        ...
        logger = get_logger(self.get_safe_path(self.log_d, 'NaverCafe_user_data.log'),
                            logsize=1024*1024*10)
        self.config['logger'] = logger
        PySelenium.__init__(self, **self.config)

        self.logger.info(f'Starting Naver Cafe Login... making user_data ')
    ...
    def login(self):
        try:
            ...
            if inner_html.find('gnb_login_button') > 0:
                # 로그인 되어있는지 체크
                e_s = e.find_element_by_xpath('//*[@id="gnb_login_button"]/..')
                if e_s.get_attribute('style') == 'display: none;':
                    self.logger.info('#'*20+'Logging in..')
                    return 0
            return 1
            #     # 로그인 클릭
            ...
            #
            # # 상단 ID로그인 클릭
            # e = self.driver.find_element_by_xpath('//a[@id="loinid"]')
            # self.safe_click(e)
...
################################################################################
def do_start(**kwargs):
    with NaverCafeLogin(kwargs['id'], kwargs['pw'], kwargs['user_data_path']) as ws:
        return ws.start()
...
if __name__ == '__main__':
    id = 'sebin3481'
    pw = '******'
    user_data_path = r'C:/work/voc/pySelenium/cafe/naver/PySelenium.cahce/userData'
    do_start(id=id, pw=pw, user_data_path=user_data_path)
