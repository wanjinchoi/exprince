    ...
    def get_article(self, msg, ndx):
        try:
            ...
            try:
                ...
                # 삭제된 게시글
                self.save_a = False
                self.logger.info(f'Page[{self.cur_page}:{ndx}],article_id[{msg["article_id"]}],title="{msg["title"]}" is 삭제된 게시글')
                return
            ...
            if msg['board_name'].find('간편게시판') > 0:
                ...
                msg['contents'] = contents
                # 투표
                vote_area = self.driver.find_elements_by_xpath('//div[@class="cafe_vote_wrap"]')
                if vote_area:
                    self.vote_data = True
