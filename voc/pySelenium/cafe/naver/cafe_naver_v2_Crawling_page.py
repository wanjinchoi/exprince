...
#
#  * [2024/05/30]
#     - 카페 댓글 리스트 UI 변경으로 Xpath 수정
#  * [2024/05/13]
#     - 카페 댓글 내용의 멘션 부분 제외
#  * [2024/03/04]
#     - 분리된 페이지 선택하는 부분에 에러가 존재하여 수정(케이스 분류)
#     (검색 결과가 존재하지 않는 경우, 지정된 페이지보다 적은 페이지가 노출되는 경우, 여러개의 페이지가 노출되는 경우)
#  * [2024/02/28]
#     - 페이지 분리하여 수집
...
class NaverCafeSearch(PySelenium):
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            self.logger.info(f"crawling article time: {elapsed_time / 1000} sec")
            # 이전 페이지
            self.driver.back()
            try:
                self.switch_to_iframe_by_name('cafe_main')
        ...
        try:
            ...
            for i in range(len(bil)):
                ...
                if len(self.output['article_list']) >= \
                    ...
                    self.logger.error('Stop crawling because max_article is' + str({self.config['params']['site']['max_articles']}))
                    self.is_done = True
                    break
    ...
    def save(self):
        ...
        if 'page_separation' in self.output['config']['params']['site']:
            del self.output['config']['params']['site']['page_separation']
    ...
    def page_separation(self):
        ...
        try:
            ...
            # "카페 메인 (cafe_main)" iFrame으로 이동
            self.switch_to_iframe_by_name('cafe_main')
            e = self.driver.find_elements_by_xpath('//div[@class="prev-next"]//a')
            if e:
                # 페이지 목록
                # ple = self.get_by_xpath('//div[@class="prev-next"]')
                is_on = True
                while is_on:
                    ...
                    ple = self.get_by_xpath('//div[@class="prev-next"]')
                    # 페이지 개수가 10개 이하인 경우
                    if len(ple.find_elements_by_xpath('.//a')) <= 10:
                        if len(ple.find_elements_by_xpath('.//a')) <= int(self.config['params']['site']['page_separation']):
                            self.logger.error(f'num_page is less than page_separation')
                            self.is_done = True
                            return
                        else:
                            for pa in ple.find_elements_by_xpath('.//a'):
    ...
    def start(self, config_f, i=None, keyword=None):
        try:
            ...
            if self.config['params']['site']['get_cafe_info']:
                self.get_cafe_info()
            self.search()
            if self.config['params']['site']['page_separation'] > 1:
                self.page_separation()
...
if __name__ == '__main__':
    _config_f = 'cafe_naver_page.yaml'
