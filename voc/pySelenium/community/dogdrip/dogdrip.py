...
class DogdripSearch(PySelenium):
        ...
        try:
            ...
            while True:
                for i, cmt_e in enumerate(comments):
                    ...
                    # 댓글 작성 시간
                    cmt_time = cmt_e.find_element_by_xpath('.//div[@class="ed flex flex-right"]//span')
                    when_cmt = cmt_time.text.strip()
                    ddd, hhh, mmm = 0, 0, 0
                    # 38분 전
                    if when_cmt.find('분') > 0:
                        mm = re.sub(r'[^0-9]', '', when_cmt)
                        mmm = int(mm)
                    # 5시간 전
                    elif when_cmt.find('시간') > 0:
                        hh = re.sub(r'[^0-9]', '', when_cmt)
                        hhh = int(hh)
                    elif when_cmt.find('일') > 0:
                        dd = re.sub(r'[^0-9]', '', when_cmt)
                        ddd = int(dd)
                    else:
                        cmt['create_ts'] = datetime.datetime.strptime(when_cmt, '%Y.%m.%d').strftime('%Y.%m.%d %H:%M:%S')
                    ...
                    # 댓글 닉네임
                    e = cmt_e.find_element_by_xpath('.//div[@class="ed inline-flex flex-middle margin-right-small"]/h6/a')
                    ea = e.text.strip()
                    cmt['nickname'] = ea
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            # 추천수
            content_e = self.get_by_xpath('//div[@id="article_1"]')
            e = content_e.find_element_by_xpath('.//span[@class="btnRv btnRo"]//button//span[2]')
            # msg['like'] = int(e.text.split()[1].replace(',', ''))
            msg['like'] = int(e.text.strip())
            # 싫어요
            e = content_e.find_element_by_xpath('.//span[@class="btnRv btnRx"]//button//span[2]')
            # msg['dislike'] = int(e.text.split()[1].replace(',', ''))
            msg['dislike'] = int(e.text.strip())
            ...
            e = content_e.find_element_by_xpath('./div')
