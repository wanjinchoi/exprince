...
# --------
#  * [2024/10/11]
#     - 사이트 접속 시 캡챠 발생하면 페이지 스크린샷, HTML 남김
#  * [2024/09/30]
#     - 신규 댓글 노출이 무한 스크롤이라서 댓글이 있는 경우 전체 수집하고 스크린샷 진행
#  * [2024/09/18]
#     - 게시글과 댓글의 등록 시간 형식 변경되어 날짜 형식 변경(2024.03.24 00:00:00 -> 2024/03/24 00:00:00)
#  * [2024/09/05]
#     - xpath가 변경된 경우, 수집 실패가 발생하도록 모듈 수정
#  * [2024/08/26]
...
#  * [2024/05/09]
#     -  본문 XPath 수정(xpath가 다른 게시글이 존재-p태그 없는)
#  * [2024/03/19]
#     -  에러 코드 세분화
#     -  에러 로그 세분화
#     -  게시글 존재 여부 확인 로직 수정
...
#     -  본문 XPath 수정
#  * [2023/03/30]
#     -  게시판명이 존재하는 게시글의 경우 board_name에 데이터 입력 나머지는 null
#  * [2023/03/27]
#     -  댓글 작성시간, 댓글 ID xpath 변경
#  * [2022/12/08]
#     -  페이지 테이블 xpath 변경
#  * [2022/08/31]
#     -  article_id xpath 변경
...
class Instiz01(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Instiz01.log'),
                            logsize=1024 * 1024 * 10)
        self.logger_c = get_logger(self.get_safe_path(log_d, 'Instiz01_HTML.log'),
                            logsize=1024 * 1024 * 10)
        ...
        self.logger.info(f'Starting Instiz01 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        try:
            ...
            s_shot = self.config['target']['folder'].replace('latest', 'logs') + '.png'
            s_shot = s_shot[:s_shot.rfind('/')] + '/list_capture' + s_shot[s_shot.rfind('/'):]
            self.driver.save_screenshot(s_shot)
            # HTML 저장
            # 현재 페이지의 소스 코드 가져오기
            html_source = self.driver.page_source
            # HTML 소스를 텍스트로 로깅
            self.logger_c.info(f"HTML 소스:\n{html_source}")
            raise
    # ==========================================================================
    ...
    def get_comment(self, msg):
        try:
            ...
            comments = e_a.find_elements_by_xpath('.//div[@class="comment_line"]')
            parent_comment_id = ''
            ...
            for i, cmt_e in enumerate(comments):
                ...
                # 댓글 작성 시간
                e = cmt_e.find_element_by_xpath('./span[2]')
            ...
            self.logger.error(f'get_comment: error: {str(err)}')
            raise
    # ========================================================================
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            try:
                ...
                return
            # 4) 작성 시간
            e = e_ab.find_element_by_xpath('.//td[@class="tb_lr minitext"]/div/span[1]')
            ...
            msg['author'] = ""
            # 본문
            e = e_ab.find_element_by_xpath('.//div[@class="memo_content"]|.//div[@class="adsense memo_content"]')
            try:
                sub = self.get_by_xpath('//div[@id="bottom_btnbox"]').text.strip()
            except:
                sub = ''
            contents = e.text.strip()
            msg['contents'] = contents.replace(sub, '')
            # 본문 HTML
            ...
            if inner_html.find('img') > 0:
                for j, sub_e in enumerate(e.find_elements_by_xpath('//p/img')):
            ...
            if msg['num_comments'] == 0:
                # 게시글 스크린샷(신규 댓글 노출이 무한 스크롤이라서 댓글이 있는 경우 전체 수집하고 스크린샷 진행)
            ...
            try:
                ...
                cmt_page_n = 1
                while True:
                    e = self.get_by_xpath('//iframe[@id="ifrm_notify"]/..//div[@id="ajax_comment"]//tbody//table[@id="indextable"]')
                    cmt_ple = e.find_element_by_xpath('./tbody//tr')
                    # e = cmt_ple.find_element_by_xpath('./td[@class="indexing1"]')
                    # is_on = int(e.text.strip())
                    # self.move_to_element(cmt_ple)
                    now_page = False
                    ...
                    for cmt_p in cmt_ps:
                        if cmt_p.text == '◀ 이전':
                            ...
                            self.implicitly_wait(after_wait=1)
                            break
                            # 댓글이 실시간 업데이트 되면 num_comment에 반영 안 될 때가 있어서 초과함
    ...
    def remove_html(self):
        ...
        try:
            # 게시글 목록 댓글 수 제거
            try:
                top_area = self.get_by_xpath('//span[@class="cmt2"]')
        ...
        try:
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    v = a_url.rpartition('name/')[2].partition('?')[0]
                    msg['article_id'] = v
                    ...
                    # 4) 좋아요
                    e = ea.find_element_by_xpath('./td[@class="listno"][2]')
                    msg['like'] = int(e.text.strip())
                    # 5) 게시판명
                    for e_a in ea.find_elements_by_xpath('./td[1]/a'):
                        a = e_a.text.strip()
                        msg['board_name'] = str(a)
                    # 6) 제목: title
                    # 제목 옆 댓글 수 제거
                    try:
                        ...
                        self.logger.error(f'제목 옆 댓글 수 없음')
...
def do_start(**kwargs):
    try:
        with Instiz01(kwargs['config_f']) as ws:
            ws.start()
    except Exception as err:
        return 1
