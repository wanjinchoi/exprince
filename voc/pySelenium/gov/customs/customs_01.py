...
Description
===========
ARGOS LABS Web scraping module for customs (관세청)
"""
# Authors
# ===========
#
...
from alabslib.selenium import PySelenium, ActionChains
...
################################################################################
class CustomsSearch(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'CustomsSearch.log'),
        ...
        self.logger.info(f'Starting Customs Crawling... with '
                         f'config:\n{out_config}')
    ...
    def get_article(self, title_e, msg, ndx):
        ...
        try:
            ...
            msg['contents'] = ''
            try:
                contents = self.get_by_xpath('//*[@id="nttViewForm"]/div[1]/table/tbody/tr[3]/td')
            except:
                msg['contents'] = ''
            else:
            ...
            msg['attachment_url'] = []
            try:
                e = self.get_by_xpath('//*[@id="nttViewForm"]//ul[@class="list_file"]')
    ...
    def get_page(self):
        try:
            ...
            # 검색 결과물 페이지 읽기
            e = self.get_by_xpath('//*[@id="sub_content"]/div[2]/div[2]/table/tbody')
            ...
            for i in range(len(bil)):
                ...
                try:
                    ...
                    # 게시글 제목
                    try:
                        title = bi.find_element_by_xpath('./td[@data-table="subject"]')
                    except:
                        msg['title'] = ''
                    else:
                        if title.text.strip() == '':
                    ...
                    # 게시글 id
                    try:
                        article_id = bi.find_element_by_xpath('./td[@data-table="subject"]/a')
                    except:
                        msg['article_id'] = ''
                    else:
                        if article_id.get_attribute('data-id') == '':
                            msg['article_id'] = ''
                        else:
                            msg['article_id'] = article_id.get_attribute('data-id')
                    ...
                    # 등록일
                    try:
                        create_ts = bi.find_element_by_xpath('./td[@data-table="date"]')
                    except:
                        ...
                        if create_ts.text.strip() == '':
                            ...
                            msg['create_ts'] = create_ts.text.strip() + ' 00:00:00'
                    ...
                    # 게시글 본문으로 이동
                    title_e= bi.find_element_by_xpath('./td[@data-table="subject"]/a')
    ...
    def next_page(self):
        try:
            ...
            try:
                ...
                pa_list = ple.find_elements_by_xpath('./a | ./span')
            except:
                # 페이지가 없는 경우
                self.is_done = True
                return
            ...
            for pa in pa_list:
                ...
                if is_current:
                    ...
                    for npa in nple.find_elements_by_xpath('./a | ./span'):
    ...
    def start(self):
        try:
            ...
            # 검색 대상
            # 1. 알림/소식 > 공지사항 > 보도자료
            # 2. 알림/소식 > 공고/공시 > 행정규칙 행정예고

            # 1. 알리/소식 > 공지 사항 > 보도자료로 이동

            # action = ActionChains(self.driver)
            # first_menu = self.get_by_xpath('//*[@id="gnb"]/ul/li[3]')
            # action.move_to_element(first_menu).perform()
            #
            # second_level_menu = self.get_by_xpath('//*[@id="gnb"]/ul/li[3]/ul/li[1]/a')
            # self.safe_click(second_level_menu)
            # self.implicitly_wait(after_wait=1)
            #
            # third_level_menu = self.get_by_xpath('//*[@id="gnb"]/ul/li[3]/ul/li[1]/ul/li[2]/a')
            ...
            # action.move_to_element(first_menu).perform()
            #
            # second_level_menu = self.get_by_xpath('//*[@id="gnb"]/ul/li[3]/ul/li[2]/a')
            # self.safe_click(second_level_menu)
            # self.implicitly_wait(after_wait=1)
            #
            # third_level_menu = self.get_by_xpath('//*[@id="gnb"]/ul/li[3]/ul/li[2]/ul/li[3]/a')
            # self.safe_click(third_level_menu)
            # self.implicitly_wait(after_wait=1)
...
################################################################################
def do_start(**kwargs):
    with CustomsSearch(kwargs['config_f']) as ws:
        ws.start()


################################################################################
if __name__ == '__main__':
    _config_f = 'customs_01.yaml'
