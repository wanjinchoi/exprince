...
################################################################################
class FoodInfoSearch(PySelenium):
    # ==========================================================================
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'FoodInfoSearch.log'),
        ...
        self.logger.info(f'Starting FoodInfo Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        # 검색 단추
        e = self.get_by_xpath('//input[@title="검색"]',
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            # msg['author'] = ""
            # 2) 조회수: view_count
            e = e_ab.find_element_by_xpath('./tr[2]/td[@colspan][2]')
            msg['view_count'] = int(e.text.strip())
            # 3) 게시글 내용 :
            e = e_ab.find_element_by_xpath('./tr[3]/td')
            ...
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('attach_file') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//a[@class="attach_file"]')):
                    self.move_to_element(sub_k)
                    sub_k_url = sub_k.get_attribute('onclick')
            ...
            # 이전 페이지
            if len(msg['attachment_url']) == 0:
                self.driver.back()
            else:
                self.driver.back()
                self.driver.back()
                self.implicitly_wait(after_wait=1)
            e_bt = self.get_by_xpath('//div[@id="idNavi"]')
            e_bt_s = e_bt.find_elements_by_xpath('.//a')
            for e_tt in e_bt_s:
    ...
    def get_page(self):
        try:
            ...
            # 페이지 테이블 구해오기
            e = self.get_by_xpath('//div[@class="bbs_list"]//tbody')
            es = e.find_elements_by_xpath('./tr/td[1]/..')
    ...
    def next_page(self):
        try:
            self.switch_to_window(0)
            # 페이지 목록
            e = self.get_by_xpath('//form[@name="frmMain"]')
            ple = e.find_element_by_xpath('.//div[@id="idNavi"]')
            e = ple.find_element_by_xpath('.//a[@title="선택됨"]')
            is_on = int(e.text.strip())
            self.move_to_element(ple)
            for pa, sub_pa in enumerate(ple.find_elements_by_xpath('.//a')):
...
################################################################################
def do_start(**kwargs):
    with FoodInfoSearch(kwargs['config_f']) as ws:
        ws.start()
        return 0
