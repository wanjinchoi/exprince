"""
====================================
 :mod:`gov/ food_safety_korea_02` : 국내 뉴스
====================================
...
################################################################################
class FoodSafetyKorea_02(PySelenium):
    # ==========================================================================
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'FoodSafetyKorea_02.log'),
        ...
        self.logger.info(f'Starting FoodSafetyKorea_News Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        # 검색어 입력
        e = self.get_by_xpath('//input[@id="search_keyword"]')
        ...
        try:
            ...
            if inner_html.find('file-container') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//div[@class="file-container"]//td[3]/a')):
                    ...
                    sub_k_url = sub_k.get_attribute('href').strip()
        ...
        try:
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    e = self.driver.find_elements_by_xpath('//div[@class="pagination compact-theme simple-pagination"]/ul/li[@class]')
                    if e[-1].text != str(self.cur_page):
                        page_ok = False
                        while not page_ok:
                            pages_e = self.driver.find_elements_by_xpath('//div[@class="pagination compact-theme simple-pagination"]/ul/li')
                            for p_e in pages_e:
                                if p_e.text == str(self.cur_page):
                                    self.safe_click(p_e)
                                    self.implicitly_wait(after_wait=5)
                                    page_ok = True
                                    break
...
################################################################################
def do_start(**kwargs):
    with FoodSafetyKorea_02(kwargs['config_f']) as ws:
        ws.start()
        return 0
...
if __name__ == '__main__':
    _config_f = 'food_safety_korea_02.yaml'
