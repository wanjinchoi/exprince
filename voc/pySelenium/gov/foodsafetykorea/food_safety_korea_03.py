"""
====================================
 :mod:`gov/ food_safety_korea_03` : 행정 처분
====================================
...
################################################################################
class FoodSafetyKorea_03(PySelenium):
    # ==========================================================================
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'FoodSafetyKorea_03.log'),
        ...
        self.logger.info(f'Starting FoodSafetyKorea_03 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        # 검색어 입력
        e = self.get_by_xpath('//fieldset/input[@id="search_keyword"]')
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            # 0) 대표자 :
            # e = e_ab.find_element_by_xpath('.//dl/dd[2]')
            # msg['author'] = e.text.strip()

            # 1) 게시글 내용 :
            e = e_ab.find_element_by_xpath('./div')
        ...
        try:
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    # 3) 제목: title
                    e_t = ea.find_element_by_xpath('.//li[1]')
                    msg['title'] = e_t.text.strip()
                    # 4) 소제목
                    # e = ea.find_element_by_xpath('.//li[2]')
                    # msg['category'] = e.text.strip()
                    # 5) 작성 시간
                    e = ea.find_element_by_xpath('.//li[4]//dd')
...
################################################################################
def do_start(**kwargs):
    with FoodSafetyKorea_03(kwargs['config_f']) as ws:
        ws.start()
        return 0
...
if __name__ == '__main__':
    _config_f = 'food_safety_korea_03.yaml'
