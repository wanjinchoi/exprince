...
################################################################################
class FoodSafetyKorea_04(PySelenium):
    # ==========================================================================
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'FoodSafetyKorea_04.log'),
        ...
        self.logger.info(f'Starting FoodSafetyKorea_04 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        # 검색어 입력
        e = self.get_by_xpath('//input[@id="searchInpText"]')
        self.send_keys(e, self.config['params']['site']['search'])

        # 검색 단추
        e = self.get_by_xpath('//button[@id="btnSearch"]',
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//div[@class="gallery type2 txt-type long-title"]/ul', timeout=1)

            # 1) 게시글 내용 :
            e = e_ab.find_element_by_xpath('.//dl//dd/ul')
    ...
    def get_page(self):
        try:
            ...
            # 페이지 테이블 구해오기
            e = self.get_by_xpath('//div[@class="gallery type2 txt-type long-title"]/ul')
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    # 1) 제목: title 한글명_영문명
                    e = ea.find_element_by_xpath('./h4/strong')
                    e_k = e.text.strip()
                    e = ea.find_element_by_xpath('./h4/span')
                    e_s = e.text.strip()
                    msg['title'] = e_k + '_' + e_s
                    # 2) 게시글 id : article_id
                    msg['article_id'] = str(msg['page']) + '_' + str(i+1)
                    # 3) 작성자 : 제조 국가_수출 회사
                    e = ea.find_element_by_xpath('.//dt/ul/li[4]/span')
                    e_a = e.text.strip()
                    e = ea.find_element_by_xpath('.//dt/ul/li[3]/span')
                    e_v = e.text.strip()
                    # msg['author'] = e_v + '_' + e_a
                    # 4) 제품 구분
                    # e = ea.find_element_by_xpath('.//dt/ul/li[1]/span')
                    # msg['sortation'] = e.text.strip()
                    # # 5) 품목명
                    # e = ea.find_element_by_xpath('.//dt/ul/li[2]/span')
                    # msg['item'] = e.text.strip()
                    # 6) 작성 시간
                    e = ea.find_element_by_xpath('.//dd/ul/li[3]/span')
    ...
    def next_page(self):
        try:
            self.switch_to_window(0)
            # 페이지 목록
            e = self.get_by_xpath('//section[@id="content"]')
            ple = e.find_element_by_xpath('.//p[@class="page_navi"]')
            self.move_to_element(ple)
            for pa in ple.find_elements_by_xpath('.//button'):
                if pa.get_attribute('class') == 'page_next':
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=1)
                    return
            self.is_done = True
...
################################################################################
def do_start(**kwargs):
    with FoodSafetyKorea_04(kwargs['config_f']) as ws:
        ws.start()
        return 0
...
if __name__ == '__main__':
    _config_f = 'food_safety_korea_04.yaml'
