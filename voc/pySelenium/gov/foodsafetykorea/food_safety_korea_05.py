"""
====================================
 :mod:`gov/ food_safety_korea_05` : 국내식품 부적합
====================================
...
################################################################################
class FoodSafetyKorea_05(PySelenium):
    # ==========================================================================
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'FoodSafetyKorea_05.log'),
        ...
        self.logger.info(f'Starting FoodSafetyKorea_05 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        # 검색어 입력
        e = self.get_by_xpath('//fieldset/input[@id="search_keyword01"]')
        self.send_keys(e, self.config['params']['site']['search'])

        # 검색 단추
        e = self.get_by_xpath('//fieldset/a[@class="btn btn-default"]',
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            if self.config['params']['site']['capture_article']:
                # save capture
                # e_body = self.get_by_xpath('//div[@class="fancybox-skin"]')
            ...
            e_ab = self.get_by_xpath('//div[@class="inner-body"]', timeout=1)

            # 1) 게시글 내용 :
            e = e_ab.find_element_by_xpath('.//div[@class="issue-info"]')
            ...
            if inner_html.find('img') > 0:
                for j, sub_e in enumerate(e_ab.find_elements_by_tag_name('img')):
                    self.move_to_element(sub_e)
                    sub_e_url = sub_e.get_attribute('src')
                    if sub_e_url not in msg['image_url_list']:
            ...
            # 이전 페이지
            e = self.get_by_xpath('//a[@class="fancybox-item fancybox-close"]')
            self.safe_click(e)
            self.implicitly_wait(after_wait=1)
            try:
        ...
        try:
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    e = ea.find_element_by_xpath('./td[@class="num"]')
                    msg['article_number'] = e.text.strip()
                    # 1) 게시글 id : article_id
                    e = ea.find_element_by_xpath('./td[@style="text-align:left;"]/a')
                    e_url = e.get_attribute('id')
                    a_url = e.get_attribute('href')
                    msg['article_id'] = e_url
                    # 2) 게시글 주소: article_url
                    msg['article_url'] = a_url
                    # 3) 제목: title
                    e_t = ea.find_element_by_xpath('./td[@style="text-align:left;"]/a')
                    msg['title'] = e_t.text.strip()
                    # 4) 작성자
                    # e = ea.find_element_by_xpath('./td[7]')
                    # msg['author'] = e.text.strip()
                    # # 5) 대분류
                    # e = ea.find_element_by_xpath('./td[4]')
                    # msg['L_sortation'] = e.text.strip()
                    # # 6) 중분류
                    # e = ea.find_element_by_xpath('./td[5]')
                    # msg['M_sortation'] = e.text.strip()
                    # # 7) 시험 항목
                    # e = ea.find_element_by_xpath('./td[6]')
                    # msg['test_category'] = e.text.strip()
                    # 8) 작성 시간
                    e = ea.find_element_by_xpath('./td[13]')
    ...
    def next_page(self):
        try:
            self.switch_to_window(0)
            # 페이지 목록
            e = self.get_by_xpath('//div[@class="list-container"]')
...
################################################################################
def do_start(**kwargs):
    with FoodSafetyKorea_05(kwargs['config_f']) as ws:
        ws.start()
        return 0
...
if __name__ == '__main__':
    _config_f = 'food_safety_korea_05.yaml'
