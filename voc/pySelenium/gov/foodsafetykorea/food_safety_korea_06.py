"""
====================================
 :mod:`gov/ food_safety_korea_06` : 회수, 판매중지
====================================
...
################################################################################
class FoodSafetyKorea_06(PySelenium):
    # ==========================================================================
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'FoodSafetyKorea_06.log'),
        ...
        self.logger.info(f'Starting FoodSafetyKorea_06 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        # 검색어 입력
        e = self.get_by_xpath('//input[@title="검색어를 입력하세요"]')
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            # 1) 게시글 내용 :
            e = e_ab.find_element_by_xpath('.//div[@class="issue-info"]/dl')
    ...
    def get_page(self):
        try:
            ...
            # 페이지 테이블 구해오기
            e = self.get_by_xpath('//div[@id="listFrame"]')
            es = e.find_elements_by_xpath('./a')
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    # 1) 카테고리 : Category
                    # e = ea.find_element_by_xpath('.//li[@class="datetxt"][3]')
                    # msg['category'] = e.text.partition('식품분류 :')[2].strip()
                    # 2) 게시글 id : article_id
                    e_url = ea.get_attribute('id')
                    a_url = ea.get_attribute('href')
                    msg['article_id'] = e_url
                    # 3) 게시글 주소: article_url
                    msg['article_url'] = a_url
                    # 4) 제목: title
                    e = ea.find_element_by_xpath('.//li[@class="issue"]')
                    msg['title'] = e.text.strip()
                    # 4) 작성 시간
                    e = ea.find_element_by_xpath('.//li[@class="datetxt"][2]')
                    msg['create_ts'] = e.text.partition('등록일자 :')[2].strip() + ' 00:00:00'
                    # 5) 작성자
                    e_t = ea.find_element_by_xpath('.//li[@class="product"]')
                    # msg['author'] = e_t.text.strip()
...
################################################################################
def do_start(**kwargs):
    with FoodSafetyKorea_06(kwargs['config_f']) as ws:
        ws.start()
        return 0
...
if __name__ == '__main__':
    _config_f = 'food_safety_korea_06.yaml'
