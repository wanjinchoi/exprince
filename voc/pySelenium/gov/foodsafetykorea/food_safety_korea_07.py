"""
====================================
 :mod:`gov/ food_safety_korea_07` : 수입 신고 및 검사 : 검사지시
====================================
...
################################################################################
class FoodSafetyKorea_07(PySelenium):
    # ==========================================================================
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'FoodSafetyKorea_07.log'),
        ...
        self.logger.info(f'Starting FoodSafetyKorea_07 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        # # 식품 안전 목록 클릭
        # e = self.get_by_xpath('//ul[@class="depth_list depth1_list cut"]/li[1]')
        # self.safe_click(e)
        # # 수입 식품, 수입 신고 및 검사 클릭
        # e_a = e.find_element_by_xpath('.//div[@class="depth2_content"]/ul/li[3]/ul/li[2]')
        # self.safe_click(e_a)
        # 검사 지시 클릭
        e = self.get_by_xpath('//div[@class="cat-container"]/ul/li[3]')
        self.safe_click(e)
        self.implicitly_wait(after_wait=1)
        #
        # # 검색어 입력
        # e = self.get_by_xpath('//input[@title="검색어"]')
        # self.send_keys(e, self.config['params']['site']['search'])
        #
        # # 검색 필터
        # if self.config['params']['site']['search_filter'] in ['내용', '제목']:
        #     e = self.get_by_xpath('//select[@title="검색구분 선택"]')
        ...
        #
        # # 검색 단추
        # e = self.get_by_xpath('//div[@class="search-wrap"]/a',
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            if self.config['params']['site']['capture_article']:
                ...
                e_heads = self.driver.find_elements_by_xpath('//div[@id="lnb"]|'
                                                             '//div[@class="total_menu"]|'
                                                             '//div[@class="sub_chatbot_wrap"]'
                                                             )
            ...
            e_ab = self.get_by_xpath('//div[@class="view-container"]', timeout=1)

            # 0) 추천 : Like
            e = e_ab.find_element_by_xpath('.//div[@class="like"]/a/span[2]')
            msg['like'] = int(e.text.strip())

            # 1) 게시글 내용 :
            e = e_ab.find_element_by_xpath('.//div[@class="post"]')
            ...
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('file-container') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//div[@class="file-container"]//td[3]/a')):
                    self.move_to_element(sub_k)
                    sub_k_url = sub_k.get_attribute('href')
                    msg['attachment_url'].append(sub_k_url)
                    sub_k_n = sub_k.find_element_by_xpath('./../../td[1]/a')
                    sub_k_name = sub_k_n.text.strip().replace(' ', '_')
                    # 다운로드폴더에 다운 받음
            ...
            self.switch_to_window(0)
            # e = self.get_by_xpath('//div[@class="cat-container"]/ul/li[3]')
            e = self.get_by_xpath('//a[@id="div_ctgType01_2"]')
            self.safe_click(e)
            self.implicitly_wait(after_wait=1)
            # 같은 페이지 찾아서 클릭
            e_bt = self.get_by_xpath('//div[@class="board-footer"]/div')
            e_bt_s = e_bt.find_elements_by_xpath('.//span[@class="current"]|.//a')
            for e_tt in e_bt_s:
                # if str(msg['page']) != e_b.text:
                if e_tt.text == str(msg['page']):
                    self.safe_click(e_tt)
                    self.implicitly_wait(after_wait=1)
                    return
                else:
                    continue
    ...
    def get_page(self):
        try:
            ...
            # 페이지 테이블 구해오기
            e = self.get_by_xpath('//tbody[@id="listFrame"]')
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    e = ea.find_element_by_xpath('./td[1]/span[2]')
                    msg['article_number'] = e.text.strip()
                    # 1) 카테고리 : Category
                    # e = ea.find_element_by_xpath('./td[2]/span[2]')
                    # msg['category'] = e.text.strip()
                    # 2) 게시글 id : article_id
                    e_url = ea.find_element_by_xpath('./td[3]/span[2]/a')
                    a_url = e_url.get_attribute('href')
                    v = a_url.partition("('")[2].partition("'")[0]
                    msg['article_id'] = v
                    # 3) 제목: title
                    e_t = ea.find_element_by_xpath('./td[3]/span[2]/a')
                    msg['title'] = e_t.text.strip()
                    # 4) 작성 시간
                    e = ea.find_element_by_xpath('./td[5]/span[2]')
                    msg['create_ts'] = e.text.strip() + ' 00:00:00'
                    # 5) 작성자
                    e = ea.find_element_by_xpath('./td[4]/span[2]')
                    msg['author'] = e.text.strip()
                    # 6) 조회수: view_count
                    e = ea.find_element_by_xpath('./td[6]/span[2]')
    ...
    def next_page(self):
        try:
            self.switch_to_window(0)
            # 페이지 목록
            e = self.get_by_xpath('//section')
            ple = e.find_element_by_xpath('.//div[@class="board-footer"]/div')
            self.move_to_element(ple)
            for pa in ple.find_elements_by_xpath('.//a'):
                if pa.get_attribute('class') == 'page-link next':
                    self.safe_click(pa)
...
################################################################################
def do_start(**kwargs):
    with FoodSafetyKorea_07(kwargs['config_f']) as ws:
        ws.start()
        return 0
...
if __name__ == '__main__':
    _config_f = 'food_safety_korea_07.yaml'
