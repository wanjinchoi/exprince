...
class Gjcity01(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Gjcity01.log'),
        ...
        self.logger.info(f'Starting Gjcity01 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//div[@class="bod_view"]', timeout=1)

            # 5) 작성 시간
            e = e_ab.find_element_by_xpath('.//li[@class="view_date"]')
            msg['create_ts'] = e.get_attribute('innerHTML').split(' ')[1].replace('-','.') + ' 00:00:00'
            # 6) 조회수
            e = e_ab.find_element_by_xpath('.//li[@class="view_hit"]')
            msg['view_count'] = int(e.get_attribute('innerHTML').split(' ')[1])
            ...
            if inner_html.find('file_list') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//dl[@class="view_file clFix"]//li/a[1]')):
        ...
        try:
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    # 1) 분류 :
                    e = ea.find_element_by_xpath('./td[@class="list_num"]')
                    # if e.text.strip() == '':
                    #     continue
                    ...
                    # 2) 게시글 id : article_id
                    e_url = ea.find_element_by_xpath('./td[@class="list_tit"]/a')
                    a_url = e_url.get_attribute('onclick')
                    v = a_url.split(',')[1].replace('\'', '')
                    msg['article_id'] = v
                    # 3) 작성자
                    e = ea.find_element_by_xpath('./td[@class="list_part"]')
                    msg['author'] = e.text.strip()
                    # 4) 제목: title
                    e_t = ea.find_element_by_xpath('./td[@class="list_tit"]/a')
    ...
    def next_page(self):
        try:
            ...
            ple = e.find_element_by_xpath('.//div[@class="bod_page"]')
    ...
    def start(self):
        try:
            ...
            # self.search()
            # self.enable_download_headless(self.driver, self.get_download_path())
...
################################################################################
def do_start(**kwargs):
    with Gjcity01(kwargs['config_f']) as ws:
        ws.start()
        return 0
