...
class Gnews01(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Gnews01.log'),
        ...
        self.logger.info(f'Starting Gnews01 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//div[@class="postView_regular"]', timeout=1)

            # 작성시간
            try:
                e = e_ab.find_element_by_xpath('.//div[@class="postinfo"]/span[3]')
                msg['create_ts'] = e.text.replace('  ', ' ')
            except:
                ...
            # 본문
            e = e_ab.find_element_by_xpath('.//div[@class="postBody"]')
            ...
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('fileSet') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//div[@class="fileSet"]//li/a[1]')):
        ...
        try:
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    # 6) 조회수
                    e = ea.find_element_by_xpath('./td[@class="text-center"][5]')
                    msg['view_count'] = int(e.text.strip())
                    # 3) 작성자
                    e = ea.find_element_by_xpath('./td[@class="text-center"][2]')
                    msg['author'] = e.text.strip()
                    # 5) 작성 시간
                    e = ea.find_element_by_xpath('./td[@class="text-center"][4]')
    ...
    def next_page(self):
        try:
            ...
            # 페이지 목록
            e = self.get_by_xpath('//div[@id="contentsWrap"]')
            ple = e.find_element_by_xpath('.//div[@class="pagination"]')
            e = ple.find_element_by_xpath('.//li[@class="on"]')
            is_on = int(e.text.strip())
            self.move_to_element(ple)
            for pa in ple.find_elements_by_xpath('.//li'):
                if pa.get_attribute('class') == 'p_img page-next':
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=1)
                    return
                elif pa.get_attribute('class') in ['p_img page-last', 'p_img page-first', 'p_img page-prev']:
                    continue
...
################################################################################
def do_start(**kwargs):
    with Gnews01(kwargs['config_f']) as ws:
        ws.start()
        return 0
