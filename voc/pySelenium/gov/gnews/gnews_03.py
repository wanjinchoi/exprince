...
class Gnews03(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Gnews03.log'),
        ...
        self.logger.info(f'Starting Gnews03 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//form[@id="otherform"]', timeout=1)

            # 3) 조회수
            e = e_ab.find_element_by_xpath('.//dd[4]')
            msg['view_count'] = int(e.text.partition('조회')[2].strip())
            # 본문
            e = e_ab.find_element_by_xpath('.//div[@class="content"]')
            ...
            if inner_html.find('file') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//dl[@class="file"]//li/a[3]')):
        ...
        try:
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    v = a_url.partition('bIdx=')[2].partition('&')[0]
                    msg['article_id'] = v
                    ...
                    msg['author'] = e.text.partition('담당자')[2].strip()
                    # 4) 작성 시간
        ...
        try:
            ...
            for pa in ple.find_elements_by_xpath('.//a'):
                if pa.get_attribute('title') == '다음 페이지':
                    ...
                    self.implicitly_wait(after_wait=1)
                    return
                elif pa.get_attribute('title') in ['마지막 페이지', '처음 페이지', '이전 페이지']:
                    continue
...
################################################################################
def do_start(**kwargs):
    with Gnews03(kwargs['config_f']) as ws:
        ws.start()
        return 0
