...
class Gnews04(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Gnews04.log'),
        ...
        self.logger.info(f'Starting Gnews04 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            if inner_html.find('file') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//dl[@class="file"]//dd/ul/li/a[@title="다운로드"]')):
...
################################################################################
def do_start(**kwargs):
    with Gnews04(kwargs['config_f']) as ws:
        ws.start()
        return 0
