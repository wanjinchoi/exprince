...
class Gyeongnam02(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Gyeongnam02.log'),
        ...
        self.logger.info(f'Starting Gyeongnam02 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def next_page(self):
        try:
            ...
            for pa in ple.find_elements_by_xpath('.//a'):
                if pa.get_attribute('innerHTML').find('다음 10개의 페이지') > 0:
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=1)
                    return
                elif pa.get_attribute('innerHTML').find('마지막 페이지') > 0:
                    continue
                elif pa.get_attribute('innerHTML').find('첫 페이지') > 0:
                    continue
                elif pa.get_attribute('innerHTML').find('이전 10개의 페이지') > 0:
                    continue
...
################################################################################
def do_start(**kwargs):
    with Gyeongnam02(kwargs['config_f']) as ws:
        ws.start()
        return 0
