...
class Gyeongnam03(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Gyeongnam03.log'),
        ...
        self.logger.info(f'Starting Gyeongnam03 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//div[@class="basicView"]', timeout=1)
            # 3) 작성자
            e = e_ab.find_element_by_xpath('.//div[@class="titleField"]/ul/li[4]/span')
            msg['author'] = e.text.strip()
            # 5) 작성 시간
            e = e_ab.find_element_by_xpath('.//div[@class="titleField"]/ul/li[2]/span')
    ...
    def next_page(self):
        try:
            ...
            for pa in ple.find_elements_by_xpath('.//a'):
                if pa.get_attribute('innerHTML').find('다음 5개의 페이지') > 0:
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=1)
                    return
                    ...
                    continue
                elif pa.get_attribute('innerHTML').find('이전 5개의 페이지') > 0:
                    continue
...
################################################################################
def do_start(**kwargs):
    with Gyeongnam03(kwargs['config_f']) as ws:
        ws.start()
        return 0
