"""
====================================
 :mod:`gov/ Gyeongnam_02 : 보도&해명설명 자료
====================================
...
# --------
#  * [2023/05/30]
#     - article_id 중복 발생으로 게시글 넘버 합침
#  * [2023/05/11]
#     - 사이트 오류로 인해 다음 페이지 넘어가는 부분에 refresh 추가
#  * [2023/04/27]
#     - 사이트 오류로 인해 refresh 로직 추가
#  * [2023/04/24]
#     - 스크린샷 상단바 제거
#  * [2023/02/06]
#     - 게시글 조회수 주석 처리
...
class Gyeongnam04(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Gyeongnam04.log'),
        ...
        self.logger.info(f'Starting Gyeongnam04 Crawaling... with '
                         f'config:\n{out_config}')
        ...
        try:
            ...
            if self.config['params']['site']['capture_article']:
                ...
                if self.config['params']['kwargs']['headless']:
                    ...
                    e_headers = self.driver.find_elements_by_xpath('//header[@id="header"]')
            ...
            e_ab = self.get_by_xpath('//div[@class="board"]', timeout=1)
            # 3) 작성자
            e = e_ab.find_element_by_xpath('.//div[@class="conField"]/ul/li[3]/p')
            msg['author'] = e.text.strip()
            # 본문
            e = e_ab.find_element_by_xpath('.//div[@class="conText"]')
            ...
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('attach_1') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//a[@class="file"]')):
        ...
        try:
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    e_number = e.text.strip()
                    msg['article_number'] = e_number
                    # 2) 게시글 id : article_id
                    e_url = ea.find_element_by_xpath('./td[@class="title"]/a')
                    a_url = e_url.get_attribute('href')
                    v = a_url.rpartition('no=')[2].partition('&')[0]
                    msg['article_id'] = e_number + '_' + v

                    # 주소
                    msg['article_url'] = a_url

                    # 5) 작성 시간
                    e = ea.find_element_by_xpath('./td[@class="date"]')
                    msg['create_ts'] = e.text.strip().replace('-', '.') + ' 00:00:00'
                    # 6) 조회수
                    # e = ea.find_element_by_xpath('./td[@class="hit"]')
                    # msg['view_count'] = int(e.text.strip())
                    # 4) 제목: title
                    e_t = ea.find_element_by_xpath('./td[@class="title"]/a')
    ...
    def next_page(self):
        try:
            ...
            # 페이지 목록
            e = self.get_by_xpath('//div[@id="subCnt"]')
            ...
            for pa in ple.find_elements_by_xpath('.//a'):
                if pa.get_attribute('innerHTML').find('next 페이지') > 0:
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=1)
                    return
                elif pa.get_attribute('innerHTML').find('끝 페이지') > 0:
                    continue
                elif pa.get_attribute('innerHTML').find('처음 페이지') > 0:
                    continue
                elif pa.get_attribute('innerHTML').find('이전 페이지') > 0:
                    continue
    ...
    def start(self):
        try:
            ...
            print(self.output['latest_create_article_'
                              'ts'])
...
################################################################################
def do_start(**kwargs):
    with Gyeongnam04(kwargs['config_f']) as ws:
        ws.start()
        return 0
