...
class IncheonSearch(PySelenium):
    ...
    def get_article(self, title_e, msg, ndx):
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f'*** ERROR **** : Page[{self.cur_page}:{ndx}], title="{msg["title"]}"')
                e = self.get_by_xpath('//div[@class="board-view board-item-group-wrap"]')
                pass

            # 내용
            try:
                contents = e.find_element_by_xpath('.//div[@class="board-view-contents"]')
            ...
            try:
                ...
                for group in se:
                    ...
                    if '공공누리' in group.text.strip() :
                        ...
                        try:
                            afs = group.find_elements_by_xpath('.//div[@class="file-preview-down-group"]')
                        except:
                            pass
                        else:
                            for af in afs:

                                attachment_name = af.find_element_by_xpath('./span[@class="file-name"]').text.strip()
                                attachment_url = af.find_element_by_xpath('./a[@class="btn btn-small btn-download"]').get_attribute('href')
    ...
    def get_page(self):
        try:
            ...
            # 검색 결과물 페이지 읽기
            e = self.get_by_xpath('//div[@class="board-data-list board-has-sm-view"]/table/tbody')
            ...
            for i in range(len(bil)):
                ...
                try:
                    ...
                    bi = ail[i]
                    # 공지
                    if bi.get_attribute('class') == 'notice':
                        continue

                    # 게시글 제목
                    try:
                        title = bi.find_element_by_xpath('.//span[@class="subject"]')
                    except:
                        msg['title'] = ''
                    else:
                        if title.text.strip() == '':
                    ...
                    if msg['article_url'] == '':
                        ...
                        msg['article_id'] = re.search(r'nttNo=(\d+)&', msg['article_url']).group(1)

                    try:
                        tds = bi.find_elements_by_tag_name('td')
                    except:
                        ...
                        # 작성자
                        msg['author'] = tds[3].text.strip()

                        # 등록일
                        msg['create_ts'] = tds[4].text.strip().replace('-', '.') + ' 00:00:00'

                        # 조회수
                        msg['view_count'] = int(tds[5].text.strip())
