...
class IncheonSearch(PySelenium):
    ...
    def get_article(self, title_e, msg, ndx):
        ...
        try:
            ...
            msg['attachment_url'] = []
            try:
                e = self.get_by_xpath('.//div[@class="file-preview-down-group"]')
            except:
                pass
            else:
                attachment_name = e.find_element_by_xpath('.//span').text.strip()
                attachment_url = e.find_element_by_xpath('.//a[@class="btn btn-small"]').get_attribute('href')
    ...
    def get_page(self):
        try:
            ...
            # 검색 결과물 페이지 읽기
            e = self.get_by_xpath('//div[@class="board-data-list board-has-sm-view"]//tbody')
            ...
            for i in range(len(bil)):
                ...
                try:
                    ...
                    # 게시글 URL
                    try:
                        url = items[1].find_element_by_xpath('./a')
                    except:
                        msg['article_url'] = ''
                    else:
                    ...
                    if msg['article_url'] == '':
                        ...
                        msg['article_id'] = re.search(r'\/(\d+)$', msg['article_url']).group(1)

                    # 게시글 본문으로 이동
                    title_e = items[1].find_element_by_xpath('./a')
    ...
    def start(self):
        try:
            ...
            # 인천시보 클릭
            e = self.get_by_xpath('//*[@id="snb"]/div/div[2]/nav/ul/li[4]/div/ul/li[2]/a')
            self.safe_click(e)
            self.implicitly_wait(after_wait=1)
