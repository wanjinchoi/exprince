...
class IncheonSearch(PySelenium):
    ...
    def get_article(self, title_e, msg, ndx):
        ...
        try:
            ...
            # 게시글 URL: 각 게시글 별로 url을 가져와도 해당 게시글로 바로 접속 불가능함.목록 url만 추출됨
            msg['article_url'] = self.driver.current_url.replace('view', '')

            # 작성자
            try:
                e = self.get_by_xpath('//div[@class="board-item-group items2"]/dl[2]')
                author = e.find_element_by_xpath('./dd')
            except:
                msg['author'] = ''
            else:
            ...
            # 내용
            try:
                contents = self.get_by_xpath('//div[@class="board-view-contents"]')
            except:
                msg['contents'] = ''
            else:
            ...
            try:
                ...
                afs = e.find_elements_by_xpath('.//div[@class="file-preview-down-group"]')
            except:
                pass
            else:
                for af in afs:
    ...
    def get_page(self):
        try:
            ...
            # 검색 결과물 페이지 읽기
            e = self.get_by_xpath('//div[@class="board-data-list"]//tbody')
            ...
            for i in range(len(bil)):
                ...
                try:
                    ...
                    # 게시글 제목
                    try:
                        title = items[4].find_element_by_xpath('./a')
                    except:
                        msg['title'] = ''
                    else:
                        if title.text.strip() == '':
                    ...
                    if msg['title'] == '':
                        ...
                        msg['article_id'] = re.search(r'f_goView\((\d+)\);', article_id).group(1)

                    # 등록일
                    try:
                        create_ts = items[1].text.strip()
                    except:
    ...
    def start(self):
        try:
            ...
            # 입법예고 클릭
            e = self.get_by_xpath('//*[@id="snb"]/div/div[2]/nav/ul/li[4]/div/ul/li[4]/a')
            self.safe_click(e)
            self.implicitly_wait(after_wait=1)
