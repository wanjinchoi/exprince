...
class Jeju01(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Jeju01.log'),
        ...
        self.logger.info(f'Starting Jeju01 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//table[@class="table table-view table-board table-bordered table-nocap"]', timeout=1)

            # 날짜
            try:
                e = e_ab.find_element_by_xpath('./tbody/tr[2]/td[1]')
                msg['create_ts'] = datetime.datetime.strptime(e.text, '%Y-%m-%d %H:%M:%S').strftime('%Y.%m.%d %H:%M:%S')
            except:
                ...

            # 본문
            e = e_ab.find_element_by_xpath('.//td[@class="article-contents left"]')
            ...
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('btn btn-sm btn-default') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//a[@class="btn btn-sm btn-default"]/../a[1]')):
    ...
    def next_page(self):
        try:
            ...
            # 페이지 목록
            e = self.get_by_xpath('//div[@class="defaultBoard"]')
            ple = e.find_element_by_xpath('.//div[@class="paging-container"]')
...
################################################################################
def do_start(**kwargs):
    with Jeju01(kwargs['config_f']) as ws:
        ws.start()
        return 0
