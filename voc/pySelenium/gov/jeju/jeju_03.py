...
class Jeju03(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Jeju03.log'),
        ...
        self.logger.info(f'Starting Jeju03 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//div[@class="module-wrapper mw-m"]', timeout=1)

            # 3) 조회수
            e = e_ab.find_element_by_xpath('.//div[@class="news-info"]/div[@class="info-line"][1]/p[2]')
            msg['view_count'] = int(e.text.partition('|')[2].strip())
            # 3) 작성자
            e = e_ab.find_element_by_xpath('.//div[@class="news-info"]/div[@class="info-line"][1]/p[1]')
            msg['author'] = e.text.partition('|')[2].strip()
            # 5) 작성 시간
            e = e_ab.find_element_by_xpath('.//div[@class="news-info"]/div[@class="info-line"][1]/p[3]')
            msg['create_ts'] = e.text.partition('|')[2].replace('-', '.').strip()
            # 본문
            e = e_ab.find_element_by_xpath('.//div[@class="article-contents"]')
            ...
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('file2') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//p[@class="file2"]/a[1]')):
    ...
    def get_page(self):
        try:
            ...
            self.switch_to_window(1)
            # 페이지 테이블 구해오기
            e = self.get_by_xpath('//ul[@class="list-unstyled newsBoardBox"]')
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    v = a_url.rpartition('seq=')[2]
                    msg['article_id'] = v
                    # 2) 게시글 주소:
                    msg['article_url'] = a_url
                    # 3) 제목: title
                    e_t = ea.find_element_by_xpath('./a/strong')
    ...
    def next_page(self):
        try:
            ...
            # 페이지 목록
            e = self.get_by_xpath('//div[@class="newsBoard"]')
            ple = e.find_element_by_xpath('.//ul[@class="pagination pagination-sm"]')
            e = ple.find_element_by_xpath('.//a[@title="현재페이지"]')
            is_on = int(e.text.strip())
            self.move_to_element(ple)
            for pa in ple.find_elements_by_xpath('.//a'):
                if pa.get_attribute('title') == '다음 페이지':
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=1)
                    return
                elif pa.get_attribute('title') in ['첫 페이지', '이전 페이지', '마지막 페이지']:
                    continue
...
################################################################################
def do_start(**kwargs):
    with Jeju03(kwargs['config_f']) as ws:
        ws.start()
        return 0
