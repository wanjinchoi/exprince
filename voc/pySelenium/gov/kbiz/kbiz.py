...
class KbizSearch(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'KbizSearch.log'),
        ...
        self.logger.info(f'Starting Kbiz Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        # 검색어 입력
        e = self.get_by_xpath('//input[@class="input_s"]')
        self.send_keys(e, self.config['params']['site']['search'])

        # 검색 필터
        if self.config['params']['site']['search_filter'] in ['내용', '제목']:
            # 버튼을 눌러야 패스가 생김
            e = self.get_by_xpath('//div[@class="select_box"]')
            self.safe_click(e)
            e = self.get_by_xpath('//div[@class="select_box on"]')
            e_s = e.find_elements_by_xpath('./div/ul/li')
            for e_f in e_s:
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            if self.config['params']['site']['capture_article']:
                ...
                head_es = self.driver.find_elements_by_xpath('//header')
            ...
            e_a = self.get_by_xpath('//div[@class="detail-body"]', timeout=1)

            # 게시글 내용:
            # 사진 안 갖고 오게 하는 거 한 번 체크해야한다 .,. 미리 보기?
            e = e_a.find_element_by_xpath('.//p[2]')
            ...
            # 첨부파일
            e_ab = self.get_by_xpath('//div[@class="detail-head mt60"]', timeout=1)
            inner_html = e_ab.get_attribute('innerHTML')
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('file-wrap') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//div[@class="file-wrap"]/ul/li/a[1]')):
    ...
    def get_page(self):
        try:
            ...
            # 페이지 테이블 구해오기
            e = self.get_by_xpath('//ul[@class="board-list"]')
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    msg['article_id'] = v
                    # 2) 제목: title
                    e = ea.find_element_by_xpath('.//a//p')
                    msg['title'] = e.text.strip()
                    # 3) 작성일: create_ts 12
                    e = ea.find_element_by_xpath('.//a//em//span[@class="fl date"]')
    ...
    def next_page(self):
        try:
            self.switch_to_window(0)
            # 페이지 목록
            e = self.get_by_xpath('//div[@class="page"]')
            ple = e.find_element_by_xpath('.//ul[@class="pagination"]')
            e = ple.find_element_by_xpath('.//li')
            is_on = int(e.text.strip())
            self.move_to_element(ple)
            # 현재 페이지 번호 구하기
            for pa in ple.find_elements_by_xpath('.//li//a'):
                if pa.get_attribute('class') == 'num active':
...
################################################################################
def do_start(**kwargs):
    with KbizSearch(kwargs['config_f']) as ws:
        ws.start()
        return 0
