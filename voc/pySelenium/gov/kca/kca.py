...
class KcaSearch(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'KcaSearch.log'),
        ...
        self.logger.info(f'Starting Kca Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        # 검색 단추
        e = self.get_by_xpath('//button[@class="btn_big"]',
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            if self.config['params']['site']['capture_article']:
                ...
                head_es = self.driver.find_elements_by_xpath('//div[@id="aside"]|'
                                                             '//div[@id="go_top"]')
            ...
            e_ab = self.get_by_xpath('//table[@class="board_insert"]/tbody', timeout=1)

            # 게시글 내용: 내용이 없음
            # e = e_ab.find_element_by_xpath('.//td[@class="substance"]')
            # msg['comments'] = e.text.strip()
            ...
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('fileDiv1') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//td[@colspan="5"]/div/a[1]')):
    ...
    def get_page(self):
        try:
            ...
            # 페이지 테이블 구해오기
            e = self.get_by_xpath('//table[@class="board m_board"]/tbody')
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    e = ea.find_element_by_xpath('.//td[@class="brd_none b_num"]')
                    msg['article_number'] = e.text.strip()
                    # 1) 게시글 id : article_id  300143_55268553
                    e_url = ea.find_element_by_xpath('.//td[@class="title"]/a')
                    ...
                    # 3) 제목: title
                    e = ea.find_element_by_xpath('.//td[@class="title"]/a')
                    msg['title'] = e.text.strip()
                    # 4) 조회수: view_count 123
                    e = ea.find_element_by_xpath('.//td[@class="b_hit"]')
                    msg['view_count'] = int(e.text.strip())
                    # 5) 작성일: create_ts
                    e = ea.find_element_by_xpath('.//td[@class="b_date"]')
                    msg['create_ts'] = e.text.replace('-', '.') + ' 00:00:00'
                    # 6) 담당 부서 : department
                    e = ea.find_element_by_xpath('.//td[@class="b_write"]')
    ...
    def next_page(self):
        try:
            self.switch_to_window(0)
            # 페이지 목록
            e = self.get_by_xpath('//form[@id="cmsBoardData"]')
            ple = e.find_element_by_xpath('.//div[@class="paginate"]')
            self.move_to_element(ple)
            is_on = False
            for pa in ple.find_elements_by_xpath('.//a'):
                if pa.get_attribute('title') == '선택됨':
                    is_on = True
                    continue
                if is_on or pa.get_attribute('class') == 'bt_next':
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=1)
                    return
            self.is_done = True
    ...
    def save_attachment(self, article):
        for i, attach_url in enumerate(article['attachment_url']):
            ...
            #SSLError 발생해서 varify=False 추가
            file = requests.get(attach_url, stream=True, verify=False)
...
################################################################################
def do_start(**kwargs):
    with KcaSearch(kwargs['config_f']) as ws:
        ws.start()
        return 0
