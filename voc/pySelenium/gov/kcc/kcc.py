...
class KccSearch(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'KccSearch.log'),
        ...
        self.logger.info(f'Starting Kcc Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        # 검색어 입력
        e = self.get_by_xpath('//li[@class="li_cent"]/input')
        self.send_keys(e, self.config['params']['site']['search'])

        # 검색 단추
        e = self.get_by_xpath('//li[@class="li_last"]/input',
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//div[@class="sub_right_con"]/table[1]', timeout=1)
            # # 담당부서
            # e = e_ab.find_element_by_xpath('./tbody/tr[1]/td[@class="txtL"]')
            # msg['department'] = e.text.strip()
            # 작성자
            e = e_ab.find_element_by_xpath('./tbody/tr[2]/td[@class="txtL"][1]')
            msg['author'] = e.text.strip()
            # # 연락처
            # e = e_ab.find_element_by_xpath('./tbody/tr[2]/td[@class="txtL"][2]')
            # msg['contact_number'] = e.text.strip()
            # 게시글 내용:
            e = e_ab.find_element_by_xpath('.//tr/td[@class="table_con"]')
            ...
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('file_download') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//a[@title="파일 다운로드"]')):
    ...
    def get_page(self):
        try:
            ...
            # 페이지 테이블 구해오기
            e = self.get_by_xpath('//div[@class="disN"]')
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    e_url = ea.find_element_by_xpath('.//td[@class="txtL"]/a')
                    msg['article_id'] = e_url.get_attribute('href').rpartition('=')[2]
                    # 2) 제목: title
                    e_t = ea.find_element_by_xpath('.//td[@class="txtL"]')
                    msg['title'] = e_t.text.strip()
                    # 게시글 주소: article_url
                    e = ea.find_element_by_xpath('.//td[@class="txtL"]/a')
    ...
    def next_page(self):
        try:
            self.switch_to_window(0)
            # 페이지 목록
            e = self.get_by_xpath('//div[@class="sub_right_con"]')
            ple = e.find_element_by_xpath('.//div[@class="page"]')
            ...
            for pa in ple.find_elements_by_xpath('.//a'):
                if pa.get_attribute('class') == 'on':
                    is_on = True
                    continue
                if is_on or pa.get_attribute('class') == 'img rts2':
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=1)
                    return
            self.is_done = True
...
################################################################################
def do_start(**kwargs):
    with KccSearch(kwargs['config_f']) as ws:
        ws.start()
        return 0
