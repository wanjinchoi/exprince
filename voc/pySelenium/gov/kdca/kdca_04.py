...
class Kdca04(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Kdca04.log'),
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//div[@id="center"]', timeout=1)

            # 1) 게시글 내용 :
            e = e_ab.find_element_by_xpath('.//div[@id="contentBody"]')
            ...
            if inner_html.find('img') > 0:
                for j, sub_e in enumerate(e.find_elements_by_tag_name('img')):
                    ...
                    if sub_e.get_attribute('alt') in ['연혁', '조문체계도버튼', '위임행정규칙', '규제', '판례', '닫기', '별표',
                                                      '파일 다운로드', '부칙보기', '관련규제버튼', '위임행정규칙버튼', '부칙', '펼침']:
                        continue
            ...
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('pconfile pb30') > 0:
                for k, sub_k in enumerate(
                        e_ab.find_elements_by_xpath('.//ul[@class="pconfile pb30"]//li//a[2]')):
                    self.move_to_element(sub_k)
                    sub_k_url = sub_k.get_attribute('href')
                    msg['attachment_url'].append(sub_k_url)
                    sub_k_n = sub_k.find_element_by_xpath('./../a[1]').text
                    sub_k_name = re.sub('[\/:*?"<>|]', '', sub_k_n) + '.hwp'
                    msg['attachment_name'].append(sub_k_name)

            # 파일 전체 저장
            # 저장 버튼
            e = self.get_by_xpath('//div[@class="r_bx"]/a[@title="저장"]')
            self.safe_click(e)
            self.implicitly_wait(after_wait=1)
            # 부칙 저장 설정 클릭
            e_file = self.get_by_xpath(
            ...
            sub_down = e_file.find_element_by_xpath('.//div[@id="noticeDiv"]//a[@id="aBtnOutPutSave"]')
            self.safe_click(sub_down)
            self.implicitly_wait(after_wait=1)
        ...
        try:
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    # 2) 게시글 id : article_id
                    e_url = ea.find_element_by_xpath('./li[3]/a')
                    a_url = e_url.get_attribute('href')
                    v = a_url.partition('Seq=')[2]
                    msg['article_id'] = v
                    # 4) 제목: title
                    e_t = ea.find_element_by_xpath('./li[3]/a/span')
...
################################################################################
def do_start(**kwargs):
    with Kdca04(kwargs['config_f']) as ws:
        ws.start()
        return 0
