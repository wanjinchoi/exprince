...
class Kdca05(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Kdca.log'),
        ...
        self.logger.info(f'Starting Kdca Crawaling... with '
                         f'config:\n{out_config}')
        ...
        try:
            ...
            if self.config['params']['site']['capture_article']:
                ...
                if self.config['params']['kwargs']['headless']:
                    ...
                    # 캡처시 상단바와 사이드바, 하단에 top으로 올라가는 버튼 삭제 element 삭제
                    e_heads = self.driver.find_elements_by_xpath('//header|'
                                                                 '//aside|'
                                                                 '//a[@class="go_top active"]|'
                                                                 '//a[@class="go_top"]')
            ...
            # e_ab = self.get_by_xpath('//div[@class="tstyle_view bd2"]', timeout=1)
            e_ab = self.get_by_xpath('//div[@class="tstyle_view bd2"]|//div[@class="tstyle_view "]', timeout=1)

            # 본문
            e = e_ab.find_element_by_xpath('.//div[@class="tb_contents"]')
            ...
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('add_file') > 0:
                set1 = set(os.listdir(self.get_download_path()))
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//ul[@class="add_file"]/li//a[1]')):
                ...
                sub_k_names = list(set1 ^ set2)
                msg['attachment_name'] = sub_k_names
                # 파일 이동
                for sub_k_name in sub_k_names:
        ...
        try:
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    v = a_url.partition('_no=')[2].partition('&')[0]
                    msg['article_id'] = v
                    # 3) 작성자
                    e = ea.find_element_by_xpath('./li[3]')
                    msg['author'] = e.text.strip()
                    # 4) 제목: title
                    e_t = ea.find_element_by_xpath('./li[2]/a/span')
                    msg['title'] = e_t.text.strip()
                    self.move_to_element(e_t)
                    # 5) 작성 시간
                    e = ea.find_element_by_xpath('./li[4]')
    ...
    def next_page(self):
        try:
            ...
            # 페이지 목록
            e = self.get_by_xpath('//section[@class="content_body"]')
            ple = e.find_element_by_xpath('.//div[@class="board_pager"]')
            self.move_to_element(ple)
            for pa in ple.find_elements_by_xpath('.//a'):
                if pa.get_attribute('class') == 'pageNext':
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=1)
                    return
            self.is_done = True
...
################################################################################
def do_start(**kwargs):
    with Kdca05(kwargs['config_f']) as ws:
        ws.start()
        return 0
