...
class Mafra01(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Mafra_01.log'),
        ...
        self.logger.info(f'Starting Mafra_01 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        # 검색어 입력
        e = self.get_by_xpath('//dd/input[@id="srchWrd"]')
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            # 0) 작성자 : author
            e = e_ab.find_element_by_xpath('.//dd[@class="name"]')
            msg['author'] = e.text.strip()
            # 1) 부서:
            # e = e_ab.find_element_by_xpath('.//dd[@class="group"]')
            # msg['department'] = e.text.strip()
            # 4) 조회수: view_count
            e = e_ab.find_element_by_xpath('.//dd[@class="hit"]')
...
################################################################################
def do_start(**kwargs):
    with Mafra01(kwargs['config_f']) as ws:
        ws.start()
        return 0
