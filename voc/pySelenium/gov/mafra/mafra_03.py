...
class Mafra03(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Mafra_03.log'),
        ...
        self.logger.info(f'Starting Mafra_03 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        # 검색어 입력
        e = self.get_by_xpath('//dd/input[@id="srchWrd1"]')
        self.send_keys(e, self.config['params']['site']['search'])

        # 검색 단추
        e = self.get_by_xpath('//input[@type="submit"]',
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            if self.config['params']['site']['capture_article']:
                ...
                e_heads = self.driver.find_elements_by_xpath('//div[@id="masterNavigation"]')
            ...
            e_ab = self.get_by_xpath('//div[@class="view"]', timeout=1)

            # 0) 작성자 : author
            e = e_ab.find_element_by_xpath('.//dd[@class="date"]')
            e_pc = e.text.partition(' \u2002')[0]
            e_p = e.text.partition('\u2002')[2].rpartition('\u2002')[0].strip()
            e_p2 = e.text.partition('\u2002')[2].rpartition('\u2002')[2].strip()
            msg['author'] = e_p2 + '_' + e_p
            # 2) 작성 시간
            msg['create_ts'] = e_pc
            # 1) 게시글 내용 :
            e = e_ab.find_element_by_xpath('.//div[@class="view_contents"]')
            ...
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('file_list') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//div[@class="file_list"]/ul/li/a[1]')):
    ...
    def get_page(self):
        try:
            ...
            # 페이지 테이블 구해오기
            e = self.get_by_xpath('//div[@class="list"]//tbody')
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    # 1) 게시글 id : article_id
                    e_url = ea.find_element_by_xpath('./td[2]/p/a')
                    a_url = e_url.get_attribute('href')
                    v = a_url.rpartition('/')[0].rpartition('/')[2]
                    msg['article_id'] = v
                    # 2) 게시글 주소: article_url
                    msg['article_url'] = a_url
                    # 3) 제목: title
                    e = ea.find_element_by_xpath('./td[2]/p/a')
                    msg['title'] = e.text.strip()
                    # 4) 작성 시간
                    e = ea.find_element_by_xpath('.//dd[@class="date"]')
    ...
    def next_page(self):
        try:
            self.switch_to_window(0)
            # 페이지 목록
            e = self.get_by_xpath('//div[@class="_fnctWrap _articleTable"]')
            ple = e.find_element_by_xpath('.//div[@class="_paging"]/div')
            e = ple.find_element_by_xpath('.//strong')
            is_on = int(e.text.strip())
            self.move_to_element(ple)
            for pa in ple.find_elements_by_xpath('.//a'):
                if pa.get_attribute('class') == '_next':
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=1)
                    return
                elif pa.get_attribute('class') == '_last':
                    continue
                elif pa.get_attribute('class') == '_first':
                    continue
                elif pa.get_attribute('class') == '_prev':
                    continue
...
################################################################################
def do_start(**kwargs):
    with Mafra03(kwargs['config_f']) as ws:
        ws.start()
        return 0
