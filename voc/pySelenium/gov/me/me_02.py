...
class Me02(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Me02.log'),
        ...
        self.logger.info(f'Starting Me02 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        # 검색어 입력
        e = self.get_by_xpath('//input[@id="searchValue"]')
        self.send_keys(e, self.config['params']['site']['search'])

        # 검색 필터
        if self.config['params']['site']['search_filter'] in ['내용', '제목', '제목+내용']:
            e = self.get_by_xpath('//select[@id="searchKey"]')
        ...
        # 검색 단추
        e = self.get_by_xpath('//input[@alt="검색"]',
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//div[@id="boardTableWrapper"]', timeout=1)

            # 1) 부서명: Department
            # e = e_ab.find_element_by_xpath('//div[@class="view_info01"]/ul/li[2]/dl/dd')
            # msg['department'] = e.text.strip()
            # 2) 작성 시간
            e = e_ab.find_element_by_xpath('.//div[@class="view_info02"]/ul/li[1]/dl/dd')
            msg['create_ts'] = e.text.replace('-', '.').strip() + ' 00:00:00'
            # 3) 작성자
            e = e_ab.find_element_by_xpath('.//div[@class="view_info01"]/ul/li[1]/dl/dd')
            msg['author'] = e.text.strip()
            # 4) 조회수: view_count
            e = e_ab.find_element_by_xpath('.//div[@class="view_info01"]/ul/li[3]/dl/dd')
            msg['view_count'] = int(e.text.strip().replace(',', ''))

            # 1) 게시글 내용 :
            e = e_ab.find_element_by_xpath('.//div[@class="view_con"]')
            ...
            if inner_html.find('view_file') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//div[@class="view_file"]/ul/li/a[1]')):
                    self.move_to_element(sub_k)
                    sub_k_url = sub_k.get_attribute('href')
                    msg['attachment_url'].append(sub_k_url)
                    sub_k_name = sub_k.text.rpartition('(')[0].strip()
    ...
    def get_page_notice(self):
        try:
            self.cur_page += 1
            self.switch_to_window(1)
            # 페이지 테이블 구해오기
            e = self.get_by_xpath('//table[@class="table_case01"]/tbody')
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    # 0) 게시글 번호 : article_number : 공지는 읽지 않게 하기
                    e = ea.find_element_by_xpath('./td[1]/span')
                    # if e.text != '':
                    #     continue
                    # else:
                    msg['article_number'] = '공지'
                    # 2) 게시글 id : article_id
                    e_url = ea.find_element_by_xpath('./td[2]/a')
                    a_url = e_url.get_attribute('href')
                    v = a_url.partition('boardId=')[2].partition('&')[0]
                    msg['article_id'] = v
    ...
    def next_page(self):
        try:
            self.switch_to_window(0)
            # 페이지 목록
            e = self.get_by_xpath('//div[@class="paging"]')
            ple = e.find_elements_by_xpath('.//a[@class="next"]/img[@alt="다음페이지로 이동"]|'
                                           './/span/img[@alt="다음페이지로 이동(비활성)"]')
            self.move_to_element(e)
            # 다음페이지로 이동 버튼 (맨 마지막 페이지일 경우 path가 다름)
            for pa in ple:
                if pa.get_attribute('alt') == '다음페이지로 이동':
                    self.safe_click(pa)
...
################################################################################
def do_start(**kwargs):
    with Me02(kwargs['config_f']) as ws:
        ws.start()
        return 0
