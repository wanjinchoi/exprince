...
class Me03(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Me_03.log'),
        ...
        self.logger.info(f'Starting Me_03 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        e = self.get_by_xpath('//*[@id="CREATE_DATE"]')
        self.safe_click(e)
        self.implicitly_wait(after_wait=1)
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            # 2) 작성 시간
            e = e_ab.find_element_by_xpath('.//div[@class="view_info02"]/ul/li[2]/dl/dd')
    ...
    def next_page(self):
        try:
            self.switch_to_window(0)
            # 페이지 목록
            e = self.get_by_xpath('//div[@class="content_body"]')
            ple = e.find_element_by_xpath('.//div[@class="paging"]')
            self.move_to_element(ple)
            for pa in ple.find_elements_by_xpath('./span/a'):
...
################################################################################
def do_start(**kwargs):
    with Me03(kwargs['config_f']) as ws:
        ws.start()
        return 0
