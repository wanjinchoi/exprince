...
Description
===========
ARGOS LABS Web scraping module for mogef (여성가족부)
"""
# Authors
# ===========
#
...
from alabslib.selenium import PySelenium, Select
...
class MogefSearch(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'MogefSearch.log'),
        ...
        self.logger.info(f'Starting Mogef Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def get_article(self, title_e, msg, ndx):
        ...
        try:
            ...
            e = self.get_by_xpath('//*[@id="contents"]/table[1]/tbody')
            mytr = e.find_elements_by_tag_name('tr')

            # 내용
            try:
                msg['contents'] = mytr[5].text.strip()
            except:
                msg['contents'] = ''
            ...
            if self.config['params']['site']['capture_article']:
                ...
                e_heads = self.driver.find_elements_by_xpath('//header[@id="header"]')
            ...
            # 첨부파일 이름과 URL 확보
            inner_html = mytr[4].get_attribute('innerHTML')
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('title="다운로드"') > 0:
                for k, sub_k in enumerate(e.find_elements_by_xpath('.//button[@title="다운로드"]')):
                    self.move_to_element(sub_k)
                    msg['attachment_url'].append("click")
                    # 다운로드폴더에 다운 받음
        ...
        try:
            ...
            for i in range(len(bil)):
                ...
                try:
                    ...
                    # 게시글 id : article_id
                    article_id = mytd[1].find_element_by_tag_name('a').get_attribute('href')
                    msg['article_id'] = re.search('\(\'(.+?)\'\)', article_id).group(1)
                    ...
                    # 조회수
                    msg['view_count'] = int(mytd[5].text.strip())
    ...
    def next_page(self):
        ...
        try:
            # 페이지 목록 구하기
            ple = self.get_by_xpath('//div[@class="brdPaging"]')

            # 현재 페이지 번호 구하기
            for pa in ple.find_elements_by_xpath('.//a'):
                if pa.get_attribute('class') == 'on':
                    self.current_pg_num = pa.text.strip()
                    break
    ...
    # ==========================================================================
    def get_clean_text(self, string):
        # 문자열에 포함된 특수문자를 와일드카드로 대치
        text = re.sub('[^A-Za-z0-9가-힣]', '*', string)
        return text
    ...
    def save_article(self, article):
        ...
        # 첨부파일 저장
        # cwd = os.getcwd()
        # os.chdir(self.get_download_path())
        # crawling_dir = self.get_safe_path(self.config['target']['folder'], article['article_id'])
        # for x, download_file in enumerate(article['af_href']):
        #     if 'Downloads' not in article['af_href'][x]:
        #         save_loc_fn = self.get_safe_path(crawling_dir, article['af_name'][x])
        #         urlretrieve(article['af_href'][x], save_loc_fn)
        #     else:
        #         # renamed_fileName = self.get_clean_text(article['af_name'][x])
        #         # src_file = renamed_fileName
        #         src_file = article['af_name'][x]
    ...
    def start(self):
        try:
            ...
            #
            # # 검색어 입력
            # e = self.get_by_xpath('//*[@id="searchKeyword"]')
            # self.send_keys(e, self.config['params']['site']['search'])
            #
            # # 제목 및 내용 검색 선택
            # select = Select(self.driver.find_element_by_name("searchCondition"))
            # select.select_by_visible_text(self.config['params']['site']['search_filter'])
            # self.implicitly_wait(after_wait=1)

            # # 검색 단추
            # e = self.get_by_xpath('//*[@id="listForm"]/fieldset/div/ul/li[2]/div/input[2]',
...
################################################################################
def do_start(**kwargs):
    with MogefSearch(kwargs['config_f']) as ws:
        ws.start()


################################################################################
if __name__ == '__main__':
    _config_f = 'mogef.yaml'
