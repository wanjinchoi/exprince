...
class Mohw01(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Mohw01.log'),
        ...
        self.logger.info(f'Starting Mohw Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def _screenshot(self, f):
        ...
        e = self.driver.find_element_by_xpath('//div[@class="f_bottom"]')
        self.driver.set_window_size(S('Width'), e.location['y'])  # May need manual adjustment
        self.driver.find_element_by_xpath('//div[@class="container"]').screenshot(f)
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//div[@class="board_view"]', timeout=1)

            # 0) 작성자 :
            e = e_ab.find_element_by_xpath('.//div[@class="bv_category"]/ul/li[3]/span[2]')
            msg['author'] = e.text.strip()
            # 1) 부서:
            # e = e_ab.find_element_by_xpath('.//div[@class="bv_category"]/ul/li[4]/span[2]')
            # msg['department'] = e.text.strip()
            # 2) 연락처:
            # e = e_ab.find_element_by_xpath('.//div[@class="bv_category"]/ul/li[5]/span[2]')
            # msg['author_number'] = e.text.strip()
            # 3) 게시글 내용 : 게시글 내용이 없는 경우가 있음
            e = e_ab.find_element_by_xpath('.//div[@class="bvc_txt"]')
            ...
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('bvf_lst') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//div[@class="bvf_lst"]/ul/li/a[1]')):
                    self.move_to_element(sub_k)
                    sub_k_url = sub_k.get_attribute('href')
                    msg['attachment_url'].append(sub_k_url)
                    sub_k_n = sub_k.text.rpartition('(')[0].strip()
    ...
    def next_page(self):
        try:
            self.switch_to_window(0)
            # 페이지 목록
            e = self.get_by_xpath('//div[@class="sub_content"]')
            ple = e.find_element_by_xpath('.//div[@class="paging"]')
            e = ple.find_element_by_xpath('.//li[@id="pageNum"]/strong')
            is_on = int(e.text.strip())
            self.move_to_element(ple)
            for pa in ple.find_elements_by_xpath('.//a'):
                if pa.get_attribute('title') == '다음(예전) 10개 페이지 이동':
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=1)
                    return
                elif pa.get_attribute('title') in ['첫페이지(최신)', '이전(최신) 10개 페이지 이동', '마지막페이지(예전) 이동']:
                    continue
...
################################################################################
def do_start(**kwargs):
    with Mohw01(kwargs['config_f']) as ws:
        ws.start()
        return 0
