...
class MolitSearch(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'MolitSearch.log'),
        ...
        self.logger.info(f'Starting Molit Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        # 검색어 입력
        e = self.get_by_xpath('//div[@class="sear_word_txt"]/input')
        self.send_keys(e, self.config['params']['site']['search'])

        # 검색 필터 : 내용 선택
        e = self.get_by_xpath('//div[@class="sear_sel_btn"]/i[2]/input')
        self.safe_click(e)

        # 검색 단추
        e = self.get_by_xpath('//div[@class="sear_word_txt"]/button',
    ...
    def get_comment(self, msg):
        try:
            e = self.get_by_xpath('//ul[@class="reply_lst"]')
            ...
            for i, cmt_e in enumerate(comments):
                ...
                # 댓글 nickname # 여기 다시 살피기
                e = cmt_e.find_element_by_xpath('./em')
                cmt['nickname'] = e.text.partition('2')[0].strip()
                self.move_to_element(e)

                # 댓글 작성 일시
                e = cmt_e.find_element_by_xpath('./em/span')
                cmt['create_ts'] = e.text.replace('-', '.') + ' 00:00:00'

                # 댓글 내용
                e = cmt_e.find_element_by_xpath('./div')
                cmt['contents'] = e.text.strip()

                # 댓글 아이디
                eid = cmt_e.find_element_by_xpath('./div/a')
                es = eid.get_attribute('onclick')
                v = es.rpartition("');")[0].rpartition("'")[2]
                cmt['comment_id'] = v
        ...
        try:
            ...
            if self.config['params']['site']['capture_article']:
                ...
                if self.config['params']['kwargs']['headless']:
                    ...
                    e_heads = self.driver.find_elements_by_xpath('//nav[@id="gnb"]|'
                                                                 '//div[@class="btnAllMn"]'
                                                                 )
            ...
            e_ab = self.get_by_xpath('//div[@class="bd_view"]', timeout=1)

            # 0) 소제목:
            inner_html = e_ab.get_attribute('innerHTML')
            try:
                e = e_ab.find_element_by_xpath('.//ul[@class="bd_view_ul_lst"]')
                sub_title = e.text.strip()
            except:
                sub_title = ""

            # 1) 작성일: create_ts
            e = e_ab.find_element_by_xpath('.//ul[@class="bd_view_ul_info"]/li[2]/span')
            msg['create_ts'] = e.text.replace('-', '.') + ':00'
            # 2) 연락처 :
            # e = e_ab.find_element_by_xpath('.//ul[@class="bd_view_ul_info"]/li[2]/span')
            # msg['contact_number'] = e.text.strip()
            # 3) 작성자 :
            e = e_ab.find_element_by_xpath('.//ul[@class="bd_view_ul_info"]/li[1]/span')
            msg['author'] = e.text.strip()
            # 4) 게시글 내용 :
            e = e_ab.find_element_by_xpath('.//div[@class="bd_view_cont"]')
            msg['contents'] = e.text.strip()
            if sub_title:
                msg['contents'] = sub_title + '\n' + msg['contents']
            ...
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('file') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//li[@class="file"]/span/a[1]')):
            ...
            msg['num_comments'] = self.get_comment(msg)
    ...
    def get_page(self):
        try:
            ...
            self.switch_to_window(1)
            # 페이지 테이블 구해오기
            e = self.get_by_xpath('//table[@class="table line_no bd_tbl bd_tbl_ul"]/tbody')
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    e = ea.find_element_by_xpath('.//td[@class="bd_num"]')
                    msg['article_number'] = e.text.strip()
                    # 1) 게시글 id : article_id
                    e_url = ea.find_element_by_xpath('.//td[@class="bd_title"]/a')
                    ...
                    # 3) 제목: title
                    e = ea.find_element_by_xpath('.//td[@class="bd_title"]/a')
                    msg['title'] = e.text.strip()
                    # 4) 조회수: view_count
                    e = ea.find_element_by_xpath('.//td[@class="bd_inquiry"]')
                    msg['view_count'] = int(e.text.strip())
                    # # 5) 분야 / 구분
                    # e = ea.find_element_by_xpath('.//td[@class="bd_field"]')
    ...
    def next_page(self):
        try:
            self.switch_to_window(0)
            # 페이지 목록
            e = self.get_by_xpath('//div[@id="contents"]')
            ple = e.find_element_by_xpath('.//div[@class="bd_pagination"]')
            e = ple.find_element_by_xpath('.//strong')
            is_on = int(e.text.strip())
            self.move_to_element(ple)
            for pa in ple.find_elements_by_xpath('.//a'):
                if pa.get_attribute('class') == 'direction next':
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=1)
                    return
                elif pa.get_attribute('class') == 'direction last':
                    continue
                elif pa.get_attribute('class') == 'direction first':
                    continue
                elif pa.get_attribute('class') == 'direction prev':
                    continue
...
################################################################################
def do_start(**kwargs):
    with MolitSearch(kwargs['config_f']) as ws:
        ws.start()
        return 0
