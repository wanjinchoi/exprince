...
class NaqsSearch(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'NaqsSearch.log'),
        ...
        self.logger.info(f'Starting Naqs Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        #전체 메뉴 클릭
        e = self.get_by_xpath('//*[@id="menu_all"]/a')
        self.safe_click(e)
        self.implicitly_wait(after_wait=1)

        # 공지/공고 클릭
        e = self.get_by_xpath('//*[@id="txt"]/ul/li[4]/ul/li[1]/ul/li[1]/a')
        self.safe_click(e)
        self.implicitly_wait(after_wait=1)
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            # 1) 이메일
            # e = e_ab.find_element_by_xpath('./tr[3]/td[1]')
            # msg['mail'] = e.text.strip()
            # # 2) 연락처
            # e = e_ab.find_element_by_xpath('./tr[3]/td[2]')
            # msg['contact_number'] = e.text.strip()
            # 3) 게시글 내용 :
            e = e_ab.find_element_by_xpath('.//div[@class="viewCon"]')
            ...
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('b_fileList') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//ul[@class="b_fileList"]/li/a')):
                    self.move_to_element(sub_k)
                    sub_k_url = sub_k.get_attribute('href')
                    msg['attachment_url'].append(sub_k_url)
                    sub_k_n = sub_k.text.partition('다운로드')[0].strip()
    ...
    def get_page(self):
        try:
            ...
            # 페이지 테이블 구해오기
            e = self.get_by_xpath('//table[@class="board_list"]/tbody')
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    # 2) 게시글 id : article_id
                    e_url = ea.find_element_by_xpath('./td[@class="left"]/a')
                    ...
                    # 3) 제목: title
                    e_t = ea.find_element_by_xpath('./td[@class="left"]/a')
                    msg['title'] = e_t.text.strip()
                    # 4) 작성 시간
                    e = ea.find_element_by_xpath('./td[4]')
                    msg['create_ts'] = e.text.strip().replace('-', '.')
                    # 5) 작성자
    ...
    def next_page(self):
        try:
            self.switch_to_window(0)
            # 페이지 목록
            e = self.get_by_xpath('//div[@id="txt"]')
            ple = e.find_element_by_xpath('.//div[@class="pageWrap"]')
            e = ple.find_element_by_xpath('.//a[@class="paging-item strongSelected"]')
            is_on = int(e.text.strip())
            self.move_to_element(ple)
            for pa in ple.find_elements_by_xpath('.//a'):
                # 여기 다시 하기 다음으로 넘어가는 거 안 된다
                if pa.get_attribute('title') == '다음 페이지리스트로 이동':
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=1)
                    return
                elif pa.get_attribute('title') == '마지막 페이지리스트로 이동':
                    continue
                elif pa.get_attribute('title') == '처음 페이지리스트로 이동':
                    continue
                elif pa.get_attribute('title') == '이전 페이지리스트로 이동':
                    continue
...
################################################################################
def do_start(**kwargs):
    with NaqsSearch(kwargs['config_f']) as ws:
        ws.start()
        return 0
