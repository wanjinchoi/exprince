...
class NfqsSearch(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'NfqsSearch.log'),
        ...
        self.logger.info(f'Starting Nfqs Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        # 새소식 보도 클릭
        e = self.get_by_xpath('//div[@class="mNews"]//li[1]')
        self.safe_click(e)
        self.implicitly_wait(after_wait=1)

        # 공지/공고 클릭
        e = self.get_by_xpath('//ul[@id="snb"]/li[1]/ul/li[1]')
        self.safe_click(e)
        self.implicitly_wait(after_wait=1)
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//div[@class="board-wrap board-view-wrap"]', timeout=1)

            # 1) 게시글 내용 :
            e = e_ab.find_element_by_xpath('.//div[@class="board-view-cont"]')
            ...
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('board-view-file') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//div[@class="board-view-file"]/div/a[1]')):
                    self.move_to_element(sub_k)
                    sub_k_url = sub_k.get_attribute('href')
                    msg['attachment_url'].append(sub_k_url)
                    sub_k_n = sub_k.text.partition('\n')[2].rpartition('(')[0].strip()
    ...
    def get_page(self):
        try:
            ...
            # 페이지 테이블 구해오기
            e = self.get_by_xpath('//table[@class="board-list-table"]/tbody')
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    # 2) 게시글 id : article_id
                    e_url = ea.find_element_by_xpath('./td[@class="subject"]/a')
    ...
    def next_page(self):
        try:
            ...
            self.switch_to_window(0)
            # 페이지 목록
            e = self.get_by_xpath('//div[@class="board-wrap"]')
            ple = e.find_element_by_xpath('.//div[@class="board-pagelist"]')
            self.move_to_element(ple)
            for pa in ple.find_elements_by_xpath('.//a'):
                if pa.get_attribute('class') == 'nextpage':
                    self.safe_click(pa)
...
################################################################################
def do_start(**kwargs):
    with NfqsSearch(kwargs['config_f']) as ws:
        ws.start()
        return 0
