...
class NifdsSearch(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'NifdsSearch.log'),
        ...
        self.logger.info(f'Starting Nifds Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//div[@class="content_guide"]', timeout=1)

            # 0) 작성자
            e = e_ab.find_element_by_xpath('.//ul/li[2]')
            msg['author'] = e.text.partition('작성자 :')[2].strip()

            # 1) 조회수: view_count
            e = e_ab.find_element_by_xpath('.//ul/li[4]')
            e_view = e.text.strip()
            msg['view_count'] = int(re.sub(r'[^0-9]', '', e_view))

            # 2) 게시글 내용 : 게시글 내용이 없는 경우가 있음
            try:
                e = e_ab.find_element_by_xpath('.//div[@class="contents_txt"]')
            ...
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('bv_file01') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//div[@class="bv_file01"]//li/a')):
        ...
        try:
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    v = a_url.partition('seq=')[2].partition('&')[0]
                    msg['article_id'] = v
    ...
    def next_page(self):
        try:
            self.switch_to_window(0)
            # 페이지 목록
            e = self.get_by_xpath('//div[@class="content_guide"]')
            ple = e.find_element_by_xpath('.//div[@class="bbs_page"]')
            e = ple.find_element_by_xpath('.//li[@class="on"]')
            is_on = int(e.text.strip())
            self.move_to_element(ple)
            for pa in ple.find_elements_by_xpath('.//ul[@class="num"]//a''|.//li/a/img'):
                if pa.get_attribute('alt') == '다음 페이지로 ':
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=1)
                    return
                elif pa.get_attribute('alt') == '첫 페이지':
                    continue
                elif pa.get_attribute('alt') == '이전 페이지':
                    continue
                elif pa.get_attribute('alt') == '마지막 페이지로 ':
                    continue
...
################################################################################
def do_start(**kwargs):
    with NifdsSearch(kwargs['config_f']) as ws:
        ws.start()
        return 0
