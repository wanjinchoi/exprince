...
class Nifs01(PySelenium):
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            # 2) 시각 "2022-02-09 17:51:22.0"
            # e = e_ab.find_element_by_xpath('./tr[2]/td[1]')
            # msg['create_ts'] = e.text.strip()[:-2].replace('-', '.')
...
################################################################################
def do_start(**kwargs):
    with Nifs01(kwargs['config_f']) as ws:
        ws.start()
        return 0
