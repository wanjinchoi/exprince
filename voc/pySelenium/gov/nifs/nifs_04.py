...
#     - 첨부파일 수집 로직 수정
#  * [2023/05/04]
#     - 게시글 검색 날짜 확인 후 닫기 버튼 클릭 추가
#  * [2023/05/02]
#     - 첨부파일 XPath 가져오는 부분 수정
#  * [2023/03/12]
#     - 날짜 검색 기능 수정
#  * [2022/05/02]
#     - 날짜 검색 기능 추가 하루 기준이기 때문에 yaml에 stop_article_older_than에 date만 사용
#  * [2022/04/22]
#     - 게시글이 최신순으로 나열되지 않음. 설정한 수집 일자 보다 지난 게시글이 5개 이상 될때 수집 중지.
#  * [2022/02/25]
#     - starting
################################################################################
import os
...
class Nifs04(PySelenium):
    ...
    # ==========================================================================
    def search(self):
        input_date = self.config['params']['site']['stop_article_older_than']['datetime']
        strat_date = end_date = str(datetime.datetime.strptime(input_date, "%Y.%m.%d %H:%M:%S").date())
        # end_date = str(datetime.datetime.strptime(input_date, "%Y.%m.%d %H:%M:%S").date())
        end_date = datetime.datetime.now().strftime('%Y-%m-%d')

        # 시작 날짜 입력
        e = self.get_by_xpath('//*[@id="REG_DT_BEGIN"]')
        self.send_keys(e, strat_date)
        self.implicitly_wait(after_wait=1)

        # 끝 날짜 입력
        e = self.get_by_xpath('//*[@id="REG_DT_END"]')
        self.send_keys(e, end_date)
        self.implicitly_wait(after_wait=3)

        #날짜 선택 창 닫기
        e = self.get_by_xpath('//*[@id="ui-datepicker-div"]/div[2]/button[2]', cond='element_to_be_clickable')
        self.safe_click(e)
        self.implicitly_wait(after_wait=1)

        # 검색 버튼
        e = self.get_by_xpath('//*[@id="schFrm"]/div/ul/li[4]/a', cond='element_to_be_clickable')
        self.safe_click(e)
        self.implicitly_wait(after_wait=1)
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//div[@class="board-view"]', timeout=1)

            # 1) 게시글 내용 : 게시글 내용이 없는 경우가 있음
            e = e_ab.find_element_by_xpath('.//table/tbody/tr/td')
            ...
            if outer_html.find('파일') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//tr[3]//td[@colspan="5"]//ul//li')):
                    self.switch_to_window(0)
                    e_ab = self.get_by_xpath('//table/thead', timeout=1)
                    e_s = e_ab.find_elements_by_xpath('.//tr[3]//td[@colspan="5"]//ul//li//a')
                    sub_k = e_s[k]
                    # 미리보기 파일까지 합쳐져서 나온다.
                    if sub_k.get_attribute('target') == '_blank':
                        continue
    ...
    def get_page(self):
        try:
            ...
            for i, ea in enumerate(es):
                ...
                #     self.logger.debug(f"Page[{self.cur_page}:{i + 1}]"
...
################################################################################
def do_start(**kwargs):
    with Nifs04(kwargs['config_f']) as ws:
        ws.start()
        return 0
