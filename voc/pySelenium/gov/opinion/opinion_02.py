...
class Opinion02(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Opinion02.log'),
        ...
        self.logger.info(f'Starting Opinion02 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def login(self):
        try:
            ...
            # 간편로그인 형식 선택
            e = self.get_by_xpath('//div[@class="snsLogBtn"]/li[3]')
            self.safe_click(e)
            self.implicitly_wait(after_wait=2)
            # ID 입력
            e = self.get_by_xpath('//input[@id="id"]')
            self.send_keys_clipboard(e, self.config['params']['site']['userid'])
            self.implicitly_wait(after_wait=2)
            # PW 입력
            e = self.get_by_xpath('//input[@id="pw"]')
            self.send_keys_clipboard(e, self.config['params']['site']['passwd'])
            self.implicitly_wait(after_wait=2)
            e = self.get_by_xpath('//button[@class="btn_login"]')
            self.safe_click(e)
            # 충분한 딜레이가 필요
            # self.implicitly_wait(after_wait=10)

            # 전화번호를 입력하라는 alert가 나옴
            # alert = self.driver.switch_to.alert
            # alert.accept()
            self.alert_action(action='accept')
            self.implicitly_wait(after_wait=2)
    ...
    def get_comment(self, msg):
        try:
            ...
            for i, cmt_e in enumerate(comments):
                ...
                ts = e.text.partition('|')[2].rpartition('제출')[0].replace(' ', '') + ':00'
                ts = ts.rpartition('.')
                cmt['create_ts'] = ts[0] + ' ' + ts[2]
                # 댓글 아이디
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//form[@id="ogLmPpVo"]', timeout=1)
            ...
            # 1) 게시글 내용 : 패스가 변함
            comments_xpath = './/div[@class="view_con"]|.//div[@class="bodReadBodyMk"]'
            ...
            if msg['num_comments'] == 0:
                return
            e = self.get_by_xpath('//ul[@class="tab_type alignR hidden_mob_submenu"]/li[3]')
            self.safe_click(e)
            msg['comment_list'] = []
        ...
        try:
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    # 2번째 페이지부터는 쿼리스트링뒤에 pageindex가 붙음
                    if v.find('?') > 0:
                        v = v.rpartition('?')[0]
                    msg['article_id'] = v
                    ...
                    # # 6) 부서명: Department
                    # e = ea.find_element_by_xpath('./li[3]/p')
                    # msg['department'] = e.text.strip()
                    # 7) 작성 시간
                    ...
                    msg['author'] = e.text.strip()
                    # 9) 조회수: view_count
                    e = ea.find_element_by_xpath('./li[7]')
                    msg['view_count'] = int(e.text.strip().replace(',', ''))
...
################################################################################
def do_start(**kwargs):
    with Opinion02(kwargs['config_f']) as ws:
        ws.start()
        return 0
