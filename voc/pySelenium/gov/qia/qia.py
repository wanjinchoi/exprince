...
Description
===========
ARGOS LABS Web scraping module for qia (농림축산검역본부)
"""
# Authors
# * [2023/01/03]
#   site_board 변경으로 모듈 수정
# * [2022/12/12]
#   등록일 UI 변경으로 XPath 수정
# ===========
...
class QiaSearch(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'QiaSearch.log'),
        ...
        self.logger.info(f'Starting Qia Crawling... with '
                         f'config:\n{out_config}')

    # ==========================================================================
    def get_article_law(self, title_e, msg, ndx):
        ...
        try:
            ...
            # 작성자
            try:
                author = self.get_by_xpath('//*[@id="contents"]/div[1]/table/tbody/tr[3]/td/table/tbody/tr/td[1]')
            except:
                msg['author'] = ''
            else:
            ...
            msg['contents'] = ''
            try:
                contents = self.get_by_xpath('//*[@id="contents"]/div[1]/table/tbody/tr[4]/td')
            except:
                msg['contents'] = ''
            else:
            ...
            msg['attachment_url'] = []
            try:
                e = self.get_by_xpath('//*[@id="contents"]/div[1]/table/tbody/tr[6]/td/ul')
    ...
    # ==========================================================================
    def get_page_law(self):
        try:
            self.cur_page += 1

            # 검색 결과물 페이지 읽기
            e = self.get_by_xpath('//*[@id="contents"]//table/tbody')
            ...
            for i in range(len(bil)):
                ...
                try:
                    ...
                    # 게시글 제목
                    try:
                        title = bi.find_element_by_xpath('./td[4]/a[2]')
                    except:
                        msg['title'] = ''
                    else:
                        if title.text.strip() == '':
                    ...
                    # 게시글 URL
                    try:
                        url = bi.find_element_by_xpath('./td[4]/a[2]').get_attribute('href')
                    except:
                        msg['article_url'] = ''
                    else:
                        msg['article_url'] = url
                    ...
                    # 게시글 id
                    try:
                        href = bi.find_element_by_xpath('./td[4]/a[2]').get_attribute('href')
                    except:
                        msg['article_id'] = ''
                    else:
                        article_id = re.search(r'id=(\d+)\&type',  href).group(1)
                        if article_id == '':
                            msg['article_id'] = ''
                        else:
                    ...
                    # 등록일
                    try:
                        create_ts = bi.find_element_by_xpath('./td[6]')
                    except:
                    ...
                    # 게시글 본문으로 이동
                    title_e = bi.find_element_by_xpath('./td[4]/a[2]')
                    self.move_to_element(title_e)
                    self.get_article_law(title_e, msg, i+1)
    ...
    def get_article(self, title_e, msg, ndx):
        ...
        try:
            ...
            msg['contents'] = ''
            try:
                contents = self.get_by_xpath('//*[@id="contents"]/table/tbody/tr[3]')
            except:
                msg['contents'] = ''
            else:
                msg['contents'] = contents.text.strip()
            ...
            msg['attachment_url'] = []
            try:
                e = self.get_by_xpath('//*[@id="contents"]/table/tbody/tr[5]/td/ul')
                afs= e.find_elements_by_xpath('./li/a[1]')
            except:
                pass
            else:
            ...
            for i in range(len(bil)):
                ...
                try:
                    ...
                    try:
                        ...
                        article_id = re.search('id=(\d+)\&type',  href).group(1)
                        if article_id == '':
                            msg['article_id'] = ''
                        else:
                            msg['article_id'] = article_id.strip()
                    ...
                    try:
                        ...
                        if view_count.text.strip() == '':
                            ...
                            msg['view_count'] = view_count.text.strip()
    ...
    def next_page(self):
        try:
            # 페이지 목록 구하기
            try:
                ple = self.get_by_xpath('//ul[@class="paging"] | //ul[@class="paging mt_0"]')
                pa_list = ple.find_elements_by_xpath('./li/strong | ./li/a')
            except:
                # 페이지가 없는 경우
                self.is_done = True
                return
            ...
            # 현재 페이지 번호 구하기
            for pa in pa_list:
                if pa.get_attribute('class') == 'blue':
            ...
            for pa in pa_list:
                ...
                if is_current:
                    ...
                    nple = self.get_by_xpath('//ul[@class="paging"] | //ul[@class="paging mt_0"]')
                    for npa in nple.find_elements_by_xpath('./li/strong | ./li/a'):
                        if npa.get_attribute('class') == 'blue':
    ...
    def start(self):
        try:
            ...
            # 1. 알림/소식 > 보도자료·설명
            if self.config['params']['site']['site_board'] == '보도자료·설명':

                first_menu = self.get_by_xpath('//*[@id="gnb"]/li[6]/a')
                self.safe_click(first_menu)
                self.implicitly_wait(after_wait=1)

                second_level_menu = self.get_by_xpath('//*[@id="lnb"]/li[3]/a')
            ...
            # 2. 알림마당 > 법령정보 > 입법/행정예고
            elif self.config['params']['site']['site_board'] == '입법·행정예고':

                first_menu = self.get_by_xpath('//*[@id="gnb"]/li[6]/a')
                self.safe_click(first_menu)
                self.implicitly_wait(after_wait=1)

                second_level_menu = self.get_by_xpath('//*[@id="lnb"]/li[7]/a')
                self.safe_click(second_level_menu)
                self.implicitly_wait(after_wait=1)

                third_level_menu = self.get_by_xpath('//*[@id="lnb"]/li[7]/ul/li[7]/a')
...
################################################################################
def do_start(**kwargs):
    with QiaSearch(kwargs['config_f']) as ws:
        ws.start()
