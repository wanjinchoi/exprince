...
class Seoul01(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Seoul01.log'),
        ...
        self.logger.info(f'Starting Seoul01 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def get_comment(self, msg):
        try:
            ...
            comments = e_a.find_elements_by_xpath(
                './/div[@class="reply-wrapper"]' '|.//div[@class="reply-wrapper reply-best-wrapper"]')
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//div[@id="viewWrap"]', timeout=1)

            # 작성자
            e = e_ab.find_element_by_xpath('.//dd[@class="dept"]')
            msg['author'] = e.text.strip()
            # 좋아요
            e = e_ab.find_element_by_xpath('//a[@title="좋아요"]/span')
            msg['like'] = int(e.text.strip())
            # 본문
            e = e_ab.find_element_by_xpath('.//div[@id="post_content"]')
            ...
            # 댓글 창이 없는 경우도 존재
            try:
                self.switch_to_iframe('//iframe[@title="livere-comment"]')
            except:
                return
    ...
    def get_page(self):
        try:
            ...
            self.switch_to_window(1)
            # 페이지 테이블 구해오기
            e = self.get_by_xpath('//div[@class="news-lst"]')
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    # 4) 작성 시간
                    e = ea.find_element_by_xpath('.//em[@class="date"]')
                    msg['create_ts'] = e.text.partition('[')[0].strip().replace('-', '.')
                    # 5) 제목: title
                    e_t = ea.find_element_by_xpath('.//em[@class="subject"]')
    ...
    def next_page(self):
        try:
            ...
            # 페이지 목록
            e = self.get_by_xpath('//div[@class="cate-wrap"]')
            ple = e.find_element_by_xpath('.//ul[@class="pagination"]')
            e = ple.find_element_by_xpath('.//li[@class="active"]/a')
            is_on = int(e.text.partition('\n')[0].strip())
            self.move_to_element(ple)
            for pa in ple.find_elements_by_xpath('.//li'):
                if pa.get_attribute('class') == 'btn-next':
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=1)
                    return
                elif pa.get_attribute('class') in ['btn-last', 'btn-first', 'btn-prev']:
                    continue
                elif int(pa.text.partition('\n')[0].strip()) > is_on:
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=1)
                    return
            self.is_done = True
...
################################################################################
def do_start(**kwargs):
    with Seoul01(kwargs['config_f']) as ws:
        ws.start()
        return 0
