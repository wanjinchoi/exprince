...
class Seoul02(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Seoul02.log'),
        ...
        self.logger.info(f'Starting Seoul02 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//table[@id="viewTable"]', timeout=1)
...
################################################################################
def do_start(**kwargs):
    with Seoul02(kwargs['config_f']) as ws:
        ws.start()
        return 0
