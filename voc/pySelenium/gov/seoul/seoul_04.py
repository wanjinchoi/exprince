...
class Seoul04(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'Seoul04.log'),
        ...
        self.logger.info(f'Starting Seoul04 Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//table[@id="viewTable"]', timeout=2)

            # 3) 작성자
            e = e_ab.find_element_by_xpath('.//span[@class="sib-viw-type-basic-subject-info1-left"]')
            msg['author'] = e.text.strip()
            # 본문
            e = e_ab.find_element_by_xpath('.//td[@id="scrabArea"]')
            ...
            msg['attachment_url'] = []
            msg['attachment_name'] = []
            if inner_html.find('sib-viw-type-basic-file') > 0:
                for k, sub_k in enumerate(e_ab.find_elements_by_xpath('.//td[@class="sib-viw-type-basic-file"]//a')):
            ...
            # 이전 페이지
            self.driver.back()
            self.implicitly_wait(after_wait=15)
        ...
        try:
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    a_url = e_url.get_attribute('data-code')
                    msg['article_id'] = a_url
                    # 3) 조회수
    ...
    def next_page(self):
        try:
            ...
            # 페이지 목록
            e = self.get_by_xpath('//div[@id="seoul-integrated-board"]')
            ple = e.find_element_by_xpath('.//div[@class="sib-paging"]')
            e = ple.find_element_by_xpath('.//span/strong')
            is_on = int(e.text.strip())
            self.move_to_element(ple)
            for pa in ple.find_elements_by_xpath('.//a'):
                if pa.get_attribute('class') == 'sib-paging-next':
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=12)
                    return
                elif pa.get_attribute('class') in ['sib-paging-prev']:
                    continue
...
################################################################################
def do_start(**kwargs):
    with Seoul04(kwargs['config_f']) as ws:
        ws.start()
        return 0
