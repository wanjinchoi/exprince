...
class FoodNewsSearch(PySelenium):
    # ==========================================================================
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'FoodNewsSearch.log'),
        ...
        self.logger.info(f'Starting FoodNews Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        # 검색어 입력
        e_b = self.get_by_xpath('//button[@class="sch-btns"]')
        self.safe_click(e_b)
    ...
    def get_comment(self, msg):
        try:
            ...
            e_a = self.get_by_xpath('//div[@class="reply_wrap"]')
            comments = e_a.find_elements_by_xpath('.//ul[@class="reply_list"]/li')
            parent_comment_id = ''
            ...
            for i, cmt_e in enumerate(comments):
                ...
                # 댓글 id
                cmt['comment_id'] = cmt_e.get_attribute('id').rpartition('_')[2]
                # 대댓글
                e_re = cmt_e.find_element_by_xpath('./../..')
                is_reply = e_re.get_attribute('class') == 're_reply_box'
                ...
                # 댓글 nickname :
                e = cmt_e.find_element_by_xpath('.//div[@class="reply_top"]/h5')
                cmt['nickname'] = e.text.strip()

                # 댓글 작성 시각
                e = cmt_e.find_element_by_xpath('.//ul[@class="ffd"]')
                cmt['create_ts'] = e.text.strip().replace('-', '.')

                # 댓글 좋아요 / 싫어요 : 대댓글은 추천 창이 없음
                try:
                    e = cmt_e.find_element_by_xpath('.//ul[@class="btn_recom"]/li[1]/a/span[2]')
                    cmt['like'] = int(e.text.strip())
                    e = cmt_e.find_element_by_xpath('.//ul[@class="btn_recom"]/li[2]/a/span[2]')
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//article[@class="grid body"]', timeout=1)
            # 댓글 수
            e = e_ab.find_element_by_xpath('.//h4[@class="titles"]/small')
            msg['num_comments'] = int(e.text.strip())

            # 1) 게시글 내용 :
            e = e_ab.find_element_by_xpath('.//article[@id="article-view-content-div"]')
    ...
    def get_page(self):
        try:
            ...
            # 페이지 테이블 구해오기
            e = self.get_by_xpath('//section[@id="section-list"]/ul')
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    # 1) 카테고리 : board_name
                    e = ea.find_element_by_xpath('.//span[@class="byline"]/em[1]')
                    board_name = e.text.strip()
                    if board_name in ["식품산업", "정책"]:
                        msg['board_name'] = board_name
                    else:
                        self.logger.info('pass')
                        continue
                    # 2) 게시글 id : article_id
                    e_url = ea.find_element_by_xpath('.//h4[@class="titles"]/a')
                    ...
                    # 4) 제목: title
                    e_t = ea.find_element_by_xpath('.//h4[@class="titles"]/a')
                    msg['title'] = e_t.text.strip()
                    # 5) 작성 시간
                    e = ea.find_element_by_xpath('.//span[@class="byline"]/em[3]')
                    msg['create_ts'] = e.text.strip() + ':00'
                    # 6) 작성자
                    e = ea.find_element_by_xpath('.//span[@class="byline"]/em[2]')
    ...
    def next_page(self):
        try:
            self.switch_to_window(0)
            # 페이지 목록
            e = self.get_by_xpath('//section[@class="section-body"]')
...
################################################################################
def do_start(**kwargs):
    with FoodNewsSearch(kwargs['config_f']) as ws:
        ws.start()
        return 0
