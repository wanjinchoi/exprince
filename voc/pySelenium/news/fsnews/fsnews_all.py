...
class FsNewsSearch(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'FsNewsSearch.log'),
        ...
        self.logger.info(f'Starting FsNews Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        ...
        # 검색어 입력
        e = self.get_by_xpath('//input[@id="search"]')
        self.send_keys(e, self.config['params']['site']['search'])

        # 검색 단추
        e = self.get_by_xpath('//button[@title="기사검색"]',
    ...
    def get_comment(self, msg):
        try:
            ...
            e_a = self.get_by_xpath('//section[@class="container"]')
            comments = e_a.find_elements_by_xpath('.//article')
            parent_comment_id = ''
            ...
            for i, cmt_e in enumerate(comments):
                ...
                # 대댓글
                is_reply = cmt_e.get_attribute('data-mode') == 'sub_reply'
                ...
                # 댓글 nickname :
                e = cmt_e.find_element_by_xpath('.//div[@class="comments-user"]/strong')
                cmt['nickname'] = e.text.strip()

                # 댓글 작성 시각
                e = cmt_e.find_element_by_xpath('.//div[@class="comments-user"]/small')
                cmt['create_ts'] = e.text.strip().replace('-', '.')

                # 댓글 좋아요 / 싫어요 :
                e = cmt_e.find_element_by_xpath('.//div[@class="comments-vote"]/button[1]/span')
                cmt['like'] = int(e.text.strip())
                e = cmt_e.find_element_by_xpath('.//div[@class="comments-vote"]/button[2]/span')
                cmt['hate'] = int(e.text.strip())

                # 댓글 내용
                e = cmt_e.find_element_by_xpath('.//section[@class="comments-content"]')
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            e_ab = self.get_by_xpath('//div[@class="user-snb-wrapper"]', timeout=1)
            # 댓글 수
            e = e_ab.find_element_by_xpath('.//span[@class="text-muted"]')
            msg['num_comments'] = int(e.text.strip())

            # 1) 게시글 내용 :
            e = e_ab.find_element_by_xpath('.//div[@id="article-view-content-div"]')
            ...
            if inner_html.find('img') > 0:
                for j, sub_e in enumerate(e.find_elements_by_xpath('//figure/img')):
            ...
            # 댓글 전체보기
            try:
                e = e_ab.find_element_by_xpath('.//footer[@class="reply-footer"]')
                self.safe_click(e)
                self.implicitly_wait(after_wait=1)
                while True:
                    c = self.get_by_xpath('//div[@class="user-snb-wrapper"]')
                    c_ple = c.find_element_by_xpath('.//ul[@class="pagination text-center"]')
                    e = c_ple.find_element_by_xpath('./li[@class="current user-bg"]')
                    is_on = int(e.text.strip())
                    self.move_to_element(c_ple)
                    # 답글이 1 이상이면 클릭하기
                    for ca, sub_ca in enumerate(c.find_elements_by_xpath('.//button[@class="comments-reply"]')):
                        e = sub_ca.text.partition('답글')[2]
                        if e.find('쓰기') >= 0:
                            continue
                        elif e.find('쓰기') < 0:
                    ...
                    for c_b, sub_cmt in enumerate(c_ple.find_elements_by_xpath('./li')):
                        if sub_cmt.get_attribute('class') == 'pagination-next':
                            self.safe_click(sub_cmt)
                            self.implicitly_wait(after_wait=1)
                            return
                        elif sub_cmt.get_attribute('class') == 'pagination-start':
                            continue
                        elif sub_cmt.get_attribute('class') == 'pagination-onenext':
                            continue
                        elif sub_cmt.get_attribute('class') == 'pagination-end':
                            continue
                        elif sub_cmt.get_attribute('class') == 'pagination-oneprev':
                            continue
                        elif int(sub_cmt.text.strip()) > is_on:
                            self.safe_click(sub_cmt)
                            self.implicitly_wait(after_wait=1)
                            break
                    if msg['num_comments'] == len(msg['comment_list']):
                        for c_back in range(is_on):
                            self.driver.back()
                        break

            except:
                # 답글 창이 있는 경우
                for ca, sub_ca in enumerate(e_ab.find_elements_by_xpath('.//button[@class="comments-reply"]')):
                    e = sub_ca.text.partition('답글')[2]
                    if e.find('쓰기') >= 0:
                        continue
                    elif e.find('쓰기') < 0:
    ...
    def get_page(self):
        try:
            ...
            # 페이지 테이블 구해오기
            e = self.get_by_xpath('//section[@class="article-list-content type-sm text-left"]')
            ...
            for i, ea in enumerate(es):
                ...
                try:
                    ...
                    # 카테고리 (board_name)
                    e = ea.find_element_by_xpath('.//div[@class="list-dated"]')
                    e_a = e.text.partition('|')[0].strip()
                    if e_a in ['산업&기업', '이슈', '정책&제도', '위생&안전', '식품&유통', '시설&기자재', '위탁급식']:
                        msg['board_name'] = e_a
                    else:
                        continue
                    # 1) 게시글 id : article_id
                    e_url = ea.find_element_by_xpath('.//div[@class="list-titles"]/a')
                    ...
                    # 3) 제목: title
                    e_t = ea.find_element_by_xpath('.//div[@class="list-titles"]/a/strong')
                    msg['title'] = e_t.text.strip()
                    # 4) 작성 시간
                    e = ea.find_element_by_xpath('.//div[@class="list-dated"]')
                    msg['create_ts'] = e.text.rpartition('|')[2].strip().replace('-', '.') + ':00'
                    # 5) 작성자
                    e = ea.find_element_by_xpath('.//div[@class="list-dated"]')
                    msg['author'] = e.text.partition('|')[2].partition('|')[0].partition('기자')[0].strip()
    ...
    def next_page(self):
        try:
            self.switch_to_window(0)
            # 페이지 목록
            e = self.get_by_xpath('//div[@class="article-list"]')
            ple = e.find_element_by_xpath('.//ul[@class="pagination text-center"]')
            e = ple.find_element_by_xpath('./li[@class="current user-bg"]')
            is_on = int(e.text.strip())
            self.move_to_element(ple)
            for pa in ple.find_elements_by_xpath('.//li'):
                # 여기 다시 하기 다음으로 넘어가는 거 안 된다
                if pa.get_attribute('class') == 'pagination-next':
                    self.safe_click(pa)
                    self.implicitly_wait(after_wait=1)
                    return
                elif pa.get_attribute('class') == 'pagination-start':
                    continue
                elif pa.get_attribute('class') == 'pagination-onenext':
                    continue
                elif pa.get_attribute('class') == 'pagination-end':
                    continue
                elif pa.get_attribute('class') == 'pagination-previous':
                    continue
                elif pa.get_attribute('class') == 'pagination-oneprev':
                    continue
...
################################################################################
def do_start(**kwargs):
    with FsNewsSearch(kwargs['config_f']) as ws:
        ws.start()
        return 0
