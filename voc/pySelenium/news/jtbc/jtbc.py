...
Description
===========
ARGOS LABS Web scraping module for jtbc(jtbc)
"""
# Authors
# ===========
#
...
#     - 게시글 목록 xpath 변경
#  * [2022/04/27]
#     - 댓글 대댓글 작성일자 시간타입 수정 %H -> %I
#  * [2022/04/21]
#     - 댓글 대댓글 create_ts 포맷에 에러가 있어서 수정
#  * [2022/04/15]
#     - 댓글에 null 값 제거
#  * [2022/03/14]
#     - 1차 버전 완료
...
class JtbcSearch(PySelenium):
    ...
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'JtbcSearch.log'),
        ...
        self.logger.info(f'Starting Jtbc Crawling... with '
                         f'config:\n{out_config}')

    # ==========================================================================
    def get_cmt_reply(self, msg):

        e = self.get_by_xpath('//div[@class="comment_wrapper"]')
        self.move_to_element(e)

        se = e.find_element_by_xpath('./div[@id="cmt_reg"]')
        cmt_num = se.find_element_by_xpath('.//em[@id="tCount"]')
        # 댓글이 없으면 다음 게시글로 돌아간다
        if cmt_num.text.strip().replace('개', '') == '0':
            msg['num_comments'] = 0
            self.cmt_done = True
            return
        else:
            msg['num_comments'] = int(cmt_num.text.strip().replace('개', ''))

        self.move_to_element(se)

        # '댓글 펼쳐보기' 혹은 ' N개 더보기'를 클릭하면 cmt_list 항목 수가 바뀜
        cmt_list = e.find_elements_by_xpath('.//div[@id="TCMT_ContBox"]/ul/li')
        # del cmt_list[0] # 데이타 정보가 없는 헤드 정보임
        while len(cmt_list) != 0: # 첫 번째 : 댓글이 없는 경우를 제외시킴

            # 두 번째 : 펼쳐야 하는 경우가 없는 경우는 빠져 나가기
            fold_flag = False
            # for i, cmt_e in enumerate(cmt_list):
            #     if cmt_e.get_attribute('class') == 'cmt_add':
            #         fold_flag = True
            for more in self.driver.find_elements_by_xpath('//div[@id="TCMT_ContBox"]/div[@class="cmt_add"]'):
            ...
            cmt_list = e.find_elements_by_xpath('.//div[@id="TCMT_ContBox"]/ul')
        ...
        for i, cmt_e in enumerate(cmt_list):  # 확보된 댓글 리스트를 하나씩 처리하기
            ...
            self.move_to_element(cmt_e)
            # try:
            #     cmt_top, cmt_bottom = cmt_e.find_elements_by_xpath('./li')
            # except:
            #     pass
            # else:
            cmt_ea = cmt_e.find_element_by_xpath('.//div[@class="comment_info"]')
            if cmt_ea.text.strip() == '이 글은 작성자 또는 관리자에 의해 삭제된 댓글입니다.':
                cmt_count += 1
                self.logger.info(f'\t   댓글 [{msg["num_comments"]}/{cmt_count}]: 삭제된 댓글')
                pass
            else:
                nickname = cmt_e.find_element_by_xpath('.//div//div[1]//div[@class="nick"]//b')
                #//*[@id="cmtItm_973347"]/div/div[1]/div[1]/b
                create_time = cmt_e.find_element_by_xpath('.//div//div[1]//div[@class="nick"]//span')
                contents = cmt_e.find_element_by_xpath('.//div//div[@class="comment_body"]//span')
                ...
                # 닉네임, 작성 일시, 내용
                create_ts = create_time.text.strip().replace('오후', 'pm').replace('오전', 'am')
                cmt['create_ts'] = datetime.datetime.strptime(create_ts, '%Y-%m-%d %p %I:%M:%S').strftime('%Y.%m.%d %H:%M:%S')
                cmt['nickname'] = nickname.text.strip()
                cmt['contents'] = contents.text.strip()

                like = cmt_e.find_element_by_xpath('.//div[@class="act_wrap"]/a[1]')
                dislike = cmt_e.find_element_by_xpath('.//div[@class="act_wrap"]/a[2]')
                cmt['like'] = int(re.sub(r'[^0-9]', '', like.text))
                cmt['dislike'] = int(re.sub(r'[^0-9]', '', dislike.text))
                ...
                if inner_html.find('attach-multi-btn') > 0 or inner_html.find('attach-image-btn') > 0:
                    cmt_img_list = cmt_e.find_elements_by_xpath('.//button[@class="attach-multi-btn"] \
                ...
                # 답글 부분
                # 답글 여부 확인
                reply_list = cmt_e.find_elements_by_xpath('.//ul[@class="reply_list"]/li')
                ...
                for j, reply_e in enumerate(reply_list):
                    ...
                    try:
                        nickname = reply_e.find_element_by_xpath('.//div[@class="nick"]/b')
                        create_time = reply_e.find_element_by_xpath('.//div[@class="nick"]/span')
                        contents = reply_e.find_element_by_xpath('.//div[@class="comment_body"]')
                    except:
                        pass
                    else:
                        # 닉네임, 작성 일시, 내용, id
                        # 작성 일시의 경우 24시간이 지나지 않으면 'xx 시간전'으로 표기되어 다음 방법을 사용함)
                        reply['comment_id'] = reply_e.get_attribute('id').strip()

                        create_ts = create_time.text.strip().replace('오후', 'pm').replace('오전', 'am')
                        reply['create_ts'] = datetime.datetime.strptime(create_ts, '%Y-%m-%d %p %I:%M:%S').strftime('%Y.%m.%d %H:%M:%S')
    ...
    def get_article(self, title_e, msg, ndx):
        ...
        try:
            ...
            # 등록일
            se = self.get_by_xpath('//*[@id="articletitle"]/div/span/span')
            create_ts = re.match('입력\s+(\d+-\d+-\d+ \d+:\d+)', se.text.strip()).group(1)
            msg['create_ts'] = create_ts.replace('-', '.') + ":00"

            # 내용
            msg['contents'] = ''
            try:
                article_body = self.get_by_xpath('//*[@id="articlebody"]/div[1]')
            except:
                msg['contents'] = ''
            else:
            ...
            if self.config['params']['site']['capture_article']:
                ...
                if self.config['params']['kwargs']['headless']:
                    ...
                    e_heads = self.driver.find_elements_by_xpath('//div[@id="gnbsticky-id"]')
    ...
    def get_page(self):
        try:
            ...
            # 검색 결과물 페이지 읽기
            e = self.get_by_xpath('//*[@id="wrap"]/div[2]//div[@class="ListUnit-styles__SearchSection-sc-12r1wue-15 jgHnGm sub"]/ul')
            ...
            for i in range(len(bil)):
                ...
                try:
                    ...
                    # 게시글 제목
                    try:
                        title = bi.find_element_by_xpath('.//div/a/strong')
                    except:
                        msg['title'] = ''
                    else:
                        if title.text.strip() == '':
                    ...
                    # 게시글 URL
                    try:
                        article_url = bi.find_element_by_xpath('.//div/a[@href]')
                    except:
                        msg['article_url'] = ''
                    else:
                    ...
                    # 게시글 id
                    msg['article_id'] = re.search('news_id=(.+?)$', msg['article_url']).group(1)

                    # 작성자(기자 혹은 기관) : 간혹 없는 경우 발생
                    try:
                        se = bi.find_element_by_xpath('.//div/div/span[@class="editor"]')
                        author = re.match('(\[.+?\])\s+(.+?)\s+기자', se.text.strip()).group(2).strip()
                    except:
                        msg['author'] = ''
                    else:
                        msg['author'] = author
    ...
    def next_page(self):
        try:
            # 페이지 목록 구하기
            try:
                ple = self.get_by_xpath('//ul[@class="clfix _pagination"]')
                pa_list = ple.find_elements_by_xpath('./li')
            except:
                # 페이지가 없는 경우
                self.is_done = True
                return
            ...
            for pa in pa_list:
                ...
                if is_current:
                    ...
                    nple = self.get_by_xpath('//ul[@class="clfix _pagination"]')
                    for npa in nple.find_elements_by_xpath('./li'):
    ...
    def start(self):
        try:
            ...
            # 검색창 열기
            e = self.get_by_xpath('//*[@id="header"]/div[2]/div[2]/div/ul[2]/li[5]/a')
            self.safe_click(e)
            self.implicitly_wait(after_wait=1)

            # 검색어 입력 및 리턴키
            e = self.get_by_xpath('//*[@id="header"]/div[2]/div[2]/div/ul[2]/li[5]/div/span/div/input')
            search_key = self.config['params']['site']['search']
            self.send_keys(e, search_key + Keys.RETURN)
            self.implicitly_wait(after_wait=1)

            # '뉴스 기사' 클릭
            e = self.get_by_xpath('//*[@id="wrap"]/div[2]/div/ul/li[5]/a')
            self.safe_click(e)
            self.implicitly_wait(after_wait=1)

            # 정렬기준 설정: 최신순
            e = self.get_by_xpath('//*[@id="wrap"]/div[2]/div/div[2]/div/main/div/ul/li[1]/ul//li/button')
            self.safe_click(e)
            self.implicitly_wait(after_wait=1)
            # e = self.get_by_xpath('//*[@id="content"]/div[3]/div[1]/div[1]/ul/li[1]/div/ul/li[2]/a')
            # self.safe_click(e)
            # self.implicitly_wait(after_wait=1)

            # 검색범위 설정: JTBC 뉴스만
            # e = self.get_by_xpath('//*[@id="content"]/div[3]/div[1]/div[1]/ul/li[2]/a')
            # self.safe_click(e)
            # self.implicitly_wait(after_wait=1)
            # e = self.get_by_xpath('//*[@id="content"]/div[3]/div[1]/div[1]/ul/li[2]/div/ul/li[2]/a')
            # self.safe_click(e)
            # self.implicitly_wait(after_wait=1)

            # 검색분야 설정: 사회
            e = self.get_by_xpath('//*[@id="wrap"]/div[2]/div/div[2]/div[1]/main/div[1]/ul/li[2]/ul/li[1]/button/span[1]')
            self.safe_click(e)
            self.implicitly_wait(after_wait=1)
            e = self.get_by_xpath('//*[@id="wrap"]/div[2]/div/div[2]/div[1]/main/div[1]/ul/li[2]/ul/li[1]/div/ul/li[4]/a')
            self.safe_click(e)
            self.implicitly_wait(after_wait=1)

            # 검색항목 설정: 전체
            e = self.get_by_xpath('//*[@id="wrap"]/div[2]/div/div[2]/div[1]/main/div[1]/ul/li[2]/ul/li[2]/button/span[1]')
            self.safe_click(e)
            self.implicitly_wait(after_wait=1)
            e.a = self.get_by_xpath('//*[@id="wrap"]/div[2]/div/div[2]/div[1]/main/div[1]/ul/li[2]/ul/li[2]/div/ul/li[1]/a')
            self.safe_click(e)
            self.implicitly_wait(after_wait=1)
...
################################################################################
def do_start(**kwargs):
    with JtbcSearch(kwargs['config_f']) as ws:
        ws.start()
