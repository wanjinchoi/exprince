...
class DCinsideSearch(PySelenium):
    ...
    def search(self):
        ...
        self.safe_click(e)
        # 게시물 검색 클릭
        e = self.get_by_xpath('//*[@id="top"]/div/nav/ul/li[6]',
                              cond='element_to_be_clickable')
        self.safe_click(e)
                        ...
                        for j, re_cmt in enumerate(re_cmts):
                            ...
                            if e_del_reply.find('del_reply') > 0:
                                ...
                                if inner_html.find('video') > 0:
                                    ...
                                    d = re_img.find_element_by_tag_name('source')
                                    cmt['comment_img_url'].append(d.get_attribute('src'))
    ...
    def get_article(self, msg, ndx):
        try:
            ...
            # 댓글수
            e = content_e.find_element_by_xpath('.//span[@class="gall_comment"]')
            msg['num_comments'] = int(e.text.split()[1].replace(',', ''))
        ...
        try:
            ...
            for i in range(1, row_max + 1):
                ...
                try:
                    ...
                    msg['article_id'] = url.split('&no=')[1]
                    #msg['article_id'] = url.split('/')[5]
                    # 3) 갤러리이름 : board_name
                    gal = self.get_by_xpath('//div[@class="page_head clear"]//h2/a')
                    gal_name = gal.text.strip()
                    msg['site_name'] = gal_name
                    msg['board_name'] = gal_name
                    content_e = self.get_by_xpath('//div[@class="view_content_wrap"]')
                    # 제목
                    e = self.get_by_xpath('//span[@class="title_subject"]')
...
if __name__ == '__main__':
    _config_f = r'C:\work\voc\pySelenium\old_data\디시인사이드\dcinside.yaml'
