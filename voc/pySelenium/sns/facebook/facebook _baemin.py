...
################################################################################
class BaeminFacebookSearch(PySelenium):
    # ==========================================================================
    def __init__(self, config_f):
        ...
        if not os.path.exists(log_d):
            os.makedirs(log_d)
        logger = get_logger(self.get_safe_path(log_d, 'BaeminFacebookSearch.log'),
        ...
        self.logger.info(f'Starting BaeminFacebook Crawaling... with '
                         f'config:\n{out_config}')
    ...
    def search(self):
        # 배민 페이지
        self.driver.get('https://www.facebook.com/smartbaedal')

        # 상단 배너 삭제
        es = self.driver.find_elements_by_xpath('//div[@role="banner"]|'
                                                '//div[@class="rq0escxv du4w35lb rek2kq2y lpgh02oy"]')
    ...
    def get_article(self, msg, ndx, e_ab):
        try:
            ...
            article_id = re.sub(r'[^0-9]', '', msg['create_ts']) + '_' + msg['author']
            msg['article_id'] = re.sub(r'[\/:*?"<>|]', '', article_id)

            self.logger.info(f'Page[{self.cur_page}:{ndx+1}],article_id[{msg["article_id"]}],'
                             f'create_ts = {msg["create_ts"]}')
            ...
            try:
                while True:
                    # 더보기
                    e = self.get_by_xpath('//div[@class="k4urcfbm dp1hu0rb d2edcug0 cbu4d94t j83agx80 bp9cbjyn"]/div',
                                          timeout=3, wait_until_valid_text=True)
...
################################################################################
def do_start(**kwargs):
    with BaeminFacebookSearch(kwargs['config_f']) as ws:
        ws.start()
        return 0
...
if __name__ == '__main__':
    _config_f = 'facebook_baemin.yaml'
