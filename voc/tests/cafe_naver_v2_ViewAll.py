...
#
#  * [2024/01/17]
#     - 게시글 목록에 노출되는 게시글 수 50개 씩으로 변경
#  * [2023/12/14]
#     - '전체글보기'로 수집
#  * [2023/12/08]
#     - '답글 게시글' 스킵 추가
#  * [2023/11/30]
#     - 조회수 노출 오류 처리 - 숫자를 제외한 문자가 있으면 조회수 0으로 치환 (EXTVOCGE-1367)
#  * [2023/11/01]
#     - 게시글 내부 이미지 수집 오류 회피 로직 버그 수정
#  * [2023/11/01]
#     - 네이버 로그인 확인 추가
#  * [2023/10/11]
#     - 변경한 웹 크롤링 방식 수집 모듈 추가
#       카페 접속 -> 반복 ( 키워드 검색 -> 수집 ) -> 크롬 브라우저 종료
#  * [2023/06/27]
...
class NaverCafeSearch(PySelenium):
    ...
    def __init__(self, config_f, keyword, i):
        ...
        folder_name = "__".join(
            [str(self.config['params']['site']['site_number']),
             self.config['params']['site']['stu_timestamp']]
        )
    ...
    def search(self):
        ...
        e = self.get_by_xpath('//*[@id="menuLink0"]')
        self.safe_click(e)
        ...
        # 게시글 목록 개수 50개씩 으로 변경
        for i in e.find_elements_by_xpath('.//ul[@class="select_list"]//li'):
            t_se = re.sub(r'[^0-9]', '', i.text.strip())
            if t_se == '50':
                self.safe_click(i)
                self.implicitly_wait(after_wait=1)
                break
        ...
        # iframe 변경해서 스크린 샷 안찍히므로 default로 변경
                        ...
                        for j, sub_e in enumerate(e.find_elements_by_xpath('.//img[@class="article_img ATTACH_IMAGE"]')):
                            ...
                            if not sub_e_url:
                                ...
                                try:
                                    ...
                                    urllib.request.urlretrieve(sub_e_url, art_img_f)
                                    msg['image_list'].append(f'{j}.png')
    ...
    def get_page(self):
        try:
            ...
            s_shot = self.config['target']['folder'].replace('pool', 'logs') + f'_{self.cur_page}.png'
            ...
            # 페이지 테이블 구해오기
            e = self.get_by_xpath('//*[@id="main-area"]/div[4]/table/tbody')
            ...
            for i in range(len(bil)):
                ...
                try:
                    ...
                    # '답글 게시글' 스킵
                    element = bi.find_element_by_xpath(".//td")
                    class_attribute = element.get_attribute("class")
                    if 'bg-color' in class_attribute:
                        self.logger.info('답글 게시글 스킵------------------------')
                        continue
                    ...
                    # 전체 보기 게시글 ID
                    url_e = title_e.get_attribute('href')
                    match = re.search(r'articleid=(\d+)', url_e)
                    msg['article_id'] = self.cafe_id+'_'+match.group(1)
                    ...
                    # 조회수에 숫자를 제외한 문자가 있는지 확인
                    if re.search(pattern, se_a):
                        msg['view_count'] = 0
                    else:
    ...
    def save(self):
        ...
        if 'stu_timestamp' in self.output['config']['params']['site']:
            del self.output['config']['params']['site']['stu_timestamp']
...
if __name__ == '__main__':
    ...
    # timestamp = '20231220-170000'
    # collect_folder = 'C:/work/voc_data/pool/카페/네이버'
