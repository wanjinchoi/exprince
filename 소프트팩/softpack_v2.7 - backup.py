    ...
    for i in range(len(ws['C']) - 1):
        ...
        if math.floor(int(price) / 120000) > 0:
            if ws['B' + str(i + 2)].value not in add_invoice_list:
                ...
                add_count += 1
    new_wb.save(r"C:\Users\다현짱\Downloads\결과물\추가박스_" + time.strftime(
            "%H") + "시.xlsx")
...
def json2excel(json):
    ...
    for i in range(all_length):
        ...
        ws['B' + str(i + 2)].value = sample['SaleList'][i]['BulkDatas'][
            'EMP_CD']
        ws['C' + str(i + 2)].value = sample['SaleList'][i]['BulkDatas'][
            'PROD_CD']
        ws['D' + str(i + 2)].value = sample['SaleList'][i]['BulkDatas'][
            'PROD_DES']
        ws['E' + str(i + 2)].value = sample['SaleList'][i]['BulkDatas']['QTY']
        try:
            ws['F' + str(i + 2)].value = sample['SaleList'][i]['BulkDatas'][
                'U_TXT1']
        except KeyError:
            pass
        try:
            ws['G' + str(i + 2)].value = sample['SaleList'][i]['BulkDatas'][
                'PJT_CD']
        except KeyError:
            pass
        ...
        ws['J' + str(i + 2)].value = sample['SaleList'][i]['BulkDatas'][
            'VAT_AMT']
        ws['K' + str(i + 2)].value = sample['SaleList'][i]['BulkDatas'][
            'U_MEMO3']
        ...
        for j in range(len(price_list)):
            ...
            if json_tmp['PROD_CD'] == price_list[j]['code']:
                if '%' in json_tmp['PRICE']:
                    ...
                    json_tmp['PRICE'] = ((100 - int(percentage[0])) / 100) * \
                                        price_list[j]['price']
                else:
                    json_tmp['PRICE'] = price_list[j]['price']
                json_tmp['SUPPLY_AMT'] = int(float(json_tmp['QTY'])) * int(
                    float(json_tmp['PRICE']))
...
def make_json(input_path, mapping_file, mapping_file2):
    ...
    for i in range(rows - 1):
        ...
        name_and_phone = ws['L' + str(i + 2)].value + "/" + ws[
            'P' + str(i + 2)].value
        try:
...
def main(input_csv):
    start = time.time()
    mapping_file = r"C:\ARGOSRPA\mapping\SNmapping.xlsx"
    mapping_file2 = r"C:\ARGOSRPA\mapping\mapping2.xlsx"
    # xlsx = csv2xlsx(input_csv)
    jsons = make_json(input_csv, mapping_file, mapping_file2)
