...
# 처리한 List는 DB에 저장
def insert_value(pk, name, phone, inert_date, login_id):
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    query = "INSERT INTO success VALUES(?, ?, ?, ?, ?)"
    cur.execute(query, (pk, name, phone, inert_date, login_id))
    conn.commit()
    conn.close()
...
def findImage(folder_path):
    ...
    for i in listdir:
        ...
        if os.path.isfile(file_path) and i.endswith(('.jpg', '.png','.JPEG','.JPG','.jpeg')):
    ...
    for i in range(len(json_data['images'][0]['fields'])):
        ...
        if '-' in id_num and id_num.count('-') == 1:
            ...
            if abs(coordinates[0][0]['x'] - coordinates[0][2]['x']) > abs(
                ...
                time.sleep(2)
                im = cv2.imread(path)
                coordinates_50_percentage = coordinates_50_percentage - 90
                ...
                pass
            if '-' in id_num and id_num.count('-') in (1, 2, 3):
                coordinates = []
                replace_num = id_num.replace("-", "")
                if replace_num.isdigit():
                ...
                if (int(s_i_n[-1]) == parity_check and 0 < int(s_i_n[6]) < 7 and int(s_i_n[4] + s_i_n[5]) < 32) or (
                        int(s_i_n[0]) == 2 and int(
...
def crawlingPage(folder_path, login_id, login_pw, announce, announce_date):
    ...
    # driver.get("https://www.fordrivers.or.kr/admin")  # 페이지 접속
    ...
    # driver.find_element(By.XPATH, '/html/body/div[2]/div[2]/div[1]/div[2]/ul/li[2]/a').click() # 신청자 서류등록 메뉴 있을 때 이미지 앨범 선택
    # driver.find_element(By.XPATH, '/html/body/div[2]/div[2]/div[3]/form/div[1]/table/tbody/tr[1]/td[1]/img').click()  # 공고조회
    ...
    # select = Select(driver.find_element(By.XPATH, '//*[@id="search_bizId"]'))
    # select.select_by_visible_text(announce)  # 사업명 선택
    # logger.info(announce, "사업명 선택 완료")
    driver.find_element(By.XPATH,'/html/body/div[3]/form/div/table/tbody/tr[1]/td[1]/select').click()    #공고조회팝업-사업명
    driver.find_element(By.XPATH, '/html/body/div[3]/form/div/table/tbody/tr[1]/td[1]/select/option[11]').click()    #주유상품권
    driver.find_element(By.XPATH, '/html/body/div[3]/form/table/tbody/tr/th[2]/img').click()    #조회
    driver.find_element(By.XPATH, '/html/body/div[3]/div/table/tbody/tr[2]/td[1]').click()      #최상단클릭
    driver.switch_to.window(driver.window_handles[0])
    driver.implicitly_wait(10)
    # driver.find_element(By.XPATH, '/html/body/div[3]/div/table/thead/tr[3]/td[0]').click()  # 위에서 첫 번째 리스트 선택
    # driver.find_element(By.XPATH, '/html/body/div[3]/div/table/thead/tr[2]/td[1]').click()  # 위에서 첫 번째 리스트 선택
    # driver.switch_to.window(driver.window_handles[0])
            ...
            if len(isInsert) == 0:  # DB에 저장된적 없거나, 검색일이 DB기준일 보다 큰 경우 재업로드라 판단하여 재시작
                ...
                for o in range(len(images)):
                    ...
                    try:
                        ...
                        try:
                            sucess_check = redactImage(response.json(),folder_path + "\\image" + str(o) + ".jpg",popup_name + "_" + popup_phone, FOLDER_NAME)
                ...
                if len(isInsert) == 0:
                    insert_value(popup_license + popup_phone[3:],
                                 ...
                                 str(datetime.now()), login_id)     ##successcheck
                else:
                    update_value(popup_license + popup_phone[3:])
            driver.switch_to.window(driver.window_handles[0])

            # ############### Step 6: 인식불가 이미지 문자 발송 (실제 가동 전 엑셀로 대체) ###################
            # alert_str = ""
            # if len(alert_list) > 0:
            #     for a in alert_list:
            #         alert_str = alert_str + a + " "
            #     insertExcel(folder_path + "\\" + "문자리스트.xlsx",
            #                 popup_name,
            #                 popup_birth, alert_str)

            # ################################## 문자 발송 ################################################
            # driver.find_element(By.XPATH, '//*[@id="t_info"]/table[4]/tbody/tr[2]/td/input')\
            #     .send_keys("아래 서류를 인식할 수 없습니다.\n" + alert_str)
            # driver.find_element(By.XPATH, '//*[@id="t_info"]/table[4]/tbody/tr[2]/td/a/img').click()
            ############################################################################################
