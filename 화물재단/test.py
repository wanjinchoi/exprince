...
def crawlingPage(folder_path, login_id, login_pw, announce, announce_date):
    ...
    #driver.find_element(By.XPATH, '/html/body/div[2]/div[2]/div[3]/form/div[2]/table/tbody/tr[1]/td[9]/img').click()
    ...
    #마지막 숫자가져오기
    last_number= int(driver.find_element(By.XPATH,'/html/body/div[2]/div[2]/div[3]/form/strong').text)
    #다시 첫번째 페이지로
    ...
    while True:
        ...
        if len(table) < 2:  # 접수내역이 없을 시 시나리오 종료
            driver.implicitly_wait(30)  # 로딩 완료 대기
            logger.info("처리할 리스트 없음 시나리오 종료")
            logger.info("20250207 효은, 이 if문 break를 주석처리하여 임시로 동작")
            # break
        for i in range(len(table)):
            ...
            if len(isInsert) == 0:  # DB에 저장된적 없거나, 검색일이 DB기준일 보다 큰 경우 재업로드라 판단하여 재시작
                for j in range(len(image_count)):
                    ...
                    logger.info("list이름",compare_list)
                    try:
                        logger.info("여기까지 왔음8")
                        logger.info(str(image_count[j]['alt']))
                        os.rename('C:\\Users\\user\\Downloads' + "\\" + compare_list[0],folder_path + "\\" + str(image_count[j]['alt']))
                        logger.info("여기까지 왔음9")
                        logger.info('C:\\Users\\user\\Downloads' + "\\" + compare_list[0], folder_path + "\\" + str(image_count[j]['alt']))
                    except Exception as e:
                        logger.info(e, "이미지 URL이 올바르지 않습니다.")
                        ### 20250207 효은, 이 try문이 except 안에 있엇는데 주석처리 하고 밑의 except문을 바깥으로 뺌
                        # try:
                ...
                for o in range(len(images)):
                    ...
                    try:
                        ...
                        os.rename(folder_path + "\\" + images[o],folder_path + "\\image" + str(o) + ".jpg")
                        try:
                            logger.info("여기까지 왔음10")
                            sucess_check = redactImage(response.json(),folder_path + "\\image" + str(o) + ".jpg",popup_name + "_" + popup_phone, FOLDER_NAME)
                            logger.info("여기까지 왔음11")
                        except Exception as e:
                            #logger.info(e, "패리티체크 실패")
                            logger.info(e)
                            alert_list.append(images[o])
                            os.rename(
                ...
                for k in upload_list:
                    logger.info(os.path.splitext(k)[0], k)
                ...
                driver.close()
                if len(isInsert) == 0:
                    logger.info(popup_license + popup_phone[3:],popup_name, popup_phone,str(datetime.now()), login_id,sucess_check,getimage)
                    insert_value(popup_license + popup_phone[3:],popup_name, popup_phone,str(datetime.now()), login_id,sucess_check,getimage)
                else:
