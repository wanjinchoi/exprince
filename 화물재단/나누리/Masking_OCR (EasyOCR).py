...
def callAPI(file_path):
    API_URL = 'https://easyocr.argos-labs.com' # 성역
    TaskRequestUrl = '/ocr?api_token='
    TaskResultUrl = '/tasks/'
    Api_token = "eec91eda92ef8e1d1d176578df65f4a5"

    # RequestException : https: // requests.readthedocs.io / en / latest / user / quickstart /
    while True:
        try:
            ...
            time.sleep(3)
        except Exception as e:  # 위 에러에서 탐지 되지 않은 모든 에러 핸들링
            print("Exception : ", e)
            time.sleep(3)
        else:   # 에러 발생 안 났을 때, 정상작동할 때 수행할 코드 분기
            # status code 400, 500 예외처리 필요?
            print(response.json()) # {'task_id': '9ae84a8b-7d7c-4845-b216-2af04971606b'}
            break
    ...
    while True:
        ...
        if task_result_data['task_status'] == 'SUCCESS':
            ...
            time.sleep(3)
            print("당첨 ", task_result_data['task_status'])
